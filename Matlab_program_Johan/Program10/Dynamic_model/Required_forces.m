...
%         phi_ddot_R(i_int) = X_ddot_phi*a_phi_R(:,i);
    ...
    F_I_acc_mean = zeros(3,nr_wb);  % Inertia_f.F_acc_st;
    F_I_vel_mean = zeros(3,nr_wb);  % Inertia_f.F_vel_st;
    M_I_acc_mean = zeros(3,nr_wb);  % Inertia_f.M_acc_st;
    M_I_vel_mean = zeros(3,nr_wb);  % Inertia_f.M_vel_st;
    F_I_mean = zeros(3,nr_wb);      % Inertia_f.F_I_st;
    M_I_mean = zeros(3,nr_wb);      % Inertia_f.M_I_st;
    F_a_L_mean = zeros(3,nr_wb);    % Aero_f.F_L_st;
    F_a_R_mean = zeros(3,nr_wb);    % Aero_f.F_R_st;
    M_a_L_mean = zeros(3,nr_wb);    % Aero_f.M_L_st;
    M_a_R_mean = zeros(3,nr_wb);    % Aero_f.M_R_st;
    F_g_mean = zeros(3,nr_wb);      % gravity_f.Fg_st;   
