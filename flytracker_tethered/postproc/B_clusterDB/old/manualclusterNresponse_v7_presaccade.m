...
savename = ([loadname(1:end-4),'_9clusters_',num2str(At_hor_thresh),'n',num2str(At_hor_thresh_min),'n',num2str(An_hor_thresh),'_startframe',num2str(start_frame),'_strokeplane',num2str(-strokeplane_angle),'deg.mat']);
...
toplot=0;
if toplot == 1
    figdir = ['accel_plots_2.5n-3n2.5_startframe',num2str(start_frame)];
    mkdir(figdir)
end
...
roll_dot_body  = rad2deg(pathDB.omega(:,:,1));
pitch_dot_body = rad2deg(pathDB.omega(:,:,2));
yaw_dot_body   = rad2deg(pathDB.omega(:,:,3));
...
for j = 1:size(x,2)
    ...
    for i = 1:size(x,1)
        if isnan(x(i,j))==0
            ...
            omega_turn_axis_body(i,j) = rad2deg(atan2(yaw_dot_body(i,j),pitch_dot_body(i,j)));
            
            omega_turn_sp(i,j) = norm([pitch_dot_sp(i,j),yaw_dot_sp(i,j)]);
            omega_turn_axis_sp(i,j) = rad2deg(atan2(yaw_dot_sp(i,j),pitch_dot_sp(i,j)));
            
            % presaccade: no trigger frame
%             if i > trigger_frame
            if i > 1
                % previous d's
...
%% run responseDB file
batch_calc_responseDB_presaccade
