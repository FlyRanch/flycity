...
savedir = ['3Dflightpaths_topview_tetamax',num2str(tetamax),'_',num2str(fps),'fps'];
mkdir(savedir)
cd(savedir)


% movfilename_top = ['3Dflightpaths_topview_tetamax',num2str(tetamax),'_',num2str(fps),'fps.avi'];
% movfilename_4views = ['3Dflightpaths_4views_tetamax',num2str(tetamax),'_',num2str(fps),'fps.avi'];
...
%     zlabel('z')
 ...
 axis off
    ...
    for k = start+movie_step:movie_step:stop
        ...
        if k<trigger_frame
            for m = 1:size(x,2)
                plot([x(k-movie_step,m) x(k,m)],[y(k-movie_step,m) y(k,m)],'b-','linewidth',linewidth_path)
            end
        else
            for m = 1:size(x,2)
                plot([x(k-movie_step,m) x(k,m)],[y(k-movie_step,m) y(k,m)],'r-','linewidth',linewidth_path)
            end
        end
        
        % expansion
        if teta(k) > 0 && teta(k) > teta(k-movie_step)
...
%         mov = addframe(mov,gcf);
        ...
        print('-dtiff','-r500',['frame',num2str(k),'.tif']);
%         toc;
    end
