...
%         [Y_ci(n,1) Y_high(n,1) Y_low(n,1)] = circ_mean_deg_nonan(Y_sub);
        ...
        % quartiles
        Y_sort = sort(Y_sub);
        Y_sort = Y_sort(isnan(Y_sort)==0);
        
        Y_low(n,1) = Y_sort(round(.25*length(Y_sort)));
        Y_ci(n,1) = Y_sort(round(.5*length(Y_sort)));
        Y_high(n,1) = Y_sort(round(.75*length(Y_sort)));
    else
        Y_ci(n,1) = nan;
        Y_high(n,1) = nan;
        Y_low(n,1) = nan;
...
% alpha(.25)
plot(angle_pre_mean,angle_post_mean,'-','linewidth',1,'color','k')
