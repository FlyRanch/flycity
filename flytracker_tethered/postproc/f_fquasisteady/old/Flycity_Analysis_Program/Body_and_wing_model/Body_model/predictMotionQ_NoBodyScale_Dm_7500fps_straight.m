function [Pnew,tnew] = predictMotionQ_NoBodyScale_Dm_7500fps_straight(Pfull)
...
if isempty(data)
    ...
    %Dm @ 7500fps
%     load('20120702_Dm_7.5Kfps_straight.mat') % straight flight, tracker data
%     load('20120702_Dm_7.5Kfps_straight_manual.mat') % straight flight, manual data
%     
%     data = [stateQ(1:3,:) stateQ_manual(1:3,:)];
%     dataQ = [stateQ(4:15,:) stateQ_manual(4:15,:)];
%     transidx = [size(stateQ,2) size(stateQ_manual,2)];
%     transidx = cumsum(transidx);
    
    load('MovModel_20120702_Dm_7500fps_straight.mat') % Dm, straight flight, manual data
    
    data = [ManualFit.solnQ(1:3,:)];
    dataQ = [ManualFit.solnQ(4:15,:)];
    transidx = [size(ManualFit.solnQ,2)];
    transidx = cumsum(transidx);
