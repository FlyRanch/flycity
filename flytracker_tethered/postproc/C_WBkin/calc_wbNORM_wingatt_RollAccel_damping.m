...
for wb = 1:size(wb_nr,1)
    counter = size(wb_nr,1) -wb
        ...
        if roll_dot_dot_mean_wb(wb) > rollaccel_limit_mod
            ...
            % damping
            rolldamp_mean_wb_now = k_damp_roll * rollvel_mean_wb_now;
            pitchdamp_mean_wb_now = k_damp_pitch * pitchvel_mean_wb_now;
            yawdamp_mean_wb_now = k_damp_yaw * yawvel_mean_wb_now;
            ...
            freqMOD_wb_now = (nanmean(f_wb_now) - f_wb_steady) / (rollaccel_mean_wb_now + rolldamp_mean_wb_now) * rollaccel_norm;
            
            strokeMOD_wb_up_now = (stroke_wb_up_interp - stroke_wb_steady) / (rollaccel_mean_wb_now + rolldamp_mean_wb_now) * rollaccel_norm;
            strokeMOD_wb_down_now = (stroke_wb_down_interp - stroke_wb_steady) / (rollaccel_mean_wb_now + rolldamp_mean_wb_now) * rollaccel_norm;
            strokeMOD_ds_up_now = (stroke_ds_up_interp - stroke_ds_steady) / (rollaccel_mean_wb_now + rolldamp_mean_wb_now) * rollaccel_norm;
            strokeMOD_ds_down_now = (stroke_ds_down_interp - stroke_ds_steady) / (rollaccel_mean_wb_now + rolldamp_mean_wb_now) * rollaccel_norm;
            strokeMOD_us_up_now = (stroke_us_up_interp - stroke_us_steady) / (rollaccel_mean_wb_now + rolldamp_mean_wb_now) * rollaccel_norm;
            strokeMOD_us_down_now = (stroke_us_down_interp - stroke_us_steady) / (rollaccel_mean_wb_now + rolldamp_mean_wb_now) * rollaccel_norm;
            
            pitchMOD_wb_up_now = (pitch_wb_up_interp - pitch_wb_steady) / (rollaccel_mean_wb_now + rolldamp_mean_wb_now) * rollaccel_norm;
            pitchMOD_wb_down_now = (pitch_wb_down_interp - pitch_wb_steady) / (rollaccel_mean_wb_now + rolldamp_mean_wb_now) * rollaccel_norm;
            pitchMOD_ds_up_now = (pitch_ds_up_interp - pitch_ds_steady) / (rollaccel_mean_wb_now + rolldamp_mean_wb_now) * rollaccel_norm;
            pitchMOD_ds_down_now = (pitch_ds_down_interp - pitch_ds_steady) / (rollaccel_mean_wb_now + rolldamp_mean_wb_now) * rollaccel_norm;
            pitchMOD_us_up_now = (pitch_us_up_interp - pitch_us_steady) / (rollaccel_mean_wb_now + rolldamp_mean_wb_now) * rollaccel_norm;
            pitchMOD_us_down_now = (pitch_us_down_interp - pitch_us_steady) / (rollaccel_mean_wb_now + rolldamp_mean_wb_now) * rollaccel_norm;
            
            devMOD_wb_up_now = (dev_wb_up_interp - dev_wb_steady) / (rollaccel_mean_wb_now + rolldamp_mean_wb_now) * rollaccel_norm;
            devMOD_wb_down_now = (dev_wb_down_interp - dev_wb_steady) / (rollaccel_mean_wb_now + rolldamp_mean_wb_now) * rollaccel_norm;
            devMOD_ds_up_now = (dev_ds_up_interp - dev_ds_steady) / (rollaccel_mean_wb_now + rolldamp_mean_wb_now) * rollaccel_norm;
            devMOD_ds_down_now = (dev_ds_down_interp - dev_ds_steady) / (rollaccel_mean_wb_now + rolldamp_mean_wb_now) * rollaccel_norm;
            devMOD_us_up_now = (dev_us_up_interp - dev_us_steady) / (rollaccel_mean_wb_now + rolldamp_mean_wb_now) * rollaccel_norm;
            devMOD_us_down_now = (dev_us_down_interp - dev_us_steady) / (rollaccel_mean_wb_now + rolldamp_mean_wb_now) * rollaccel_norm;
            
            DstrokeMOD_wb_now = Dstroke_wb_interp / (rollaccel_mean_wb_now + rolldamp_mean_wb_now) * rollaccel_norm;
            DstrokeMOD_ds_now = Dstroke_ds_interp / (rollaccel_mean_wb_now + rolldamp_mean_wb_now) * rollaccel_norm;
            DstrokeMOD_us_now = Dstroke_us_interp / (rollaccel_mean_wb_now + rolldamp_mean_wb_now) * rollaccel_norm;

            DpitchMOD_wb_now = Dpitch_wb_interp / (rollaccel_mean_wb_now + rolldamp_mean_wb_now) * rollaccel_norm;
            DpitchMOD_ds_now = Dpitch_ds_interp / (rollaccel_mean_wb_now + rolldamp_mean_wb_now) * rollaccel_norm;
            DpitchMOD_us_now = Dpitch_us_interp / (rollaccel_mean_wb_now + rolldamp_mean_wb_now) * rollaccel_norm;

            DdevMOD_wb_now = Ddev_wb_interp / (rollaccel_mean_wb_now + rolldamp_mean_wb_now) * rollaccel_norm;
            DdevMOD_ds_now = Ddev_ds_interp / (rollaccel_mean_wb_now + rolldamp_mean_wb_now) * rollaccel_norm;
            DdevMOD_us_now = Ddev_us_interp / (rollaccel_mean_wb_now + rolldamp_mean_wb_now) * rollaccel_norm;
            ...
            rolldamp_RollAccel(n_now,1) = rolldamp_mean_wb_now;
            pitchdamp_RollAccel(n_now,1) = pitchdamp_mean_wb_now;
            yawdamp_RollAccel(n_now,1) = yawdamp_mean_wb_now;
