function [xh, Sx, pNoise, oNoise, InternalVariablesDS,Features] = srcdkf_const_variablestep_NOWING(state, Sstate, pNoise, oNoise, InferenceDS,frames,useIC,plot_data)
...
%     Features = feat_detect(i,[],PAR);
    Features = feat_detect_NOWING(i,[],PAR);
    ...
    if i > PAR.startframe + PAR.framesample*PAR.NumPrevSol -1
        InferenceDS.model.Fmode = 'predict';
        %load the previous solutions
        recalcIC_Joint([],i-PAR.framesample*PAR.NumPrevSol:PAR.framesample:i-PAR.framesample*2,PAR,'load');
    else
    ...
    % linearize HFUN
    ITERMAX = 40; % maximum # of filter iterations on ith time step
...
%     if round(i/100) > round((i-PAR.framesample)/100)

    % plot every 10th frame
    if round(i/10) > round((i-PAR.framesample)/10)
