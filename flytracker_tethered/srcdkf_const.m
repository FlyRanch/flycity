...
if (exist([PAR.solutionpath PAR.solutiondirname '/fly' sprintf(['%0',num2str(PAR.digits),'d'], (frames(1)-PAR.framesample)) ...
    '.mat'],'file') == 2 && useIC == false)

    %If there exists a solution from the previous time step, load it into
    %memory.
    load([PAR.solutionpath PAR.solutiondirname '/fly' sprintf(['%0',num2str(PAR.digits),'d'], (frames(1)-PAR.framesample)) ...
        '.mat']);
    if frames(1) ~= 1
...
for i=frames(1):PAR.framesample:frames(2)
   ...
   if i == frames(1)+1
    ...
    if i > PAR.startframe+PAR.NumPrevSol-1
    ...
    % Edited 6.6.2013 to fix edgepoints error (M.George)
    % Original code:
    % [xbar,InferenceDS]  = feval(InferenceDS.ffun,InferenceDS,state,zeros(nv,1), UU1);
    % New code:
    [xbar,InferenceDS]  = feval(InferenceDS.ffun,InferenceDS,state,zeros(nv,1), UU1);  
    % xbar-state should always a matrix of zeros
    % for some reason xbar would randomly be three values an then zeros,
    % which would cause edgepoints error later in Closest_PtsNrmlGate1.m
    if sum(xbar(4:end)) ~= 0
    ...
    for n = 1:ITERMAX
        ...
        fprintf(['Data Association\n']);
            ...
            %% plot model projections FTMmod 20120607
%         Closest_PtsNrmlGate1_plot(Features,newstate,i,PAR);
%         pause(.1)
...
%             obs = [obs ; eta_scale.*reshape(Moments',[],1)];
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%
        if i >= frames(1)+1; newstate(1:7) = fixed_body_state; end;
%         newstate(1) = 3.909085934803541;
%         newstate(2) = 2.896271881686230;
%         newstate(3) = 1.966476382981551;
%         newstate(4) = 0.745686583292006;
%         newstate(5) = 0.659057729235611;
%         newstate(6) = 0.120855845599659;
%         newstate(7) = -0.106909610477284;
        %set body state to constant
        
        
        if i >= frames(1)+1; xh(1:7) = fixed_body_state; end;
%         xh(1) = 3.909085934803541;
%         xh(2) = 2.896271881686230;
%         xh(3) = 1.966476382981551;
%         xh(4) = 0.745686583292006;
%         xh(5) = 0.659057729235611;
%         xh(6) = 0.120855845599659;
%         xh(7) = -0.106909610477284;
...
if isfield(PAR,'plot_data')==0
    plot_data = true;
%     plot_data = false;
else
    plot_data = PAR.plot_data;    
end

if plot_data == 1
    
    % plot every 100th frame
    if true %round(i/100) > round((i-1)/100)

        %     Closest_PtsNrmlGate1_plotsave(Features,newstate,i,PAR);
        Closest_PtsNrmlGate1_plotsave_nozoom(Features,newstate,i,PAR);

        %Plot initial condition to check for correctness
        paste_imagefunQ_noManualFit(xh,i,PAR,Features);

        % save images
        saveas(figure(11),[PAR.solutionpath 'Projections/cam1/flyimage_' sprintf(['%0',num2str(PAR.digits),'d'], i) '.jpg'])
        saveas(figure(12),[PAR.solutionpath 'Projections/cam2/flyimage_' sprintf(['%0',num2str(PAR.digits),'d'], i) '.jpg'])
        saveas(figure(13),[PAR.solutionpath 'Projections/cam3/flyimage_' sprintf(['%0',num2str(PAR.digits),'d'], i) '.jpg'])
%         close all
%         pause(.1)
    end
end
%%
    ...
    save([PAR.solutionpath PAR.solutiondirname '/fly' sprintf(['%0',num2str(PAR.digits),'d'], i) '.mat'],'xh','InternalVariablesDS');
    save([PAR.solutionpath 'Features/Features' sprintf(['%0',num2str(PAR.digits),'d'], i) '.mat'],'Features');


    fprintf(['Finished with Frame ' num2str(i) '\n']);
    pathcheck = regexp(path,':','split');
    disp(pathcheck(2));
