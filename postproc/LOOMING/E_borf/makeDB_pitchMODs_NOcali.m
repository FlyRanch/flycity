...
for i = 1:length(list)
    ...
    if exist(file_now) == 2
        ...
        Fx_pitch(1:seq_length_max,end+1) = nan;
        Fy_pitch(1:seq_length_max,end+1) = nan;
        Fz_pitch(1:seq_length_max,end+1) = nan;
        Mx_pitch(1:seq_length_max,end+1) = nan;
        My_pitch(1:seq_length_max,end+1) = nan;
        Mz_pitch(1:seq_length_max,end+1) = nan;
...
%         Mz_now = (Mz_robo - Mz_cali_L - Mz_cali_R) * M_robo2fly;
        ...
        freq_pitch(n,1) = f_now;

        Fx_mean_pitch(n,1) = nanmean(Fx_now(n_start:n_stop));
        Fy_mean_pitch(n,1) = nanmean(Fy_now(n_start:n_stop));
        Fz_mean_pitch(n,1) = nanmean(Fz_now(n_start:n_stop));

        Mx_mean_pitch(n,1) = nanmean(Mx_now(n_start:n_stop));
        My_mean_pitch(n,1) = nanmean(My_now(n_start:n_stop));
        Mz_mean_pitch(n,1) = nanmean(Mz_now(n_start:n_stop));

        Astroke_pitch(n,1) = max([stroke_R_now;stroke_L_now]) - min([stroke_R_now;stroke_L_now]);
        Apitch_pitch(n,1) = max([pitch_R_now;pitch_L_now]) - min([pitch_R_now;pitch_L_now]);
        Adev_pitch(n,1) = max([dev_R_now;dev_L_now]) - min([dev_R_now;dev_L_now]);

        %% store pitch data
        t_pitch(1:length(t),n) = t_now;

        Fx_pitch(1:length(t),n) = Fx_now;
        Fy_pitch(1:length(t),n) = Fy_now;
        Fz_pitch(1:length(t),n) = Fz_now;

        Mx_pitch(1:length(t),n) = Mx_now;
        My_pitch(1:length(t),n) = My_now;
        Mz_pitch(1:length(t),n) = Mz_now;

        stroke_L_pitch(1:length(t),n) = stroke_L_now;
        pitch_L_pitch(1:length(t),n) = pitch_L_now;
        dev_L_pitch(1:length(t),n) = dev_L_now;

        stroke_R_pitch(1:length(t),n) = stroke_R_now;
        pitch_R_pitch(1:length(t),n) = pitch_R_now;
        dev_R_pitch(1:length(t),n) = dev_R_now;
    end
end
