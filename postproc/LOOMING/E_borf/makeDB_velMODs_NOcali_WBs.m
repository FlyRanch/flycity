...
% file_name = [save_name,'_vel_'];
list = dir([file_name,'*mat']);

% % init vars with nans
% t_vel = nan(seq_length_max,length(list));
% Fx_vel = nan(seq_length_max,length(list));
% Fy_vel = nan(seq_length_max,length(list));
% Fz_vel = nan(seq_length_max,length(list));
% Mx_vel = nan(seq_length_max,length(list));
% My_vel = nan(seq_length_max,length(list));
% Mz_vel = nan(seq_length_max,length(list));
% stroke_L_vel = nan(seq_length_max,length(list));
% pitch_L_vel = nan(seq_length_max,length(list));
% dev_L_vel = nan(seq_length_max,length(list));
% stroke_R_vel = nan(seq_length_max,length(list));
% pitch_R_vel = nan(seq_length_max,length(list));
% dev_R_vel = nan(seq_length_max,length(list));
...
for i = 1:length(list)
    ...
    if exist(file_now) == 2
        ...
        n=n+1;
        run_type_vel{n} = exp_type;
        mod_value_vel(n,1) = mod_val;
    
        t_vel(1:seq_length_max,end+1) = nan;
        
        stroke_L_vel(1:seq_length_max,end+1) = nan;
        pitch_L_vel(1:seq_length_max,end+1) = nan;
        dev_L_vel(1:seq_length_max,end+1) = nan;
        
        stroke_R_vel(1:seq_length_max,end+1) = nan;
        pitch_R_vel(1:seq_length_max,end+1) = nan;
        dev_R_vel(1:seq_length_max,end+1) = nan;
        
        Fx_vel(1:seq_length_max,1:Nwb,end+1) = nan;
        Fy_vel(1:seq_length_max,1:Nwb,end+1) = nan;
        Fz_vel(1:seq_length_max,1:Nwb,end+1) = nan;
        Mx_vel(1:seq_length_max,1:Nwb,end+1) = nan;
        My_vel(1:seq_length_max,1:Nwb,end+1) = nan;
        Mz_vel(1:seq_length_max,1:Nwb,end+1) = nan;
...
%         Mz_now = Mz_robo - Mz_cali_L_array - Mz_cali_R_array;
        ...
        %% calc wb mean data
        Fx_mean_vel(n,1) = nanmean(Fx_fly(:));
        Fy_mean_vel(n,1) = nanmean(Fy_fly(:));
        Fz_mean_vel(n,1) = nanmean(Fz_fly(:));

        Mx_mean_vel(n,1) = nanmean(Mx_fly(:));
        My_mean_vel(n,1) = nanmean(My_fly(:));
        Mz_mean_vel(n,1) = nanmean(Mz_fly(:));

        %% store vel data
        t_vel(1:length(t_now),n) = t_now;

        stroke_L_vel(1:length(t_now),n) = stroke_L_now;
        pitch_L_vel(1:length(t_now),n) = pitch_L_now;
        dev_L_vel(1:length(t_now),n) = dev_L_now;

        stroke_R_vel(1:length(t_now),n) = stroke_R_now;
        pitch_R_vel(1:length(t_now),n) = pitch_R_now;
        dev_R_vel(1:length(t_now),n) = dev_R_now;

        Fx_vel(1:length(t_now),:,n) = Fx_fly;
        Fy_vel(1:length(t_now),:,n) = Fy_fly;
        Fz_vel(1:length(t_now),:,n) = Fz_fly;

        Mx_vel(1:length(t_now),:,n) = Mx_fly;
        My_vel(1:length(t_now),:,n) = My_fly;
        Mz_vel(1:length(t_now),:,n) = Mz_fly;
    end
end

if length(t_vel) > seq_length_max
    seq_length_max
    'TOO SHORT'
end
