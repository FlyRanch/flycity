...
mkdir(['Fsteady_mov_figs2'])
cd(['Fsteady_mov_figs2'])
...
%         quiver(x_wing,y_wing,Fx_tip,Fy_tip,'linewidth',3,'color',F_map(i,:))
        h=quiver(x_wing,y_wing,Fx_tip,Fy_tip,'linewidth',3,'color',F_map(i,:));
        adjust_quiver_arrowhead_size(h, 5);
        hold on
        plot([x_LE x_TE],[y_LE y_TE],'-','linewidth',foil_width,'color',cord_map(i,:))
    ...
    axis([-90 90 -30 30]) 
    axis off
    saveas(gca, ['profNforce_Fsteady_',num2str(j),'.png'])
end
cd ..
