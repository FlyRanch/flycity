...
subset_vel = find(Fenhance_norm*mod_value_vel>=x_min-1-dmod & Fenhance_norm*mod_value_vel<=x_max-1+dmod );
...
plot(Fenhance_norm*mod_value_vel(subset_vel)+1,F_mean_vel(subset_vel)/Mg_fly,'-k','linewidth',1)
...
mkdir('MSfigs')
cd('MSfigs')
saveas(gca, 'MSfig_FenhanceINCvel_Mroll_Mpitch.fig')
saveas(gca, 'MSfig_FenhanceINCvel_Mroll_Mpitch.png')
plot2svg(['MSfig_FenhanceINCvel_Mroll_Mpitch.svg'])
cd ..
