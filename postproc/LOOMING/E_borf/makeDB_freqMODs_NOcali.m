...
for i = 1:length(list)
    ...
    if exist(file_now) == 2
        ...
        Fx_freq(1:seq_length_max,end+1) = nan;
        Fy_freq(1:seq_length_max,end+1) = nan;
        Fz_freq(1:seq_length_max,end+1) = nan;
        Mx_freq(1:seq_length_max,end+1) = nan;
        My_freq(1:seq_length_max,end+1) = nan;
        Mz_freq(1:seq_length_max,end+1) = nan;
...
%         Mz_now = (Mz_robo - Mz_cali_L - Mz_cali_R) * M_robo2fly;
        ...
        Fx_mean_freq(n,1) = nanmean(Fx_now(n_start:n_stop));
        Fy_mean_freq(n,1) = nanmean(Fy_now(n_start:n_stop));
        Fz_mean_freq(n,1) = nanmean(Fz_now(n_start:n_stop));

        Mx_mean_freq(n,1) = nanmean(Mx_now(n_start:n_stop));
        My_mean_freq(n,1) = nanmean(My_now(n_start:n_stop));
        Mz_mean_freq(n,1) = nanmean(Mz_now(n_start:n_stop));

        Astroke_freq(n,1) = max([stroke_R_now;stroke_L_now]) - min([stroke_R_now;stroke_L_now]);
        Apitch_freq(n,1) = max([pitch_R_now;pitch_L_now]) - min([pitch_R_now;pitch_L_now]);
        Adev_freq(n,1) = max([dev_R_now;dev_L_now]) - min([dev_R_now;dev_L_now]);

        %% store freq data
        t_freq(1:length(t),n) = t_now;

        Fx_freq(1:length(t),n) = Fx_now;
        Fy_freq(1:length(t),n) = Fy_now;
        Fz_freq(1:length(t),n) = Fz_now;

        Mx_freq(1:length(t),n) = Mx_now;
        My_freq(1:length(t),n) = My_now;
        Mz_freq(1:length(t),n) = Mz_now;

        stroke_L_freq(1:length(t),n) = stroke_L_now;
        pitch_L_freq(1:length(t),n) = pitch_L_now;
        dev_L_freq(1:length(t),n) = dev_L_now;

        stroke_R_freq(1:length(t),n) = stroke_R_now;
        pitch_R_freq(1:length(t),n) = pitch_R_now;
        dev_R_freq(1:length(t),n) = dev_R_now;
    end
end
