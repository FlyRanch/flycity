...
%             Mz_now(j,:) = Mz_robo(j,:) - Mz_cali_L(j) - Mz_cali_R(j);
        ...
        Fx_freq(1:length(t_now),:,n) = Fx_now;
        Fy_freq(1:length(t_now),:,n) = Fy_now;
        Fz_freq(1:length(t_now),:,n) = Fz_now;

        Mx_freq(1:length(t_now),:,n) = Mx_now;
        My_freq(1:length(t_now),:,n) = My_now;
        Mz_freq(1:length(t_now),:,n) = Mz_now;
    end
end
