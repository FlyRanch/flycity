...
%         counter = size(n_down_start_L,2)*100+size(n_down_start_L,1)-seq*100-wb
        ...
        if n_down_start_L_now > n_first(seq) && n_up_stop_L_now < n_post(seq) && ...
            ...
            pitch_mid_ds_L = pitch_ds_L_now(round(length(pitch_ds_L_now)/2));
            pitch_mid_ds_R = pitch_ds_R_now(round(length(pitch_ds_R_now)/2));
            dpitch_mid_ds = pitch_mid_ds_L - pitch_mid_ds_R;

            pitch_mid_us_L = pitch_us_L_now(round(length(pitch_us_L_now)/2));
            pitch_mid_us_R = pitch_us_R_now(round(length(pitch_us_R_now)/2));
