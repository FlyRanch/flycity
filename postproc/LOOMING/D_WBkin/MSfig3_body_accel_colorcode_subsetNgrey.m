...
for j=1:length(color_var)
    ...
    if isnan(color_var(i)) == 0 && color_var(i)~=0
        subplot(3,3,1)
        plot(t-t_shift(i),roll_plot(:,i),'-','color',[.75 .75 .75],'linewidth',.25)
        subplot(3,3,4)
        plot(t-t_shift(i),pitch_plot(:,i),'-','color',[.75 .75 .75],'linewidth',.25)
        subplot(3,3,7)
        plot(t-t_shift(i),yaw_plot(:,i),'-','color',[.75 .75 .75],'linewidth',.25)
    end
end
