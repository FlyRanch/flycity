...
for seq = 1:size(n_down_start_L,2)
    for wb = 1:size(n_down_start_L,1)-1
        
%         seq
%         wb
        
        n_start_L_now = n_down_start_L(wb,seq);
        n_stop_L_now = n_down_start_L(wb+1,seq);
        
        n_start_R_now = n_down_start_R(wb,seq);
        n_stop_R_now = n_down_start_R(wb+1,seq);
        
        % plot only within steady&maneuver
        if n_start_L_now > n_first(seq) && n_stop_L_now < n_post(seq) && ...
                n_start_R_now > n_first(seq) && n_stop_R_now < n_post(seq)
            
            if IDX(n_start_L_now,seq) == 5
                plot_color = 'k';
            else
                plot_color = 'r';
            end
            
            stroke_L_now = stroke_L_mirror(n_start_L_now:n_stop_L_now,seq);
            stroke_R_now = stroke_R_mirror(n_start_R_now:n_stop_R_now,seq);

            pitch_L_now = pitch_L_mirror(n_start_L_now:n_stop_L_now,seq);
            pitch_R_now = pitch_R_mirror(n_start_R_now:n_stop_R_now,seq);
            
            dev_L_now = dev_L_mirror(n_start_L_now:n_stop_L_now,seq);
            dev_R_now = dev_R_mirror(n_start_R_now:n_stop_R_now,seq);
            ddev_now =   ddev_mirror(n_start_L_now:n_stop_L_now,seq);
            
            xi = [1:length(ddev_now)/100:length(ddev_now)];
            
            stroke_L_interp = interp1(stroke_L_now,xi);
            stroke_R_interp = interp1(stroke_R_now,xi);
            
            pitch_L_interp = interp1(pitch_L_now,xi);
            pitch_R_interp = interp1(pitch_R_now,xi);
            
            dev_L_interp = interp1(dev_L_now,xi);
            dev_R_interp = interp1(dev_R_now,xi);
            ddev_interp = interp1(ddev_now,xi);
