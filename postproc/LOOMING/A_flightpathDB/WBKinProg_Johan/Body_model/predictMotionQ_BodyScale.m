function [Pnew,tnew] = predictMotionQ_BodyScale(Pfull,PAR)
...
% load data if it is not already in memory
    %% FTMmod Ebraheem's take-off at 6k fps
    fps_movmodel = 6000;
if isempty(data)
...
% fps of training seq to current fps
scale = 1/(fps_movmodel*PAR.dt);

% melano @ 6k fps to hydei @ 7.5k fps
scale = 42/23;
...
% if isfield(PAR,'scale_min') == 1
%     PAR.scale_min(end+1) = scale(find(min(minval)));
% else
%     PAR.scale_min = scale(find(min(minval)));
% end
    
if exist('scale_min.mat') == 2
    load('scale_min.mat')
    scale_min(end+1) = scale(scaleidx);
    save('scale_min.mat','scale_min')
else
    scale_min = scale(scaleidx);
    save('scale_min.mat','scale_min')
end
