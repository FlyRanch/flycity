function [ FM_strkpln, FM_L, FM_R ,U_left, U_right, alfa_L, alfa_R, alfa_dot_L, alfa_dot_R ] = AerodynamicNmass_forces_atTimeNspanSections( kine, body_model, wing_model, rot_lift_on)
...
%             alfa_dot_R(j,i) = -(UR(3,j,i)/(UR(1,j,i)^2+UR(3,j,i)^2))*U_dot_R(1)+(UR(1,j,i)/(UR(1,j,i)^2+UR(3,j,i)^2))*U_dot_R(3); % [ rad/s ]
            ...
            % Compute location center of mass:
            x_cm_L(j,i) = (x_LE_L(j)-chords_L(j)/2)*cos(eta_L(j,i));
            y_cm_L(j,i) = (x_LE_L(j)-chords_L(j)/2)*sin(eta_L(j,i));

            x_cm_R(j,i) = (x_LE_R(j)-chords_R(j)/2)*cos(eta_R(j,i));
            y_cm_R(j,i) = (x_LE_R(j)-chords_R(j)/2)*sin(eta_R(j,i));
