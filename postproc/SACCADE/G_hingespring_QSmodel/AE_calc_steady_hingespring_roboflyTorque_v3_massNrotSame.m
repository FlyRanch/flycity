...
m_sect = massperArea * h_sect * c_sect;     % [kg]
c_mean = mean(c_sect);                      % [m]
c_max = max(c_sect);
x_mean = mean(x_sect);                      % [m]
...
t_wb_steady = [t_wb_L_steady t_wb_R_steady];
pitch_wb_steady = [pitch_wb_L_steady pitch_wb_R_steady];
[~, ~, rot_coeffs_steady] = create_fourier_series_4thN6thN8th_order(t_wb_steady,pitch_wb_steady,rot_order,0);
...
[stroke_steady] = (calc_val_fourier_series_4thN6thN8th_order(t_norm,stroke_coeffs_steady,0))';
[dev_steady] = (calc_val_fourier_series_4thN6thN8th_order(t_norm,dev_coeffs_steady,0))';
[rot_steady] = (calc_val_fourier_series_4thN6thN8th_order(t_norm,rot_coeffs_steady,0))';

[stroke_dot_steady] = (calc_val_dot_fourier_series_4thN6thN8th_order(t_norm,stroke_coeffs_steady,0))'*f_steady;
[dev_dot_steady] = (calc_val_dot_fourier_series_4thN6thN8th_order(t_norm,dev_coeffs_steady,0))'*f_steady;
[rot_dot_steady] = (calc_val_dot_fourier_series_4thN6thN8th_order(t_norm,rot_coeffs_steady,0))'*f_steady;

[stroke_dot_dot_steady] = (calc_val_dot_dot_fourier_series_4thN6thN8th_order(t_norm,stroke_coeffs_steady,0))'*f_steady^2;
[dev_dot_dot_steady] = (calc_val_dot_dot_fourier_series_4thN6thN8th_order(t_norm,dev_coeffs_steady,0))'*f_steady^2;
[rot_dot_dot_steady] = (calc_val_dot_dot_fourier_series_4thN6thN8th_order(t_norm,rot_coeffs_steady,0))'*f_steady^2;
...
% !!! use robofly aero torque !!!
Mwing_aero_norm = My_norm_robofly;
Mwing_aero_norm_LE = My_norm_robofly_LE;

% % !!! use qs aero torque !!!
% Mwing_aero_norm = My_norm_qs2;
% Mwing_aero_norm_LE = My_norm_qs2_LE;

%% mass torque
% Lehmann etal 2011: center of mass at rotation axis: zero mass torque
r1_norm = .5;
az(:,:) = kine.Udot_right(3,:,:)*1e-3;      % [ m/s^2 ]
az_r1 = az(:,end/2);
x_cm_r1_LE = -x_sect(end/2);
Mwing_mass_norm_LE = mass_wing * az_r1 * x_cm_r1_LE / Mg_fly / l_wing;
Mwing_mass_norm = zeros(size(Mwing_mass_norm_LE)) / Mg_fly / l_wing;

% % % wing section & virtual mass
...
% 
% % wing section & NO virtual mass
% mass_wing_sect = massperArea * h_sect * c_sect;
% mass_tot_sect = mass_wing_sect;
% 
% % wing section torque axis
% x_cm_LE = (-c_sect/2);
% x_cm = (x_sect-c_sect/2);
% 
% % Lehmann etal 2011: center of mass at rotation axis: zero mass torque
% x_cm_LE = -x_sect;
% x_cm = zeros(size(x_cm_LE));
% 
% clear az Mmass_sect Mmass_sect_LE Mmass_sect2 Mmass_sect2_LE
% az(:,:) = kine.Udot_right(3,:,:)*1e-3;      % [ m/s^2 ]
% for n = 1:size(az,1)
%     Mmass_sect2(:,n) = mass_tot_sect .* x_cm .*az(n,:)';
%     Mmass_sect2_LE(:,n) = mass_tot_sect .* x_cm_LE .*az(n,:)';
%     
%     Mmass_sect(:,n)    = -mass_tot_sect .* x_cm .*y_sect*deg2rad(stroke_dot_dot_steady(n));
%     Mmass_sect_LE(:,n) = -mass_tot_sect .* x_cm_LE .*y_sect*deg2rad(stroke_dot_dot_steady(n));
% end
% Mwing_mass_norm2  = nansum(Mmass_sect)' / Mg_fly / l_wing;
% Mwing_mass_norm2_LE  = nansum(Mmass_sect_LE)' / Mg_fly / l_wing;
% 
% Mwing_mass_norm3  = nansum(Mmass_sect2)' / Mg_fly / l_wing;
% Mwing_mass_norm3_LE  = nansum(Mmass_sect2_LE)' / Mg_fly / l_wing;

%% inertial torque

% Lehmann etal 2011: center of mass at rotation axis: zero mass torque
% Iwing: wing consists of leading section (2/10*chord & .48 mass) & trailing section (8/10*chord & .52 mass)
% (based on rot axis @ 0.36 chord length
Iwing_sect =    1/12 * .48*mass_wing * (2/10*c_mean).^2 + .48*mass_wing * (x_mean-1/10*c_mean).^2 +...
                1/12 * .52*mass_wing * (8/10*c_mean).^2 + .52*mass_wing * (x_mean-6/10*c_mean).^2;
Itot = nansum(Iwing_sect);

Iwing_sect_LE = 1/12 * .48*mass_wing * (2/10*c_mean).^2 + .48*mass_wing * (1/10*c_mean).^2 +...
                1/12 * .52*mass_wing * (8/10*c_mean).^2 + .52*mass_wing * (6/10*c_mean).^2;
...
Mwing_inertial_norm  = rot_dot_dot_rad_wingref' * Itot / Mg_fly / l_wing;
Mwing_inertial_norm_LE  = rot_dot_dot_rad_wingref' * Itot_LE / Mg_fly / l_wing;

%% sum of torques equals angular acceleration: I*rot_ddot + C*rot_dot + k*(rot-rot0) = Taero + Tmass
% hing torque = - wing torque
Mwing_springdamper_norm     = -Mwing_inertial_norm + Mwing_mass_norm + Mwing_aero_norm;
Mwing_inertialmass_norm      = -Mwing_inertial_norm + Mwing_mass_norm;

Mwing_inertialmass_norm_LE   = -Mwing_inertial_norm_LE + Mwing_mass_norm_LE;
Mwing_springdamper_norm_LE  = -Mwing_inertial_norm_LE + Mwing_mass_norm_LE + Mwing_aero_norm_LE;
...
Mwing_inertialmass_2Dref = -Mwing_inertialmass_norm;
Mwing_inertial_2Dref = -Mwing_inertial_norm;
...
Mwing_inertialmass_2Dref_LE = -Mwing_inertialmass_norm_LE;
Mwing_inertial_2Dref_LE = -Mwing_inertial_norm_LE;
...
set(gca,'ytick',[-.1 0 .3])
xlabel('wingbeat cycle')
ylabel('Taero arm norm')
grid on

saveas(gca,['AeroTorque_steps',num2str(nr_timepoints),'.fig'])
saveas(gca,['AeroTorque_steps',num2str(nr_timepoints),'.png'])
plot2svg(['AeroTorque_steps',num2str(nr_timepoints),'.svg'])
...
% plot(t_norm,Mwing_spring_2Dref_model,'--b','linewidth',2)
% plot(t_norm,Mwing_damper_2Dref_model,'--r','linewidth',2)

subplot(2,3,5)
plot(t_norm,Mwing_springdamper_2Dref_model_LE,'k','linewidth',2)
% plot(t_norm,Mwing_spring_2Dref_model_LE,'--b','linewidth',2)
% plot(t_norm,Mwing_damper_2Dref_model_LE,'--r','linewidth',2)
...
plot(t_norm,-Mwing_inertial_2Dref,'-b','linewidth',2)
plot(t_norm,Mwing_mass_2Dref,'-g','linewidth',2)
plot(t_norm,Mwing_aero_2Dref,'-r','linewidth',2)

grid on
axis tight
legend('total','inertial','mass','aero','location','northwest')
% axis square
axis([0,1,-.25,.25])
set(gca,'xtick',[0 1])
set(gca,'ytick',[-.25 0 .25])
...
plot(t_norm,-Mwing_inertial_2Dref_LE,'-b','linewidth',2)
plot(t_norm,Mwing_mass_2Dref_LE,'-g','linewidth',2)
plot(t_norm,Mwing_aero_2Dref_LE,'-r','linewidth',2)

grid on
axis tight
legend('total','inertial','mass','aero','location','northeast')
% axis square
axis([0,1,-.25,.25])
set(gca,'xtick',[0 1])
set(gca,'ytick',[-.25 0 .25])
...
hold on
plot(t_norm,Mwing_springdamper_2Dref_model,'-k','linewidth',2)
plot(t_norm,Mwing_spring_2Dref_model,'-c','linewidth',2)
plot(t_norm,Mwing_damper_2Dref_model,'-m','linewidth',2)
plot(t_norm,Mwing_springdamper_2Dref,'--k','linewidth',2)

grid on
axis tight
legend('spring-damper','spring','damper','location','northwest')
% axis square
axis([0,1,-.25,.25])
set(gca,'xtick',[0 1])
set(gca,'ytick',[-.25 0 .25])
xlabel('wingbeat cycle')
ylabel('normalized hinge torque model')

subplot(2,2,4)
hold on
plot(t_norm,Mwing_springdamper_2Dref_model_LE,'-k','linewidth',2)
plot(t_norm,Mwing_spring_2Dref_model_LE,'-c','linewidth',2)
plot(t_norm,Mwing_damper_2Dref_model_LE,'-m','linewidth',2)
plot(t_norm,Mwing_springdamper_2Dref_LE,'--k','linewidth',2)

grid on
axis tight
legend('spring-damper','spring','damper','location','northeast')
% axis square
axis([0,1,-.25,.25])
set(gca,'xtick',[0 1])
set(gca,'ytick',[-.25 0 .25])
xlabel('wingbeat cycle')
ylabel('normalized LE torque model')

saveas(gca,['Twing_vs_WBcycle_hingeAndLErotation_flyNmodel_steps',num2str(nr_timepoints),'.fig'])
saveas(gca,['Twing_vs_WBcycle_hingeAndLErotation_flyNmodel_steps',num2str(nr_timepoints),'.png'])
plot2svg(['Twing_vs_WBcycle_hingeAndLErotation_flyNmodel_steps',num2str(nr_timepoints),'.svg'])
