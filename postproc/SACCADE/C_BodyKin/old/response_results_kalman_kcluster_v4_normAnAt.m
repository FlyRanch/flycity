...
load('flightpathDB_pos_qbodyEKF_NOroll_9clusters_2.5n-3n2.5_response.mat')
mkdir('response_figs')
cd('response_figs')
...
t_post = max([responseDB.t_turn_stop responseDB.t_accel_stop responseDB.t_decel_stop]')';

for i = 1:length(t_pre)
    if isnan(t_pre) == 0
    n_pre(i,1) = find(t==t_pre(i));
    n_post(i,1) = find(t==t_post(i));
    
    n_turn_start = find(t==responseDB.t_turn_start(i));
    n_turn_stop = find(t==responseDB.t_turn_stop(i));
    
    n_accel_start = find(t==responseDB.t_accel_start(i));
    n_accel_stop = find(t==responseDB.t_accel_stop(i));
    
    n_decel_start = find(t==responseDB.t_decel_start(i));
    n_decel_stop = find(t==responseDB.t_decel_stop(i));
    end
end
...
% %% calc normalized An & At
% t_pre = responseDB.t_resp;
% t_post = max([responseDB.t_turn_stop responseDB.t_accel_stop]')';
% 
% calc_normAnAt
...
figure
plot(heading_start,responseDB.At_hor_min,'*')
