...
rot_angle_Amax = atand(Dpitch_Amax./roll_mirror_Amax);
...
rot_angle_mean = atand(Dpitch_mean./roll_mirror_mean);

% stats Amax
stats_roll_atAmax = regstats(roll_mirror_Amax,turn_angle_vel_mirror,'linear');
...
% stats means
stats_roll_mean = regstats(roll_mirror_mean,turn_angle_vel_mirror,'linear');
...
stats_roll_mean.beta
stats_roll_mean.tstat.pval
[h,p]=ttest(roll_mirror_mean)
...
plot(xp,yp)
plot(turn_angle_vel_mirror,roll_mirror_Amax,'.')

yp = polyval([stats_roll_mean.beta(2) stats_roll_mean.beta(1)],xp);
plot(xp,yp,'r')
plot(turn_angle_vel_mirror,roll_mirror_mean,'.r')
...
save('attitude_atAmaxNmean_4stats.mat','turn_angle_vel_mirror',...
    'rot_angle_Amax','roll_mirror_Amax','yaw_mirror_Amax','Dpitch_Amax','pitch_Amax',...
    'rot_angle_mean','roll_mirror_mean','yaw_mirror_mean','Dpitch_mean','pitch_mean',...
    'stats_rot_angle_atAmax','stats_roll_atAmax','stats_Dpitch_atAmax','stats_yaw_atAmax',...
    'stats_rot_angle_mean','stats_roll_mean','stats_Dpitch_mean','stats_yaw_mean');
