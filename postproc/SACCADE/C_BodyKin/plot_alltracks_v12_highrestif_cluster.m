...
savedir = ['3Dflightpaths_topview_tetamax',num2str(tetamax),'_',num2str(fps),'fps_clusters'];
mkdir(savedir)
cd(savedir)
 ...
 axis tight
    ...
    for k = start+movie_step:movie_step:stop
        ...
        for m = 1:size(x,2)
            ...
            if isnan(x(k-movie_step,m)) == 0
                
                if isnan(IDX(k,m)) == 1
                    IDX(k,m) = 5;
                end

                plot([x(k-movie_step,m) x(k,m)],[y(k-movie_step,m) y(k,m)],'-','linewidth',linewidth_path,'color',cmap_k(IDX(k,m),:))
            end
        end
