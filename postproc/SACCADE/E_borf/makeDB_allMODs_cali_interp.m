...
%         ft(:,6) = filter_borf_raw(ft(:,6),butter_cut,butter_n,Nwb_total);
        ...
        t_all(1:seq_length_max,n) = nan;
        
        stroke_L_all(1:seq_length_max,n) = nan;
        pitch_L_all(1:seq_length_max,n) = nan;
        dev_L_all(1:seq_length_max,n) = nan;
        
        stroke_R_all(1:seq_length_max,n) = nan;
        pitch_R_all(1:seq_length_max,n) = nan;
        dev_R_all(1:seq_length_max,n) = nan;
        
        Fx_all(1:seq_length_max,1:Nwb,n) = nan;
        Fy_all(1:seq_length_max,1:Nwb,n) = nan;
        Fz_all(1:seq_length_max,1:Nwb,n) = nan;
        Mx_all(1:seq_length_max,1:Nwb,n) = nan;
        My_all(1:seq_length_max,1:Nwb,n) = nan;
        Mz_all(1:seq_length_max,1:Nwb,n) = nan;
...
% %             Mz_now(j,:) = Mz_robo(j,:) - Mz_cali_L(j) - Mz_cali_R(j);
        ...
        %% calc wb mean data
        Fx_mean_all(n,1) = nanmean(Fx_fly(:));
        Fy_mean_all(n,1) = nanmean(Fy_fly(:));
        Fz_mean_all(n,1) = nanmean(Fz_fly(:));

        Mx_mean_all(n,1) = nanmean(Mx_fly(:));
        My_mean_all(n,1) = nanmean(My_fly(:));
        Mz_mean_all(n,1) = nanmean(Mz_fly(:));

        %% store all data
        t_all(1:length(t_now),n) = t_now;

        stroke_L_all(1:length(t_now),n) = stroke_L_now;
        pitch_L_all(1:length(t_now),n) = pitch_L_now;
        dev_L_all(1:length(t_now),n) = dev_L_now;

        stroke_R_all(1:length(t_now),n) = stroke_R_now;
        pitch_R_all(1:length(t_now),n) = pitch_R_now;
        dev_R_all(1:length(t_now),n) = dev_R_now;

        Fx_all(1:length(t_now),:,n) = Fx_fly;
        Fy_all(1:length(t_now),:,n) = Fy_fly;
        Fz_all(1:length(t_now),:,n) = Fz_fly;

        Mx_all(1:length(t_now),:,n) = Mx_fly;
        My_all(1:length(t_now),:,n) = My_fly;
        Mz_all(1:length(t_now),:,n) = Mz_fly;
    end
end
