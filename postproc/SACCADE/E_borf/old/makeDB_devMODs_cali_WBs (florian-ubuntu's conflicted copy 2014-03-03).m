...
for i = 1:length(list)
    ...
    if exist(file_now) == 2
        ...
        t_dev(1:seq_length_max,end+1) = nan;
        
        stroke_L_dev(1:seq_length_max,end+1) = nan;
        pitch_L_dev(1:seq_length_max,end+1) = nan;
        dev_L_dev(1:seq_length_max,end+1) = nan;
        
        stroke_R_dev(1:seq_length_max,end+1) = nan;
        pitch_R_dev(1:seq_length_max,end+1) = nan;
        dev_R_dev(1:seq_length_max,end+1) = nan;
        
        Fx_dev(1:seq_length_max,1:Nwb,end+1) = nan;
        Fy_dev(1:seq_length_max,1:Nwb,end+1) = nan;
        Fz_dev(1:seq_length_max,1:Nwb,end+1) = nan;
        Mx_dev(1:seq_length_max,1:Nwb,end+1) = nan;
        My_dev(1:seq_length_max,1:Nwb,end+1) = nan;
        Mz_dev(1:seq_length_max,1:Nwb,end+1) = nan;
...
%             Mz_now(j,:) = Mz_robo(j,:) - Mz_cali_L(j) - Mz_cali_R(j);
        ...
        %% calc wb mean data
        Fx_mean_dev(n,1) = nanmean(Fx_fly(:));
        Fy_mean_dev(n,1) = nanmean(Fy_fly(:));
        Fz_mean_dev(n,1) = nanmean(Fz_fly(:));

        Mx_mean_dev(n,1) = nanmean(Mx_fly(:));
        My_mean_dev(n,1) = nanmean(My_fly(:));
        Mz_mean_dev(n,1) = nanmean(Mz_fly(:));

        %% store dev data
        t_dev(1:length(t_now),n) = t_now;

        stroke_L_dev(1:length(t_now),n) = stroke_L_now;
        pitch_L_dev(1:length(t_now),n) = pitch_L_now;
        dev_L_dev(1:length(t_now),n) = dev_L_now;

        stroke_R_dev(1:length(t_now),n) = stroke_R_now;
        pitch_R_dev(1:length(t_now),n) = pitch_R_now;
        dev_R_dev(1:length(t_now),n) = dev_R_now;

        Fx_dev(1:length(t_now),:,n) = Fx_now;
        Fy_dev(1:length(t_now),:,n) = Fy_now;
        Fz_dev(1:length(t_now),:,n) = Fz_now;

        Mx_dev(1:length(t_now),:,n) = Mx_now;
        My_dev(1:length(t_now),:,n) = My_now;
        Mz_dev(1:length(t_now),:,n) = Mz_now;
    end
end
