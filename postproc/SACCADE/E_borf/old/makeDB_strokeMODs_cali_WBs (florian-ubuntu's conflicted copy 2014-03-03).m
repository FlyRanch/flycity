...
for i = 1:length(list)
    ...
    if exist(file_now) == 2
        ...
        Fx_stroke(1:seq_length_max,1:Nwb,end+1) = nan;
        Fy_stroke(1:seq_length_max,1:Nwb,end+1) = nan;
        Fz_stroke(1:seq_length_max,1:Nwb,end+1) = nan;
        Mx_stroke(1:seq_length_max,1:Nwb,end+1) = nan;
        My_stroke(1:seq_length_max,1:Nwb,end+1) = nan;
        Mz_stroke(1:seq_length_max,1:Nwb,end+1) = nan;
...
%             Mz_now(j,:) = Mz_robo(j,:) - Mz_cali_L(j) - Mz_cali_R(j);
        ...
        %% calc wb mean data
        Fx_mean_stroke(n,1) = nanmean(Fx_fly(:));
        Fy_mean_stroke(n,1) = nanmean(Fy_fly(:));
        Fz_mean_stroke(n,1) = nanmean(Fz_fly(:));

        Mx_mean_stroke(n,1) = nanmean(Mx_fly(:));
        My_mean_stroke(n,1) = nanmean(My_fly(:));
        Mz_mean_stroke(n,1) = nanmean(Mz_fly(:));

        %% store stroke data
        t_stroke(1:length(t_now),n) = t_now;

        stroke_L_stroke(1:length(t_now),n) = stroke_L_now;
        pitch_L_stroke(1:length(t_now),n) = pitch_L_now;
        dev_L_stroke(1:length(t_now),n) = dev_L_now;

        stroke_R_stroke(1:length(t_now),n) = stroke_R_now;
        pitch_R_stroke(1:length(t_now),n) = pitch_R_now;
        dev_R_stroke(1:length(t_now),n) = dev_R_now;

        Fx_stroke(1:length(t_now),:,n) = Fx_now;
        Fy_stroke(1:length(t_now),:,n) = Fy_now;
        Fz_stroke(1:length(t_now),:,n) = Fz_now;

        Mx_stroke(1:length(t_now),:,n) = Mx_now;
        My_stroke(1:length(t_now),:,n) = My_now;
        Mz_stroke(1:length(t_now),:,n) = Mz_now;
    end
end
