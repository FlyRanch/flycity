...
load('borf_db_F_yaw_LR_cali_alldata.mat')
load('MOD_norm_data.mat')
...
mod_now = max(YawMods);
    ...
    % F ALL NO FREQ
    Fx_now = Fx_allNOfreq_mirror0(:,:,n)/Mg_fly;
...
%     Fx_now = filter_borf(Fx_now,butter_cut,butter_n); 
    ...
    Fz_now = Fz_allNOfreq_mirror0(:,:,n)/Mg_fly;
...
mod_now = max(YawMods);
    ...
    % F ALL NO FREQ
    Fx_now = Fx_allNOfreq_mirror1(:,:,n)/Mg_fly;
...
%     Fx_now = filter_borf(Fx_now,butter_cut,butter_n); 
    ...
    Fz_now = Fz_allNOfreq_mirror1(:,:,n)/Mg_fly;
...
mkdir(['MSfigs_butter_string',num2str(butter_cut)])
cd(['MSfigs_butter_string',num2str(butter_cut)])
saveas(gca, 'MSfig_YawTorque_wingprofNforce_angles_butter.fig')
saveas(gca, 'MSfig_YawTorque_wingprofNforce_angles_butter.png')
plot2svg(['MSfig_YawTorque_wingprofNforce_angles_butter.svg'])
cd ..
