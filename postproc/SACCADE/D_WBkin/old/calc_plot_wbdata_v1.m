...
roll_dot_max_wb = nan(size(n_down_start_L));
roll_dot_max_ds = nan(size(n_down_start_L));
roll_dot_max_us = nan(size(n_down_start_L));
roll_dot_max_dus = nan(size(n_down_start_L));
roll_dot_max_udsPREV = nan(size(n_down_start_L));
roll_dot_max_udsNEXT = nan(size(n_down_start_L));

roll_dot_min_wb = nan(size(n_down_start_L));
roll_dot_min_ds = nan(size(n_down_start_L));
roll_dot_min_us = nan(size(n_down_start_L));
roll_dot_min_dus = nan(size(n_down_start_L));
roll_dot_min_udsPREV = nan(size(n_down_start_L));
roll_dot_min_udsNEXT = nan(size(n_down_start_L));
...
pitch_dot_max_wb = nan(size(n_down_start_L));
pitch_dot_max_ds = nan(size(n_down_start_L));
pitch_dot_max_us = nan(size(n_down_start_L));
pitch_dot_max_dus = nan(size(n_down_start_L));
pitch_dot_max_udsPREV = nan(size(n_down_start_L));
pitch_dot_max_udsNEXT = nan(size(n_down_start_L));

pitch_dot_min_wb = nan(size(n_down_start_L));
pitch_dot_min_ds = nan(size(n_down_start_L));
pitch_dot_min_us = nan(size(n_down_start_L));
pitch_dot_min_dus = nan(size(n_down_start_L));
pitch_dot_min_udsPREV = nan(size(n_down_start_L));
pitch_dot_min_udsNEXT = nan(size(n_down_start_L));
...
yaw_dot_max_wb = nan(size(n_down_start_L));
yaw_dot_max_ds = nan(size(n_down_start_L));
yaw_dot_max_us = nan(size(n_down_start_L));
yaw_dot_max_dus = nan(size(n_down_start_L));
yaw_dot_max_udsPREV = nan(size(n_down_start_L));
yaw_dot_max_udsNEXT = nan(size(n_down_start_L));

yaw_dot_min_wb = nan(size(n_down_start_L));
yaw_dot_min_ds = nan(size(n_down_start_L));
yaw_dot_min_us = nan(size(n_down_start_L));
yaw_dot_min_dus = nan(size(n_down_start_L));
yaw_dot_min_udsPREV = nan(size(n_down_start_L));
yaw_dot_min_udsNEXT = nan(size(n_down_start_L));
...
%             stroke_wb_R_interp = interp1(stroke_wb_R_now,xi);
            ...
            % body kin data
            roll_dot_mean_wb(wb,seq) = mean(roll_dot_wb_now);
            roll_dot_mean_ds(wb,seq) = mean(roll_dot_ds_now);
            roll_dot_mean_us(wb,seq) = mean(roll_dot_us_now);
%             roll_dot_mean_dus(wb,seq) = mean(roll_dot_dus_now);
%             roll_dot_mean_udsPREV(wb,seq) = mean(roll_dot_uds_prev);
%             roll_dot_mean_udsNEXT(wb,seq) = mean(roll_dot_uds_next);
           
            roll_dot_max_wb(wb,seq) = max(roll_dot_wb_now);
            roll_dot_max_ds(wb,seq) = max(roll_dot_ds_now);
            roll_dot_max_us(wb,seq) = max(roll_dot_us_now);
%             roll_dot_max_dus(wb,seq) = max(roll_dot_dus_now);
%             roll_dot_max_udsPREV(wb,seq) = max(roll_dot_uds_prev);
%             roll_dot_max_udsNEXT(wb,seq) = max(roll_dot_uds_next);
           
            roll_dot_min_wb(wb,seq) = min(roll_dot_wb_now);
            roll_dot_min_ds(wb,seq) = min(roll_dot_ds_now);
            roll_dot_min_us(wb,seq) = min(roll_dot_us_now);
%             roll_dot_min_dus(wb,seq) = min(roll_dot_dus_now);
%             roll_dot_min_udsPREV(wb,seq) = min(roll_dot_uds_prev);
%             roll_dot_min_udsNEXT(wb,seq) = min(roll_dot_uds_next);
            
            pitch_dot_mean_wb(wb,seq) = mean(pitch_dot_wb_now);
            pitch_dot_mean_ds(wb,seq) = mean(pitch_dot_ds_now);
            pitch_dot_mean_us(wb,seq) = mean(pitch_dot_us_now);
%             pitch_dot_mean_dus(wb,seq) = mean(pitch_dot_dus_now);
%             pitch_dot_mean_udsPREV(wb,seq) = mean(pitch_dot_uds_prev);
%             pitch_dot_mean_udsNEXT(wb,seq) = mean(pitch_dot_uds_next);
           
            pitch_dot_max_wb(wb,seq) = max(pitch_dot_wb_now);
            pitch_dot_max_ds(wb,seq) = max(pitch_dot_ds_now);
            pitch_dot_max_us(wb,seq) = max(pitch_dot_us_now);
%             pitch_dot_max_dus(wb,seq) = max(pitch_dot_dus_now);
%             pitch_dot_max_udsPREV(wb,seq) = max(pitch_dot_uds_prev);
%             pitch_dot_max_udsNEXT(wb,seq) = max(pitch_dot_uds_next);
           
            pitch_dot_min_wb(wb,seq) = min(pitch_dot_wb_now);
            pitch_dot_min_ds(wb,seq) = min(pitch_dot_ds_now);
            pitch_dot_min_us(wb,seq) = min(pitch_dot_us_now);
%             pitch_dot_min_dus(wb,seq) = min(pitch_dot_dus_now);
%             pitch_dot_min_udsPREV(wb,seq) = min(pitch_dot_uds_prev);
%             pitch_dot_min_udsNEXT(wb,seq) = min(pitch_dot_uds_next);
            
            yaw_dot_mean_wb(wb,seq) = mean(yaw_dot_wb_now);
            yaw_dot_mean_ds(wb,seq) = mean(yaw_dot_ds_now);
            yaw_dot_mean_us(wb,seq) = mean(yaw_dot_us_now);
%             yaw_dot_mean_dus(wb,seq) = mean(yaw_dot_dus_now);
%             yaw_dot_mean_udsPREV(wb,seq) = mean(yaw_dot_uds_prev);
%             yaw_dot_mean_udsNEXT(wb,seq) = mean(yaw_dot_uds_next);
           
            yaw_dot_max_wb(wb,seq) = max(yaw_dot_wb_now);
            yaw_dot_max_ds(wb,seq) = max(yaw_dot_ds_now);
            yaw_dot_max_us(wb,seq) = max(yaw_dot_us_now);
%             yaw_dot_max_dus(wb,seq) = max(yaw_dot_dus_now);
%             yaw_dot_max_udsPREV(wb,seq) = max(yaw_dot_uds_prev);
%             yaw_dot_max_udsNEXT(wb,seq) = max(yaw_dot_uds_next);
           
            yaw_dot_min_wb(wb,seq) = min(yaw_dot_wb_now);
            yaw_dot_min_ds(wb,seq) = min(yaw_dot_ds_now);
            yaw_dot_min_us(wb,seq) = min(yaw_dot_us_now);
%             yaw_dot_min_dus(wb,seq) = min(yaw_dot_dus_now);
%             yaw_dot_min_udsPREV(wb,seq) = min(yaw_dot_uds_prev);
%             yaw_dot_min_udsNEXT(wb,seq) = min(yaw_dot_uds_next);
...
figure
subplot(2,2,1)
plot(ddev_max_dus(:),roll_dot_mean_wb(:),'o')
hold on
subplot(2,2,2)
plot(ddev_max_udsPREV(:),roll_dot_mean_wb(:),'or')
subplot(2,2,3)
plot(ddev_min_ds(:),roll_dot_mean_wb(:),'og')
subplot(2,2,4)
plot(ddev_min_us(:),roll_dot_mean_wb(:),'ok')

figure
subplot(2,2,1)
plot(dev_max_dus_L(:),pitch_dot_mean_wb(:),'o')
hold on
plot(dev_max_dus_R(:),pitch_dot_mean_wb(:),'or')

subplot(2,2,2)
plot(dev_max_udsPREV_L(:),pitch_dot_mean_wb(:),'o')
hold on
plot(dev_max_udsPREV_R(:),pitch_dot_mean_wb(:),'or')

subplot(2,2,3)
plot(dev_min_ds_L(:),pitch_dot_mean_wb(:),'o')
hold on
plot(dev_min_ds_R(:),pitch_dot_mean_wb(:),'or')

subplot(2,2,4)
plot(dev_min_us_L(:),pitch_dot_mean_wb(:),'o')
hold on
plot(dev_min_us_R(:),pitch_dot_mean_wb(:),'or')


figure
subplot(2,2,1)
plot(Adev_ds_L(:),pitch_dot_mean_wb(:),'o')
hold on
plot(Adev_ds_R(:),pitch_dot_mean_wb(:),'or')

subplot(2,2,2)
plot(Adev_us_L(:),pitch_dot_mean_wb(:),'o')
hold on
plot(Adev_us_R(:),pitch_dot_mean_wb(:),'or')

subplot(2,2,3)
plot(dAdev_ds(:),roll_dot_mean_wb(:),'o')
hold on

subplot(2,2,4)
plot(dAdev_us(:),roll_dot_mean_wb(:),'o')
hold on
            ...
            subplot(2,2,1)
            plot(stroke_L_interp,'color',plot_color)
            hold on
            plot(stroke_R_interp,'color',plot_color)
            subplot(2,2,2)
            plot(pitch_L_interp,'color',plot_color)
            hold on
            plot(pitch_R_interp,'color',plot_color)
            subplot(2,2,3)
            plot(dev_L_interp,'color',plot_color)
            hold on
            plot(dev_R_interp,'color',plot_color)
            subplot(2,2,4)
            plot(ddev_interp,'color',plot_color)
            hold on
% pause
