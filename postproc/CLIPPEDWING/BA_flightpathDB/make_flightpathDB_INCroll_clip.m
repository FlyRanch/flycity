function [pathDB,settings] = make_flightpathDB_INCroll_clip(pathDB,settings,clip,incq)
...
for i=start_dir:length(alldirs)
    ...
    if alldirs(i).isdir==1
        ...
        if exist(dirname)==7
            ...
            if length(alldirs(i).name) > 14
                settings.seq(seq,3) = str2num(alldirs(i).name(16));
            else
                settings.seq(seq,3) = 1;
            end
            
            settings.clip(seq,1) = clip;
