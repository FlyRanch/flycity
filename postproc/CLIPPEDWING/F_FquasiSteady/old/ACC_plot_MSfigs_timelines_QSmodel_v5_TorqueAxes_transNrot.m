...
for i = 1:length(mod_values)
    ...
    % M_R ALL
    val_now = M_R_transNrot_norm_Fenhance_all(:,n);
    t_now = [0:1/(length(val_now)-1):1];
    ...
    % M_L ALL
    val_now = M_L_transNrot_norm_Fenhance_all(:,n);
    t_now = [0:1/(length(val_now)-1):1];
...
n = find(mod_diff==min(mod_diff));
val_steady = Mx_transNrot_norm_RollTorque_allNOfreq(:,n);
...
for i = 1:length(mod_values)
    ...
    % Fx ALL
    val_now = Fx_transNrot_norm_RollTorque_allNOfreq(:,n);
    t_now = [0:1/(length(val_now)-1):1];
    ...
    % Fy ALL
    val_now = Fy_transNrot_norm_RollTorque_allNOfreq(:,n);
    t_now = [0:1/(length(val_now)-1):1];
    ...
    % Fz ALL
    val_now = Fz_transNrot_norm_RollTorque_allNOfreq(:,n);
    t_now = [0:1/(length(val_now)-1):1];
    ...
    % Mx ALL
    val_now = Mx_transNrot_norm_RollTorque_allNOfreq(:,n);
    t_now = [0:1/(length(val_now)-1):1];
    ...
    % My ALL
    val_now = My_transNrot_norm_RollTorque_allNOfreq(:,n);
    t_now = [0:1/(length(val_now)-1):1];
    ...
    % Mz ALL
    val_now = Mz_transNrot_norm_RollTorque_allNOfreq(:,n);
    t_now = [0:1/(length(val_now)-1):1];
    ...
    % M_R ALL
    val_now = M_R_transNrot_norm_RollTorque_allNOfreq(:,n);
    t_now = [0:1/(length(val_now)-1):1];
    ...
    % M_L ALL
    val_now = M_L_transNrot_norm_RollTorque_allNOfreq(:,n);
    t_now = [0:1/(length(val_now)-1):1];
...
n = find(mod_diff==min(mod_diff));
val_steady = Mz_transNrot_norm_YawTorque_allNOfreq(:,n);
...
for i = 1:length(mod_values)
    ...
    % Fx ALL
    val_now = Fx_transNrot_norm_YawTorque_allNOfreq(:,n);
    t_now = [0:1/(length(val_now)-1):1];
    ...
    % Fy ALL
    val_now = Fy_transNrot_norm_YawTorque_allNOfreq(:,n);
    t_now = [0:1/(length(val_now)-1):1];
    ...
    % Fz ALL
    val_now = Fz_transNrot_norm_YawTorque_allNOfreq(:,n);
    t_now = [0:1/(length(val_now)-1):1];
    ...
    % Mx ALL
    val_now = Mx_transNrot_norm_YawTorque_allNOfreq(:,n);
    t_now = [0:1/(length(val_now)-1):1];
    ...
    % My ALL
    val_now = My_transNrot_norm_YawTorque_allNOfreq(:,n);
    t_now = [0:1/(length(val_now)-1):1];
    ...
    % Mz ALL
    val_now = Mz_transNrot_norm_YawTorque_allNOfreq(:,n);
    t_now = [0:1/(length(val_now)-1):1];
    ...
    % M_R ALL
    val_now = M_R_transNrot_norm_YawTorque_allNOfreq(:,n);
    t_now = [0:1/(length(val_now)-1):1];
    ...
    % M_L ALL
    val_now = M_L_transNrot_norm_YawTorque_allNOfreq(:,n);
    t_now = [0:1/(length(val_now)-1):1];
...
n = find(mod_diff==min(mod_diff));
val_steady = My_transNrot_norm_PitchTorque_allNOfreq(:,n);
...
for i = 1:length(mod_values)
    ...
    % Fx ALL
    val_now = Fx_transNrot_norm_PitchTorque_allNOfreq(:,n);
    t_now = [0:1/(length(val_now)-1):1];
    ...
    % Fy ALL
    val_now = Fy_transNrot_norm_PitchTorque_allNOfreq(:,n);
    t_now = [0:1/(length(val_now)-1):1];
    ...
    % Fz ALL
    val_now = Fz_transNrot_norm_PitchTorque_allNOfreq(:,n);
    t_now = [0:1/(length(val_now)-1):1];
    ...
    % Mx ALL
    val_now = Mx_transNrot_norm_PitchTorque_allNOfreq(:,n);
    t_now = [0:1/(length(val_now)-1):1];
    ...
    % My ALL
    val_now = My_transNrot_norm_PitchTorque_allNOfreq(:,n);
    t_now = [0:1/(length(val_now)-1):1];
    ...
    % Mz ALL
    val_now = Mz_transNrot_norm_PitchTorque_allNOfreq(:,n);
    t_now = [0:1/(length(val_now)-1):1];
    ...
    % M_R ALL
    val_now = M_R_transNrot_norm_PitchTorque_allNOfreq(:,n);
    t_now = [0:1/(length(val_now)-1):1];
    ...
    % M_L ALL
    val_now = M_L_transNrot_norm_PitchTorque_allNOfreq(:,n);
    t_now = [0:1/(length(val_now)-1):1];
...
n = find(mod_diff==min(mod_diff));
val_steady = M_R_transNrot_norm_TorqueAxisR_allNOfreq(:,n);
...
for i = 1:length(mod_values)
    ...
    % Fx ALL
    val_now = Fx_transNrot_norm_TorqueAxisR_allNOfreq(:,n);
    t_now = [0:1/(length(val_now)-1):1];
    ...
    % Fy ALL
    val_now = Fy_transNrot_norm_TorqueAxisR_allNOfreq(:,n);
    t_now = [0:1/(length(val_now)-1):1];
    ...
    % Fz ALL
    val_now = Fz_transNrot_norm_TorqueAxisR_allNOfreq(:,n);
    t_now = [0:1/(length(val_now)-1):1];
    ...
    % Mx ALL
    val_now = Mx_transNrot_norm_TorqueAxisR_allNOfreq(:,n);
    t_now = [0:1/(length(val_now)-1):1];
    ...
    % My ALL
    val_now = My_transNrot_norm_TorqueAxisR_allNOfreq(:,n);
    t_now = [0:1/(length(val_now)-1):1];
    ...
    % Mz ALL
    val_now = Mz_transNrot_norm_TorqueAxisR_allNOfreq(:,n);
    t_now = [0:1/(length(val_now)-1):1];
    ...
    % M_R ALL
    val_now = M_R_transNrot_norm_TorqueAxisR_allNOfreq(:,n);
    t_now = [0:1/(length(val_now)-1):1];
    ...
    % M_L ALL
    val_now = M_L_transNrot_norm_TorqueAxisR_allNOfreq(:,n);
    t_now = [0:1/(length(val_now)-1):1];
...
mkdir('MSfigs_QSmodel')
cd('MSfigs_QSmodel')
saveas(gca, 'MSfig_M_R_timelines_QSmodel_transNrot.fig')
saveas(gca, 'MSfig_M_R_timelines_QSmodel_transNrot.png')
plot2svg(['MSfig_M_R_timelines_QSmodel_transNrot.svg'])
cd ..
