...
l_wing = wing_model.length*1e-3;
massperArea = wing_model.mass/(wing_model.area*1e-6);
...
Iwing_intact = sum(Iwing_sect);
Iaddedmass_intact = sum(Iaddedmass_sect);
Itot_intact = sum(Itot_sect);
...
for i = 1:(sect_max-sect_min+1)
    ...
    S2_damaged = sum(S2_sect_sub);
    S3_damaged = sum(S3_sect_sub);
    ...
    Iwing_damaged = sum(Iwing_sect_sub);
    Iaddedmass_damaged = sum(Iaddedmass_sect_sub);
    Itot_damaged = sum(Itot_sect_sub);
