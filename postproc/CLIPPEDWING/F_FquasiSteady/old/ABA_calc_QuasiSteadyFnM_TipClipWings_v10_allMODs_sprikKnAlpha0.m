...
h_sect = wing_model.length/nr_sect*1e-3;
x_sect = wing_model.x_LE_R'*1e-3;
y_sect = wing_model.y_sect_R(:,2)*1e-3;
c_sect = wing_model.chords_R'*1e-3;
...
for i = 1:(sect_max-sect_min+1)
    ...
    S2_sect_sub = S2_sect(1:N_cut);
    S3_sect_sub = S3_sect(1:N_cut);

    S2_damaged = nansum(S2_sect_sub);
    S3_damaged = nansum(S3_sect_sub);
    
    S2ratio = S2_damaged/S2_intact;
    S3ratio = S3_damaged/S3_intact;
    
    % wing section mass
    mass_wing_sect_damaged = mass_wing_sect_intact;
    mass_virtual_sect_damaged = mass_virtual_sect_intact;
    mass_tot_sect_damaged = mass_tot_sect_intact;

    mass_wing_sect_damaged(N_cut+1:end) = 0;
    mass_virtual_sect_damaged(N_cut+1:end) = 0;
    mass_tot_sect_damaged(N_cut+1:end) = 0;

    % Iwing & Iaddedmass
    Iwing_sect_sub = Iwing_sect(1:N_cut);
    Iaddedmass_sect_sub = Iaddedmass_sect(1:N_cut);
    Itot_sect_sub = Itot_sect(1:N_cut);
    
    Iwing_damaged = nansum(Iwing_sect_sub);
    Iaddedmass_damaged = nansum(Iaddedmass_sect_sub);
    Itot_damaged = nansum(Itot_sect_sub);
...
%     freq = freq_now;
    ...
    % calc total spanwise wing torque
...
%     freq = f_clipped_fly;
    ...
    % remove spanwise torques at removed part of damaged wing (n>N_cut)
    Maero_sect_R(N_cut+1:end,:) = 0;
...
%             end
    ...
    if isnan(n_rot_max_L) == 0
        Mwing_total_atRot_max_intactwing_all(i,1)   = Mwing_total_intactwing_all(n_rot_max_L,i);
        Mwing_mass_atRot_max_intactwing_all(i,1)    = Mwing_mass_intactwing_all(n_rot_max_L,i);
        Mwing_inertia_atRot_max_intactwing_all(i,1) = Mwing_inertia_intactwing_all(n_rot_max_L,i);
        Mwing_aero_atRot_max_intactwing_all(i,1)    = Mwing_aero_intactwing_all(n_rot_max_L,i);
    else
        Mwing_total_atRot_max_intactwing_all(i,1)   = nan;
        Mwing_mass_atRot_max_intactwing_all(i,1)    = nan;
        Mwing_inertia_atRot_max_intactwing_all(i,1) = nan;
        Mwing_aero_atRot_max_intactwing_all(i,1)    = nan;
    end
    
    if isnan(n_rot_min_L) == 0
        Mwing_total_atRot_min_intactwing_all(i,1)   = Mwing_total_intactwing_all(n_rot_min_L,i);
        Mwing_mass_atRot_min_intactwing_all(i,1)    = Mwing_mass_intactwing_all(n_rot_min_L,i);
        Mwing_inertia_atRot_min_intactwing_all(i,1) = Mwing_inertia_intactwing_all(n_rot_min_L,i);
        Mwing_aero_atRot_min_intactwing_all(i,1)    = Mwing_aero_intactwing_all(n_rot_min_L,i);
    else
        Mwing_total_atRot_min_intactwing_all(i,1)   = nan;
        Mwing_mass_atRot_min_intactwing_all(i,1)    = nan;
        Mwing_inertia_atRot_min_intactwing_all(i,1) = nan;
        Mwing_aero_atRot_min_intactwing_all(i,1)    = nan;
    end

    if isnan(n_rot_max_R) == 0
        Mwing_total_atRot_max_damagedwing_all(i,1)   = Mwing_total_damagedwing_all(n_rot_max_R,i);
        Mwing_mass_atRot_max_damagedwing_all(i,1)    = Mwing_mass_damagedwing_all(n_rot_max_R,i);
        Mwing_inertia_atRot_max_damagedwing_all(i,1) = Mwing_inertia_damagedwing_all(n_rot_max_R,i);
        Mwing_aero_atRot_max_damagedwing_all(i,1)    = Mwing_aero_damagedwing_all(n_rot_max_R,i);
    else
        Mwing_total_atRot_max_damagedwing_all(i,1)   = nan;
        Mwing_mass_atRot_max_damagedwing_all(i,1)    = nan;
        Mwing_inertia_atRot_max_damagedwing_all(i,1) = nan;
        Mwing_aero_atRot_max_damagedwing_all(i,1)    = nan;
    end
    
    if isnan(n_rot_min_R) == 0
        Mwing_total_atRot_min_damagedwing_all(i,1)   = Mwing_total_damagedwing_all(n_rot_min_R,i);
        Mwing_mass_atRot_min_damagedwing_all(i,1)    = Mwing_mass_damagedwing_all(n_rot_min_R,i);
        Mwing_inertia_atRot_min_damagedwing_all(i,1) = Mwing_inertia_damagedwing_all(n_rot_min_R,i);
        Mwing_aero_atRot_min_damagedwing_all(i,1)    = Mwing_aero_damagedwing_all(n_rot_min_R,i);
    else
        Mwing_total_atRot_min_damagedwing_all(i,1)   = nan;
        Mwing_mass_atRot_min_damagedwing_all(i,1)    = nan;
        Mwing_inertia_atRot_min_damagedwing_all(i,1) = nan;
        Mwing_aero_atRot_min_damagedwing_all(i,1)    = nan;
    end
    
    % min & max rotation angle value & location
...
%     freq = f_clipped_fly;
    ...
    % remove spanwise torques at removed part of damaged wing (n>N_cut)
    Mmass_sect_R(N_cut+1:end,:) = nan;
    Maero_sect_R(N_cut+1:end,:) = nan;
...
%% wing rotation spring constants
k_spring_intact  = (Mwing_total_atRot_min_intactwing_all  - Mwing_total_atRot_max_intactwing_all)  ./ (rot_max_intactwing_all  - rot_min_intactwing_all);
k_spring_damaged = (Mwing_total_atRot_min_damagedwing_all - Mwing_total_atRot_max_damagedwing_all) ./ (rot_max_damagedwing_all - rot_min_damagedwing_all);

aplha0_intact = rot_max_intactwing_all - Mwing_total_atRot_max_intactwing_all ./ k_spring_intact;
aplha0_damaged = rot_max_damagedwing_all - Mwing_total_atRot_max_damagedwing_all ./ k_spring_damaged;

%% save data & plot
if save_on == 1
    save(['allMODs_TipClip_freqAsym',num2str(freq_asymFitNr),'_peakloc',num2str(loc_set),'.mat'])
end
...
figure
subplot(2,2,1)
plot(rot_max_intactwing_all,Mwing_total_atRot_max_intactwing_all,'k-s','markerfacecolor','b','markersize',10)
hold on
plot(rot_max_damagedwing_all,Mwing_total_atRot_max_damagedwing_all,'k-o','markerfacecolor','c','markersize',10)
xlabel('wing rotation downstroke')
ylabel('spanwise wing torque downstroke')
legend('intact wing','damaged wing')

subplot(2,2,2)
plot(rot_min_intactwing_all,Mwing_total_atRot_min_intactwing_all,'k-s','markerfacecolor','b','markersize',10)
hold on
plot(rot_min_damagedwing_all,Mwing_total_atRot_min_damagedwing_all,'k-o','markerfacecolor','c','markersize',10)
xlabel('wing rotation upstroke')
ylabel('spanwise wing torque upstroke')
% legend('intact wing','damaged wing')

subplot(2,2,3)
plot(S2S3AmpRatioFuncs,k_spring_intact,'k-s','markerfacecolor','b','markersize',10)
hold on
plot(S2S3AmpRatioFuncs,k_spring_damaged,'k-o','markerfacecolor','c','markersize',10)
xlabel('S2S3AmpRatioFuncs')
ylabel('spring stiffness k')
% legend('intact wing','damaged wing')

subplot(2,2,4)
plot(S2S3AmpRatioFuncs,aplha0_intact,'k-s','markerfacecolor','b','markersize',10)
hold on
plot(S2S3AmpRatioFuncs,aplha0_damaged,'k-o','markerfacecolor','c','markersize',10)
xlabel('S2S3AmpRatioFuncs')
ylabel('spring set-point a0')
% legend('intact wing','damaged wing')
