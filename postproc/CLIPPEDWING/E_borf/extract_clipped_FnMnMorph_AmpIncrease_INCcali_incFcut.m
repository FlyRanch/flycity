...
%         time_norm(:,m) = [0:1/(n_wb-1):1]';
        ...
        %% min steady WB & @CoM
%         Fx_norm_MinSteady(m,1) = Fx_norm(m,1)-Fx_norm_steady;
        Fy_norm_MinSteady(m,1) = Fy_norm(m,1)-Fy_norm_steady;
%         Fz_norm_MinSteady(m,1) = Fz_norm(m,1)-Fz_norm_steady;

        Mx_norm_MinSteady(m,1) = Mx_norm(m,1)-Mx_norm_steady;
        My_norm_MinSteady(m,1) = My_norm(m,1)-My_norm_steady;
        Mz_norm_MinSteady(m,1) = Mz_norm(m,1)-Mz_norm_steady;

        My_norm_CoM(m,1) = My_norm(m,1) - d_norm_steady*Fz_norm(m,1);
        
        %% cut wing forces (Fcut = Ftotal - 1/2 * Ftotal,steady)
        Fx_norm_cut(m,1) = Fx_norm(m,1) - 0.5 * Fx_norm_steady;
        Fy_norm_cut(m,1) = Fy_norm(m,1) - 0.5 * Fy_norm_steady;
        Fz_norm_cut(m,1) = Fz_norm(m,1) - 0.5 * Fz_norm_steady;
        
        Fy_norm_cut_MinSteady(m,1) = Fy_norm_MinSteady(m,1);
