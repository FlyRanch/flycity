...
for i = 1:length(Fx_norm)
    ...
    subplot(1,3,1)
    color_val = (Fx_norm_now - Fx_min) / (Fx_max - Fx_min) * (length(cmap_F)-1) +1;
    ...
    subplot(1,3,2)
    color_val = (Fy_norm_now - Fy_min) / (Fy_max - Fy_min) * (length(cmap_F)-1) +1;
    color_now = cmap_F(round(color_val),:);
    ...
    subplot(1,3,3)
    color_val = (Fz_norm_now - Fz_min) / (Fz_max - Fz_min) * (length(cmap_F)-1) +1;
    color_now = cmap_F(round(color_val),:);
    ...
    subplot(1,3,1)
    color_val = (Mx_norm_MinSteady_now - Mx_min) / (Mx_max - Mx_min) * (length(cmap_T)-1) +1;
    ...
    subplot(1,3,2)
    color_val = (My_norm_CoM_now - My_min) / (My_max - My_min) * (length(cmap_T)-1) +1;
    color_now = cmap_T(round(color_val),:);
    ...
    subplot(1,3,3)
    color_val = (Mz_norm_MinSteady_now - Mz_min) / (Mz_max - Mz_min) * (length(cmap_T)-1) +1;
    color_now = cmap_T(round(color_val),:);
    ...
    subplot(1,2,1)
    color_val = (Fz_norm_now - Fz_min) / (Fz_max - Fz_min) * (length(cmap_F_neg)-1) +1;
