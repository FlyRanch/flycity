...
%         plot([x_wing Fx_tip],[y_wing Fy_tip],'-','linewidth',1,'color',F_map(i,:))
        h=quiver(x_wing,y_wing,Fx_tip,Fy_tip,'linewidth',2,'color',F_map(i,:));
        adjust_quiver_arrowhead_size(h, 3);
        hold on
        plot([x_LE x_TE],[y_LE y_TE],'-','linewidth',3,'color',cord_map(i,:))
        plot([x_LE],[y_LE],'o','markersize',8,'markerfacecolor',cord_map(i,:),'color',cord_map(i,:))
        axis tight
        axis equal
        axis([-90 90 -30 30]) 
        axis off
    ...
    saveas(gca, ['profNforce_PitchAccel_',num2str(j),'.tiff'])
end
cd ..
