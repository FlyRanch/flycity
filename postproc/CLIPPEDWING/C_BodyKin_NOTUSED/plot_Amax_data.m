...
hold on
X = rose(deg2rad(stim_angle_F_now),n_bins_rose);
for i=1:length(X)
...
polar(0, maxHistogramValue,'-k')
hold on
X = rose(deg2rad(Fsp_roll_now),n_bins_rose);
for i=1:length(X)
    set(X(i),'color','k','linewidth',1)
end
x = get(X,'Xdata');
...
polar(0, maxHistogramValue,'-k')
hold on
X = rose(deg2rad(roll_now),n_bins_rose);
for i=1:length(X)
    set(X(i),'color','k','linewidth',1)
end
x = get(X,'Xdata');
...
polar(0, maxHistogramValue,'-k')
hold on
X = rose(deg2rad(yaw_now),n_bins_rose);
for i=1:length(X)
    set(X(i),'color','k','linewidth',1)
end
x = get(X,'Xdata');
