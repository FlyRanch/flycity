...
%% cluster cmap
cmap_k = [0 0 0;  1 0 0];   % green; yellow; orange
...
% toplot=0;
if toplot == 1
    figdir = ['accel_plots_Ahor',num2str(A_thresh),'mps2_startframe',num2str(start_frame)];
    mkdir(figdir)
end
...
%% Manual cluster A
steady_frames = nan(size(A_hor));
steady_frames(abs(A_hor) < A_thresh) = 1;
steady_frames(abs(A_hor) >= A_thresh) = 0;

%% save data
pathDB.steady_frames = steady_frames;
...
batch_calc_responseDB_subfunc_CLIP

save(savename,'pathDB','settings')
