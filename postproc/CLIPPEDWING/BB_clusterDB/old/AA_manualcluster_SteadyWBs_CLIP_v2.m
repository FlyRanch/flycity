...
% toplot=0;
if toplot == 1
    figdir = ['accel_plots_Atot',num2str(A_thresh),'mps2_startframe',num2str(start_frame)];
    mkdir(figdir)
end
...
%% Manual cluster A
steady_frames = nan(size(A));
steady_frames(abs(A) < A_thresh) = 1;
steady_frames(abs(A) >= A_thresh) = 0;
