...
load('flightpathDB_pos_qbodyEKF_INCroll_strokeplane47.5deg_rotaxis0deg_startframe1.mat')

mkdir('EMcluster_Ahorver')
cd('EMcluster_Ahorver')
...
% !!! accel pos, decel neg !!!
A_hork = A_hork .* sign(At_hork);

% % !!! right pos, left neg !!!
% A_hork = A_hork .* sign(An_hork);
...
plot(A_hork(1:skip:end),A_verk(1:skip:end),'.k','markersize',5)
...
set(gca,'XTick',[-6:3:6],'fontsize',12)
set(gca,'YTick',[-6:3:6],'fontsize',12)


saveas(gca,['Ahorver_unclustered_skip',num2str(skip),'.fig'])
saveas(gca,['Ahorver_unclustered_skip',num2str(skip),'.png'])
% plot2svg(['Ahorver_unclustered_skip',num2str(skip),'.svg'])

%% cluster variables
X_accel = [A_hork A_verk];
...
hold off
axis equal
xlabel('A_h_o_r','fontsize',18)
ylabel('A_v_e_r','fontsize',18)
set(gca,'xlim',[-6 6],'ylim',[-6 6])
