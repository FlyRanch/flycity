...
subplot(2,2,3)
plot(SecondMomentRatio,pitch_mean_wb,'.g')
plot(SecondMomentRatio_d2ndMom,pitch_d2ndMom,'.b')

% plot trend
RC = pitchMOD_d2ndMom_meanCIstd(1);
x0 = xmax;
...
axis([ang_min ang_max 0 100*ceil(max(h)/100)])
    set(gca,'XTick',ang_min:(ang_max-ang_min)/4:ang_max) 
%     set(gca,'YTick',0:100:500,'fontsize',8)
    xlabel('pitch/d2ndMom (pitchMOD)','fontsize',10)
%     grid on

subplot(2,2,4)
plot(SecondMomentRatio_d2ndMom,pitchMOD_d2ndMom,'.b')
...
plot([x0 x1],[y0 y1],'-r','linewidth',2)
xlabel('SecondMomentRatio','fontsize',10)
ylabel('pitch/d2ndMom (pitchMOD)','fontsize',10)
