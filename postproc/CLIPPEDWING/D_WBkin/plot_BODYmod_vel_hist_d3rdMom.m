...
subplot(2,2,3)
plot(ThirdMomentRatio,V_mean_wb,'.g')
plot(ThirdMomentRatio_d3rdMom,vel_d3rdMom,'.b')

% plot trend
RC = velMOD_d3rdMom_meanCIstd(1);
x0 = xmax;
...
axis([ang_min ang_max 0 100*ceil(max(h)/100)])
    set(gca,'XTick',ang_min:(ang_max-ang_min)/4:ang_max) 
%     set(gca,'YTick',0:100:500,'fontsize',8)
    xlabel('vel/d3rdMom (velMOD)','fontsize',10)
%     grid on

subplot(2,2,4)
plot(ThirdMomentRatio_d3rdMom,velMOD_d3rdMom,'.b')
...
plot([x0 x1],[y0 y1],'-r','linewidth',2)
xlabel('ThirdMomentRatio','fontsize',10)
ylabel('vel/d3rdMom (velMOD)','fontsize',10)
