...
subplot(2,2,3)
plot(ThirdMomentRatio,slip_mean_wb,'.g')
plot(ThirdMomentRatio_d3rdMom,slip_d3rdMom,'.b')

% plot trend
RC = slipMOD_d3rdMom_meanCIstd(1);
x0 = xmax;
y0 = 0;
x1 = xmin;
y1 = y0 + (x1-x0)*RC;
...
axis([ang_min ang_max 0 100*ceil(max(h)/100)])
    set(gca,'XTick',ang_min:(ang_max-ang_min)/4:ang_max) 
%     set(gca,'YTick',0:100:500,'fontsize',8)
    xlabel('slip/d3rdMom (slipMOD)','fontsize',10)
%     grid on

subplot(2,2,4)
plot(ThirdMomentRatio_d3rdMom,slipMOD_d3rdMom,'.b')
...
plot([x0 x1],[y0 y1],'-r','linewidth',2)
xlabel('ThirdMomentRatio','fontsize',10)
ylabel('slip/d3rdMom (slipMOD)','fontsize',10)
