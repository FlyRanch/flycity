...
Mroll_accel_Fenhance = [];
Mpitch_accel_Fenhance = [];
Myaw_accel_Fenhance = [];
M_L_accel_Fenhance = [];
M_R_accel_Fenhance = [];

Mroll_damp_Fenhance = [];
Mpitch_damp_Fenhance = [];
Myaw_damp_Fenhance = [];
M_L_damp_Fenhance = [];
M_R_damp_Fenhance = [];
...
%         if abs(F_mean_wb(wb)-1) > Fenhance_limit_mod
        if F_mean_wb(wb)-1 > Fenhance_limit_mod
            ...
            velMOD_wb_now = (nanmean(V_now) - vel_wb_steady) / (F_mean_wb_now-1) * Fenhance_norm;
            ...
            Mroll_accel_Fenhance(n_now,1) = Mroll_mean_wb_accel_now;
            Mpitch_accel_Fenhance(n_now,1) = Mpitch_mean_wb_accel_now;
            Myaw_accel_Fenhance(n_now,1) = Myaw_mean_wb_accel_now;
            M_L_accel_Fenhance(n_now,1) = M_L_mean_wb_accel_now;
            M_R_accel_Fenhance(n_now,1) = M_R_mean_wb_accel_now;

            Mroll_damp_Fenhance(n_now,1) = Mroll_mean_wb_damp_now;
            Mpitch_damp_Fenhance(n_now,1) = Mpitch_mean_wb_damp_now;
            Myaw_damp_Fenhance(n_now,1) = Myaw_mean_wb_damp_now;
            M_L_damp_Fenhance(n_now,1) = M_L_mean_wb_damp_now;
            M_R_damp_Fenhance(n_now,1) = M_R_mean_wb_damp_now;

            Mroll_Fenhance(n_now,1) = Mroll_mean_wb_now;
            Mpitch_Fenhance(n_now,1) = Mpitch_mean_wb_now;
            Myaw_Fenhance(n_now,1) = Myaw_mean_wb_now;
            M_L_Fenhance(n_now,1) = M_L_mean_wb_now;
            M_R_Fenhance(n_now,1) = M_R_mean_wb_now;
            ...
            velMOD_wb_Fenhance(n_now,1) = velMOD_wb_now;
...
% wbMOD means & 95%CI
velMOD_wb_Fenhance_meanCIstd = [nanmean(velMOD_wb_Fenhance) 1.96*nanstd(velMOD_wb_Fenhance)/sqrt(length(velMOD_wb_Fenhance)) nanstd(velMOD_wb_Fenhance)];
