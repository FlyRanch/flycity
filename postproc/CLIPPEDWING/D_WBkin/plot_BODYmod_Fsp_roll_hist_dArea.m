...
subplot(2,2,3)
plot(AreaRatio,Fsp_roll_mean_wb,'.g')
plot(AreaRatio_dArea,Fsp_roll_dArea,'.b')

% plot trend
RC = Fsp_rollMOD_dArea_meanCIstd(1);
x0 = xmax;
y0 = 0;
x1 = xmin;
y1 = y0 + (x1-x0)*RC;
...
axis([ang_min ang_max 0 100*ceil(max(h)/100)])
    set(gca,'XTick',ang_min:(ang_max-ang_min)/4:ang_max) 
%     set(gca,'YTick',0:100:500,'fontsize',8)
    xlabel('FspRoll/dArea (FspRollMOD)','fontsize',10)
%     grid on

subplot(2,2,4)
plot(AreaRatio_dArea,Fsp_rollMOD_dArea,'.b')
...
plot([x0 x1],[y0 y1],'-r','linewidth',2)
xlabel('AreaRatio','fontsize',10)
ylabel('FspRoll/dArea (FspRollMOD)','fontsize',10)
