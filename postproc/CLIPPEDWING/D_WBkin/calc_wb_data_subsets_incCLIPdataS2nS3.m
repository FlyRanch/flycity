...
%         counter = size(n_down_start_L,2)*100+size(n_down_start_L,1)-seq*100-wb
        ...
        if isnan(n_down_start_L_now)==0 && isnan(n_up_stop_L_now)==0 && isnan(n_down_start_R_now)==0 && isnan(n_up_stop_R_now)==0
            ...
            SecondMomentClipped(n_now,1) = SecondMomentClipped_all(seq);
            SecondMomentIntact(n_now,1) = SecondMomentIntact_all(seq);
            SecondMomentRatio(n_now,1) = SecondMomentRatio_all(seq);

            ThirdMomentClipped(n_now,1) = ThirdMomentClipped_all(seq);
            ThirdMomentIntact(n_now,1) = ThirdMomentIntact_all(seq);
            ThirdMomentRatio(n_now,1) = ThirdMomentRatio_all(seq);

            AreaClipped(n_now,1) = AreaClipped_all(seq);
            AreaIntact(n_now,1) = AreaIntact_all(seq);
            AreaRatio(n_now,1) = AreaRatio_all(seq);

            LengthClipped(n_now,1) = LengthClipped_all(seq);
            LengthIntact(n_now,1) = LengthIntact_all(seq);
