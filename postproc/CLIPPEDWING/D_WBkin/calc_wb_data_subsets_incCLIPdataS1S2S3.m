...
SecondMomentNormClipped = [];
SecondMomentNormIntact = [];
SecondMomentRatio = [];

ThirdMomentNormClipped = [];
ThirdMomentNormIntact = [];
ThirdMomentRatio = [];
...
%         counter = size(n_down_start_L,2)*100+size(n_down_start_L,1)-seq*100-wb
        ...
        if isnan(n_down_start_L_now)==0 && isnan(n_up_stop_L_now)==0 && isnan(n_down_start_R_now)==0 && isnan(n_up_stop_R_now)==0

            n_now = n_now + 1;

            %% clip data
            clip_side(n_now,1) = clip_side_all(seq);
            clip_type(n_now,1) = clip_type_all(seq);

            FirstMomentNormClipped(n_now,1) = FirstMomentNormClipped_all(seq);
            FirstMomentNormIntact(n_now,1) = FirstMomentNormIntact_all(seq);
            FirstMomentRatio(n_now,1) = FirstMomentRatio_all(seq);

            SecondMomentNormClipped(n_now,1) = SecondMomentNormClipped_all(seq);
            SecondMomentNormIntact(n_now,1) = SecondMomentNormIntact_all(seq);
            SecondMomentRatio(n_now,1) = SecondMomentRatio_all(seq);

            ThirdMomentNormClipped(n_now,1) = ThirdMomentNormClipped_all(seq);
            ThirdMomentNormIntact(n_now,1) = ThirdMomentNormIntact_all(seq);
            ThirdMomentRatio(n_now,1) = ThirdMomentRatio_all(seq);

            AreaNormClipped(n_now,1) = AreaNormClipped_all(seq);
            AreaNormIntact(n_now,1) = AreaNormIntact_all(seq);
            AreaRatio(n_now,1) = AreaRatio_all(seq);

            LengthClipped_pixels(n_now,1) = LengthClipped_pixels_all(seq);
            LengthIntact_pixels(n_now,1) = LengthIntact_pixels_all(seq);
            LengthRatio(n_now,1) = LengthRatio_all(seq);
            ...
            %% body kin
            steady_nr_wb_now = steady_frames(n_down_start_L_now:n_up_stop_L_now,seq);
            V_wb_now = V(n_down_start_L_now:n_up_stop_L_now,seq);
            l_wing_wb_now = wing_length(seq);
            
            slip_wb_now = slip_mirror(n_down_start_L_now:n_up_stop_L_now,seq);
            pitch_wb_now = pitch(n_down_start_L_now:n_up_stop_L_now,seq);
            roll_wb_now = roll_mirror(n_down_start_L_now:n_up_stop_L_now,seq);
            
            dyaw_wb_now = dyaw_mirror(n_down_start_L_now:n_up_stop_L_now,seq);
            dpitch_wb_now = dpitch(n_down_start_L_now:n_up_stop_L_now,seq);
            droll_wb_now = droll_mirror(n_down_start_L_now:n_up_stop_L_now,seq);
            ...
            % interpolate
            stroke_wb_L_interp = radtodeg(interp1(stroke_wb_L_now,ni_wb_L));
            stroke_wb_R_interp = radtodeg(interp1(stroke_wb_R_now,ni_wb_R));
            pitch_wb_L_interp = radtodeg(interp1(pitch_wb_L_now,ni_wb_L));
            pitch_wb_R_interp = radtodeg(interp1(pitch_wb_R_now,ni_wb_R));
            dev_wb_L_interp = radtodeg(interp1(dev_wb_L_now,ni_wb_L));
            dev_wb_R_interp = radtodeg(interp1(dev_wb_R_now,ni_wb_R));
            aoa_wb_L_interp = radtodeg(interp1(aoa_wb_L_now,ni_wb_L));
            aoa_wb_R_interp = radtodeg(interp1(aoa_wb_R_now,ni_wb_R));
            U_wb_L_interp = interp1(U_wb_L_now,ni_wb_L)/1000;
            U_wb_R_interp = interp1(U_wb_R_now,ni_wb_R)/1000;
            
            stroke_ds_L_interp = radtodeg(interp1(stroke_ds_L_now,ni_ds_L));
            stroke_ds_R_interp = radtodeg(interp1(stroke_ds_R_now,ni_ds_R));
            pitch_ds_L_interp = radtodeg(interp1(pitch_ds_L_now,ni_ds_L));
            pitch_ds_R_interp = radtodeg(interp1(pitch_ds_R_now,ni_ds_R));
            dev_ds_L_interp = radtodeg(interp1(dev_ds_L_now,ni_ds_L));
            dev_ds_R_interp = radtodeg(interp1(dev_ds_R_now,ni_ds_R));
            aoa_ds_L_interp = radtodeg(interp1(aoa_ds_L_now,ni_ds_L));
            aoa_ds_R_interp = radtodeg(interp1(aoa_ds_R_now,ni_ds_R));
            U_ds_L_interp = interp1(U_ds_L_now,ni_ds_L)/1000;
            U_ds_R_interp = interp1(U_ds_R_now,ni_ds_R)/1000;
            
            stroke_us_L_interp = radtodeg(interp1(stroke_us_L_now,ni_us_L));
            stroke_us_R_interp = radtodeg(interp1(stroke_us_R_now,ni_us_R));
            pitch_us_L_interp = radtodeg(interp1(pitch_us_L_now,ni_us_L));
            pitch_us_R_interp = radtodeg(interp1(pitch_us_R_now,ni_us_R));
            dev_us_L_interp = radtodeg(interp1(dev_us_L_now,ni_us_L));
            dev_us_R_interp = radtodeg(interp1(dev_us_R_now,ni_us_R));
            aoa_us_L_interp = radtodeg(interp1(aoa_us_L_now,ni_us_L));
            aoa_us_R_interp = radtodeg(interp1(aoa_us_R_now,ni_us_R));
            ...
            stroke_wb_L(1:length(t_wb_L_now),n_now) = radtodeg(stroke_wb_L_now);
            stroke_wb_R(1:length(t_wb_R_now),n_now) = radtodeg(stroke_wb_R_now);
            pitch_wb_L(1:length(t_wb_L_now),n_now) = radtodeg(pitch_wb_L_now);
            pitch_wb_R(1:length(t_wb_R_now),n_now) = radtodeg(pitch_wb_R_now);
            dev_wb_L(1:length(t_wb_L_now),n_now) = radtodeg(dev_wb_L_now);
            dev_wb_R(1:length(t_wb_R_now),n_now) = radtodeg(dev_wb_R_now);
            aoa_wb_L(1:length(t_wb_L_now),n_now) = radtodeg(aoa_wb_L_now);
            aoa_wb_R(1:length(t_wb_R_now),n_now) = radtodeg(aoa_wb_R_now);
            ...
            stroke_ds_L(1:length(t_ds_L_now),n_now) = radtodeg(stroke_ds_L_now);
            stroke_ds_R(1:length(t_ds_R_now),n_now) = radtodeg(stroke_ds_R_now);
            pitch_ds_L(1:length(t_ds_L_now),n_now) = radtodeg(pitch_ds_L_now);
            pitch_ds_R(1:length(t_ds_R_now),n_now) = radtodeg(pitch_ds_R_now);
            dev_ds_L(1:length(t_ds_L_now),n_now) = radtodeg(dev_ds_L_now);
            dev_ds_R(1:length(t_ds_R_now),n_now) = radtodeg(dev_ds_R_now);
            aoa_ds_L(1:length(t_ds_L_now),n_now) = radtodeg(aoa_ds_L_now);
            aoa_ds_R(1:length(t_ds_R_now),n_now) = radtodeg(aoa_ds_R_now);
            ...
            stroke_us_L(1:length(t_us_L_now),n_now) = radtodeg(stroke_us_L_now);
            stroke_us_R(1:length(t_us_R_now),n_now) = radtodeg(stroke_us_R_now);
            pitch_us_L(1:length(t_us_L_now),n_now) = radtodeg(pitch_us_L_now);
            pitch_us_R(1:length(t_us_R_now),n_now) = radtodeg(pitch_us_R_now);
            dev_us_L(1:length(t_us_L_now),n_now) = radtodeg(dev_us_L_now);
            dev_us_R(1:length(t_us_R_now),n_now) = radtodeg(dev_us_R_now);
            aoa_us_L(1:length(t_us_L_now),n_now) = radtodeg(aoa_us_L_now);
            aoa_us_R(1:length(t_us_R_now),n_now) = radtodeg(aoa_us_R_now);
            ...
            %% store body kin data
            steady_nr_wb(1:fr_max,n_now) = nan;
            steady_nr_wb(1:length(steady_nr_wb_now),n_now) = steady_nr_wb_now;
            
            steady_nr_mean_wb(n_now,1) = nanmean(steady_nr_wb_now);
            V_mean_wb(n_now,1) = nanmean(V_wb_now);
            l_wing_mean_wb(n_now,1) = nanmean(l_wing_wb_now);

            slip_mean_wb(n_now,1) = nanmean(slip_wb_now);
            pitch_mean_wb(n_now,1) = nanmean(pitch_wb_now);
            roll_mean_wb(n_now,1) = nanmean(roll_wb_now);
            
            dyaw_mean_wb(n_now,1) = nanmean(dyaw_wb_now);
            dpitch_mean_wb(n_now,1) = nanmean(dpitch_wb_now);
            droll_mean_wb(n_now,1) = nanmean(droll_wb_now);
...
% save wb
save(['WBdataset_all_',num2str(n_wb),'WBs_S2nS3.mat'],...
...
't_wb_bin',...
't_ds_bin',...
't_us_bin',...
