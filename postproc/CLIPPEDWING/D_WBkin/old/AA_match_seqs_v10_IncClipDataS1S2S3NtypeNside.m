...
clip_name = dir('WingClip*');
clip_name = clip_name.name;
load(clip_name);

clip_TnS = dir('clip_type*');
clip_TnS = clip_TnS.name;
load(clip_TnS);

% loadname = 'flightpathDB_pos_qbodyEKF_INCroll_9clusters_1.4n-1.7n1.9_strokeplane47.5deg_startframe100.mat';
loadname = 'flightpathDB_pos_qbodyEKF_INCroll_strokeplane47.5deg_rotaxis0deg_startframe1.mat';
load(loadname)
...
seqs_kin = nan([length(seq_names),3]);
...
%     seqs_kin(i,2) = str2num(seq_now(end-3:end));
    ...
    file_seps = strfind(seq_now,filesep);
    seqs_kin(i,1) = str2num(seq_now(file_seps(end)+1:file_seps(end)+8));
    seqs_kin(i,2) = str2num(seq_now(file_seps(end)+11:file_seps(end)+14));

    if length(seq_now) > file_seps(end)+14
        seqs_kin(i,3) = str2num(seq_now(file_seps(end)+16));
    else
        seqs_kin(i,3) = 1;
    end
...
%     end
    ...
    % clipdata seq nr
    seqc = find(WingClipData.date == date_now & WingClipData.seq == seq_now);
    if isempty(seqc)==0
        settings.seq_clip(seqp,:) = [WingClipData.date(seqc) WingClipData.seq(seqc)];
        
        clipDB.FirstMomentCC_norm(seqp,1) = WingClipData.FirstMomentCC_norm(seqc);
        clipDB.FirstMomentCI_norm(seqp,1) = WingClipData.FirstMomentCI_norm(seqc);
        clipDB.FirstMomentTC_norm(seqp,1) = WingClipData.FirstMomentTC_norm(seqc);
        clipDB.FirstMomentTI_norm(seqp,1) = WingClipData.FirstMomentTI_norm(seqc);
        clipDB.FirstMomentRatioCentroid(seqp,1) = WingClipData.FirstMomentRatioCentroid(seqc);
        clipDB.FirstMomentRatioTip(seqp,1) = WingClipData.FirstMomentRatioTip(seqc);
        
        clipDB.SecondMomentCC_norm(seqp,1) = WingClipData.SecondMomentCC_norm(seqc);
        clipDB.SecondMomentCI_norm(seqp,1) = WingClipData.SecondMomentCI_norm(seqc);
        clipDB.SecondMomentTC_norm(seqp,1) = WingClipData.SecondMomentTC_norm(seqc);
        clipDB.SecondMomentTI_norm(seqp,1) = WingClipData.SecondMomentTI_norm(seqc);
        clipDB.SecondMomentRatioCentroid(seqp,1) = WingClipData.SecondMomentRatioCentroid(seqc);
        clipDB.SecondMomentRatioTip(seqp,1) = WingClipData.SecondMomentRatioTip(seqc);
        
        clipDB.ThirdMomentCC_norm(seqp,1) = WingClipData.ThirdMomentCC_norm(seqc);
        clipDB.ThirdMomentCI_norm(seqp,1) = WingClipData.ThirdMomentCI_norm(seqc);
        clipDB.ThirdMomentTC_norm(seqp,1) = WingClipData.ThirdMomentTC_norm(seqc);
        clipDB.ThirdMomentTI_norm(seqp,1) = WingClipData.ThirdMomentTI_norm(seqc);
        clipDB.ThirdMomentRatioCentroid(seqp,1) = WingClipData.ThirdMomentRatioCentroid(seqc);
        clipDB.ThirdMomentRatioTip(seqp,1) = WingClipData.ThirdMomentRatioTip(seqc);
        
        clipDB.WingAreaC_norm(seqp,1) = WingClipData.WingAreaC_norm(seqc);
        clipDB.WingAreaI_norm(seqp,1) = WingClipData.WingAreaI_norm(seqc);
        clipDB.WingAreaRatio(seqp,1) = WingClipData.WingAreaRatio(seqc);
        
        clipDB.WingLengthC_pixels(seqp,1) = WingClipData.WingLengthC_pixels(seqc);
        clipDB.WingLengthI_pixels(seqp,1) = WingClipData.WingLengthI_pixels(seqc);
        clipDB.WingLengthRatio(seqp,1) = WingClipData.WingLengthRatio(seqc);
    end
    ...
    if isempty(seqTnS)==0
        ...
        clipDB.clip_type(seqp,1) = clip_typeNside.clip_type(seqTnS);
        clipDB.clip_side(seqp,1) = clip_typeNside.clip_side(seqTnS);
    end
        
    % kin seq nr
    seqk = find(seqs_kin(:,1) == date_now & seqs_kin(:,2) == seq_now & seqs_kin(:,3) == part_now);
    if isempty(seqk)==0
