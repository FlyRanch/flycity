...
%             settings.seq(seq,2) = str2num(dir_now(end-3:end));
            ...
            file_seps = strfind(dir_now,filesep);
            settings.seq(seq,1) = str2num(dir_now(file_seps(end)+1:file_seps(end)+8));
            settings.seq(seq,2) = str2num(dir_now(file_seps(end)+11:file_seps(end)+14));
            
            if length(dir_now) > file_seps(end)+14
                settings.seq(seq,3) = str2num(dir_now(file_seps(end)+16));
%             else
%                 settings.seq(seq,3) = 1;
            end
