function pte_body_strokepl_L_R(settings, seq_nr, qL1_f2,qL2_f2,qL3_f2,qL4_f2,qR1_f2,qR2_f2,qR3_f2,qR4_f2,t,fig_nr, save_on_off)
    ...
    title('Comparison wing kinematics left and right, body mounted strokeplane')
    xlabel('t [s]')
    ylabel('phi [deg]')
    legend('left','right')
    ...
    saveas(fig_nr, [char(settings.plot_folders(1)) '/' char(settings.sequence_names(seq_nr)) '/pte_bref_lr'], 'fig')
    
    saveas(fig_nr, [char(settings.plot_folders(2)) '/pte_bref_lr/pte_bref_lr_' int2str(seq_nr)], 'fig')
