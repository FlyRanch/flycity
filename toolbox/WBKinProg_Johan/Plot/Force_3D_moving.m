function Force_3D_moving(settings,pathDB,seq_nr,save_on_off)
    ...
    movfilename = ['Force_movie_3D_moving_' char(settings.sequence_names(i)) '.mj2'];
    ...
    for m= 1:end_L
        ...
        xsi = pathDB.xsi_body_mean(m,1,i);
            ...
            zlabel('z')
            hold on;
            subplot(2,3,2); Force_plot4(settings, pathDB, i, m, pi, theta, 0 ,1,0);
            xlabel('x')
            ylabel('y')
            zlabel('z')
            view(180,0)
            hold on;
            subplot(2,3,3); Force_plot4(settings, pathDB, i, m, pi, 0, xsi ,1,1);
            xlabel('x')
            ylabel('y')
            zlabel('z')
            view(0,90)
            ...
            view(135,30)
            hold on;
            subplot(2,3,5); Force_plot4(settings, pathDB, i, m, pi, theta, 0 ,0,1);
            xlabel('x')
            ylabel('y')
            zlabel('z')
            view(0,0)
            hold on;
            subplot(2,3,6); Force_plot4(settings, pathDB, i, m, phi, 0, 0,1,1);
            xlabel('x')
            ylabel('y')
            zlabel('z')
            view(90,0)
        ...
        saveas(fignum, [char(settings.plot_folders(3)) '/' char(settings.sequence_names(seq_nr)) '/Force_3D_moving/' int2str(m) ], 'fig')

        saveas(fignum, [char(settings.plot_folders(3)) '/' char(settings.sequence_names(seq_nr)) '/Force_3D_moving/' int2str(m) ], 'png')
