function [ L_wingtip_path, R_wingtip_path, L_wingbeat_loc, R_wingbeat_loc] = Wingbeat_LocFix( qL1, qL2, qL3, qL4, qR1, qR2, qR3, qR4, wing_l)
...
%% compare deviation values for down vs up
...
%% compare stroke values for down vs up

% left
str_L = Lwingtip(locL,3);

    if mean(str_L(1:2:end)) > mean(str_L(2:2:end))
       
        loc_upst_L = locL(1:2:end);
        loc_dwnst_L = locL(2:2:end);
        
    elseif mean(str_L(1:2:end)) < mean(str_L(2:2:end))
    ...
    % remove upstroke before downstroke
    if loc_upst_L(1) < loc_dwnst_L(1)
        loc_upst_L(1) = [];
    end
    
    % remove downstroke after upstroke
    if length(loc_upst_L) > length(loc_dwnst_L)
...
% right
str_R = Rwingtip(locR,3);

    if mean(str_R(1:2:end)) > mean(str_R(2:2:end))
       
        loc_upst_R = locR(1:2:end);
        loc_dwnst_R = locR(2:2:end);
        
    elseif mean(str_R(1:2:end)) < mean(str_R(2:2:end))
