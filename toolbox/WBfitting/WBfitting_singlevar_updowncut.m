function [a_fit, a_avg] = WBfitting_singlevar_updowncut(n_pol,t_loc,var_loc,Rds)
...
N_wb = length(n_stop);
    ...
    for j = 1:N_wb
        ...
        [ a1, a2, ratio_1_2 ] = local_fit_updowncut(var_loc(n_start(j):n_stop(j)),n_pol,Rds);
...
%         a_fit_wb1_wb2(j,2) = floor(((2-ratio_1_2)*(n_stop(j)-n_start(j)+1))/2);
        a_fit_wb1_wb2(j,1) = ceil(Rds*n_stop(j)-n_start(j)+1);
        a_fit_wb1_wb2(j,2) = (n_stop(j)-n_start(j)+1) -a_fit_wb1_wb2(j,1);
    end    
...
%         [a_avg1, a_avg2] = average_fit_periodic(a_fit.var1,a_fit.var2,n_pol);
        [a_avg1, a_avg2] = average_fit_periodic_updowncut(a_fit.var1,a_fit.var2,n_pol,Rds);
