...
function [pos,vel,accel] = batch_kalman_pos_Johan(pos_obs,t,settings,plotting,saving)
...
%     az(start:stop,i) = xsmooth(9,:)./dt./dt;
    ...
    % johan's smoother
    [ xsmooth ] = XYZ_body( Y, dt );

    x(start:stop,i) = xsmooth(1,:);
    y(start:stop,i) = xsmooth(2,:);
    z(start:stop,i) = xsmooth(3,:);
    
    u(start:stop,i) = xsmooth(4,:);
    v(start:stop,i) = xsmooth(5,:);
    w(start:stop,i) = xsmooth(6,:);
    
    ax(start:stop,i) = xsmooth(7,:);
    ay(start:stop,i) = xsmooth(8,:);
    az(start:stop,i) = xsmooth(9,:);
