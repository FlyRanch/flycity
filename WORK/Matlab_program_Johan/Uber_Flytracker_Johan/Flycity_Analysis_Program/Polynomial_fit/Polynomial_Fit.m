function Polynomial_Fit( settings, pathDB )
    ...
    for i = 1:N
        ...
        for j = 1:nr_wb
            ...
            rand_wbs.(char(wb_name)).seq_nr                         = i;
            ...
            rand_wbs.(char(wb_name)).wing_model.x_LE_L              = pathDB.wing_model.x_LE_L(i,:);
            ...
            rand_wbs.(char(wb_name)).wing_model.x_LE_R              = pathDB.wing_model.x_LE_R(i,:);
            ...
            rand_wbs.(char(wb_name)).body_kin.xyz                   = pathDB.filt.xyz(wb_loc(1):wb_loc(2),:,i);
            rand_wbs.(char(wb_name)).body_kin.qb                    = pathDB.filt.qB(wb_loc(1):wb_loc(2),:,i);
            rand_wbs.(char(wb_name)).body_kin.uvw                   = pathDB.filt.uvw(wb_loc(1):wb_loc(2),:,i);
            rand_wbs.(char(wb_name)).body_kin.wb                    = pathDB.filt.wB(wb_loc(1):wb_loc(2),:,i);
            rand_wbs.(char(wb_name)).body_kin.a_xyz                 = pathDB.filt.a_xyz(wb_loc(1):wb_loc(2),:,i);
            rand_wbs.(char(wb_name)).body_kin.xyz_mean              = mean(pathDB.filt.xyz(wb_loc(1):wb_loc(2),:,i));
            rand_wbs.(char(wb_name)).body_kin.qb_mean               = mean(pathDB.filt.qB(wb_loc(1):wb_loc(2),:,i));
            rand_wbs.(char(wb_name)).body_kin.uvw_mean              = mean(pathDB.filt.uvw(wb_loc(1):wb_loc(2),:,i));
            rand_wbs.(char(wb_name)).body_kin.wb_mean               = mean(pathDB.filt.wB(wb_loc(1):wb_loc(2),:,i));
            rand_wbs.(char(wb_name)).body_kin.a_xyz_mean            = mean(pathDB.filt.a_xyz(wb_loc(1):wb_loc(2),:,i));
...
%     
%     maneuver_t = load('maneuver.mat');
%     
%     % Remove weird wingbeats:
%     
%     [ maneuver_ax, weird_wb_ax ] = Remove_missed_wb( settings, maneuver_t.maneuver.ax );
%     [ maneuver_ay, weird_wb_ay ] = Remove_missed_wb( settings, maneuver_t.maneuver.ay );
%     [ maneuver_az, weird_wb_az ] = Remove_missed_wb( settings, maneuver_t.maneuver.az );
%     [ maneuver_wx, weird_wb_wx ] = Remove_missed_wb( settings, maneuver_t.maneuver.wx );
%     [ maneuver_wy, weird_wb_wy ] = Remove_missed_wb( settings, maneuver_t.maneuver.wy );
%     [ maneuver_wz, weird_wb_wz ] = Remove_missed_wb( settings, maneuver_t.maneuver.wz );

    % Remove weird wingbeats:
    
    [ maneuver_ax, weird_wb_ax ] = Remove_missed_wb( settings, maneuver.ax );
    [ maneuver_ay, weird_wb_ay ] = Remove_missed_wb( settings, maneuver.ay );
    [ maneuver_az, weird_wb_az ] = Remove_missed_wb( settings, maneuver.az );
    [ maneuver_wx, weird_wb_wx ] = Remove_missed_wb( settings, maneuver.wx );
    [ maneuver_wy, weird_wb_wy ] = Remove_missed_wb( settings, maneuver.wy );
    [ maneuver_wz, weird_wb_wz ] = Remove_missed_wb( settings, maneuver.wz );
    ...
    maneuver.ax = maneuver_ax;
    maneuver.ay = maneuver_ay;
    maneuver.az = maneuver_az;
    maneuver.wx = maneuver_wx;
    maneuver.wy = maneuver_wy;
    maneuver.wz = maneuver_wz;
