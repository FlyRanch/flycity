function Aero_Inertia_damping( settings, pathDB )
    ...
    [ kine_avg ] = angular_velocities_polynomial( a_avg );
    ...
    [ F_0, M_0 ] = Initial_cg_and_orientation( body_model, wing_model, a_avg, vb_0, wb_0);
...
%                 wb_0_range = zeros(3,nr_steps+1);
                ...
                for k = 1:(nr_steps+1)
                    ...
                    vb_0_range(:,k) = [vx_forward_range(k); vb_0(2); vb_0(3)];
                    wb_0_range(:,k) = wb_0;
...
%                 wb_0_range = zeros(3,nr_steps+1);
                ...
                for k = 1:(nr_steps+1)
                    ...
                    vb_0_range(:,k) = [vx_back_range(k); vb_0(2); vb_0(3)];
                    wb_0_range(:,k) = wb_0;
...
%                 wb_0_range = zeros(3,nr_steps+1);
                ...
                for k = 1:(nr_steps+1)
                    ...
                    vb_0_range(:,k) = [vb_0(1); vy_range(k); vb_0(3)];
                    wb_0_range(:,k) = wb_0;
...
%                 wb_0_range = zeros(3,nr_steps+1);
                ...
                for k = 1:(nr_steps+1)
                    ...
                    vb_0_range(:,k) = [vb_0(1); vb_0(2); vz_up_range(k)];
                    wb_0_range(:,k) = wb_0;
...
%                 wb_0_range = [ wx_range; zeros(1,nr_steps+1); zeros(1,nr_steps+1)];
                ...
                for k = 1:(nr_steps+1)
                    ...
                    vb_0_range(:,k) = vb_0;
                    wb_0_range(:,k) = [ wx_range(k); wb_0(2); wb_0(3)];
...
%                 wb_0_range = [ zeros(1,nr_steps+1); wy_down_range; zeros(1,nr_steps+1)];
                ...
                for k = 1:(nr_steps+1)
                    ...
                    vb_0_range(:,k) = vb_0;
                    wb_0_range(:,k) = [ wb_0(1); wy_down_range(k); wb_0(3)];
...
%                 wb_0_range = [ zeros(1,nr_steps+1); wy_up_range; zeros(1,nr_steps+1)];
                ...
                for k = 1:(nr_steps+1)
                    ...
                    vb_0_range(:,k) = vb_0;
                    wb_0_range(:,k) = [ wb_0(1); wy_up_range(k); wb_0(3)];
...
%                 wb_0_range = [ zeros(1,nr_steps+1); zeros(1,nr_steps+1); wz_range];
                ...
                for k = 1:(nr_steps+1)
                    ...
                    vb_0_range(:,k) = vb_0;
                    wb_0_range(:,k) = [ wb_0(1); wb_0(2); wz_range(k)];
...
%             Mz_tot(:,j,i)         = M_tot(:,i,j,8)-MI(:,i,j,8);
            ...
            Fx_forward_A(:,j,i)   = FA(:,i,j,1)-F_0;
            Mx_forward_A(:,j,i)   = MA(:,i,j,1)-M_0;

            Fx_back_A(:,j,i)      = FA(:,i,j,2)-F_0;
            Mx_back_A(:,j,i)      = MA(:,i,j,2)-M_0;

            Fy_A(:,j,i)           = FA(:,i,j,3)-F_0;
            My_A(:,j,i)           = MA(:,i,j,3)-M_0;

            Fz_up_A(:,j,i)        = FA(:,i,j,4)-F_0;
            Mz_up_A(:,j,i)        = MA(:,i,j,4)-M_0;

            Fx_A(:,j,i)           = FA(:,i,j,5)-F_0;
            Mx_A(:,j,i)           = MA(:,i,j,5)-M_0;

            Fy_down_A(:,j,i)      = FA(:,i,j,6)-F_0;
            My_down_A(:,j,i)      = MA(:,i,j,6)-M_0;

            Fy_up_A(:,j,i)        = FA(:,i,j,7)-F_0;
            My_up_A(:,j,i)        = MA(:,i,j,7)-M_0;

            Fz_A(:,j,i)           = FA(:,i,j,8)-F_0;
            Mz_A(:,j,i)           = MA(:,i,j,8)-M_0;
...
%     print ([char(settings.plot_loc) '/Damping/Inertial_wz.eps'] ,'-depsc2'); 
    ...
    for j = 1:(nr_steps+1)
        ...
        plot(vx_forward_range,Fx_forward_tot(1,:,j),'Color',[ (0.5+(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) ])
        plot(vx_back_range,Fx_back_tot(1,:,j),'Color',[ (0.5+(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) ])
        hold off
        hold off
    end
    xlabel('v_x [mm/s]')
    ...
    for j = 1:(nr_steps+1)
        ...
        plot(vx_forward_range,Fx_forward_tot(2,:,j),'Color',[ (0.5-(0.5*(j-1))/nr_steps) (0.5+(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) ])
        plot(vx_back_range,Fx_back_tot(2,:,j),'Color',[ (0.5-(0.5*(j-1))/nr_steps) (0.5+(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) ])
        hold off
        hold off
    end
    xlabel('v_x [mm/s]')
    ...
    for j = 1:(nr_steps+1)
        ...
        plot(vx_forward_range,Fx_forward_tot(3,:,j),'Color',[ (0.5-(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) (0.5+(0.5*(j-1))/nr_steps) ])
        plot(vx_back_range,Fx_back_tot(3,:,j),'Color',[ (0.5-(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) (0.5+(0.5*(j-1))/nr_steps) ])
        hold off
        hold off
    end
    xlabel('v_x [mm/s]')
    ...
    for j = 1:(nr_steps+1)
        ...
        plot(vx_forward_range,Fx_forward_A(1,:,j),'Color',[ (0.5+(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) ])
        plot(vx_back_range,Fx_back_A(1,:,j),'Color',[ (0.5+(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) ])
        hold off
        hold off
    end
    xlabel('v_x [mm/s]')
    ...
    for j = 1:(nr_steps+1)
        ...
        plot(vx_forward_range,Fx_forward_A(2,:,j),'Color',[ (0.5-(0.5*(j-1))/nr_steps) (0.5+(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) ])
        plot(vx_back_range,Fx_back_A(2,:,j),'Color',[ (0.5-(0.5*(j-1))/nr_steps) (0.5+(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) ])
        hold off
        hold off
    end
    xlabel('v_x [mm/s]')
    ...
    for j = 1:(nr_steps+1)
        ...
        plot(vx_forward_range,Fx_forward_A(3,:,j),'Color',[ (0.5-(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) (0.5+(0.5*(j-1))/nr_steps) ])
        plot(vx_back_range,Fx_back_A(3,:,j),'Color',[ (0.5-(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) (0.5+(0.5*(j-1))/nr_steps) ])
        hold off
        hold off
    end
    xlabel('v_x [mm/s]')
    ...
    for j = 1:(nr_steps+1)
        ...
        plot(vx_forward_range,Fx_forward_I(1,:,j),'Color',[ (0.5+(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) ])
        plot(vx_back_range,Fx_back_I(1,:,j),'Color',[ (0.5+(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) ])
        hold off
        hold off
    end
    xlabel('v_x [mm/s]')
    ...
    for j = 1:(nr_steps+1)
        ...
        plot(vx_forward_range,Fx_forward_I(2,:,j),'Color',[ (0.5-(0.5*(j-1))/nr_steps) (0.5+(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) ])
        plot(vx_back_range,Fx_back_I(2,:,j),'Color',[ (0.5-(0.5*(j-1))/nr_steps) (0.5+(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) ])
        hold off
        hold off
    end
    xlabel('v_x [mm/s]')
    ...
    for j = 1:(nr_steps+1)
        ...
        plot(vx_forward_range,Fx_forward_I(3,:,j),'Color',[ (0.5-(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) (0.5+(0.5*(j-1))/nr_steps) ])
        plot(vx_back_range,Fx_back_I(3,:,j),'Color',[ (0.5-(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) (0.5+(0.5*(j-1))/nr_steps) ])
        hold off
        hold off
    end
    xlabel('v_x [mm/s]')
    ylabel('F_z [N]')
    hold off
    [~,h1] = suplabel('Damping forces v_x', 't');
    set(h1,'FontSize',10)
    print ([char(settings.plot_loc) '/Damping/Damp_vx.eps'] ,'-depsc2'); 
    ...
    for j = 1:(nr_steps+1)
        hold on
        subplot(3,3,1); hold on
        plot(vy_range,Fy_tot(1,:,j),'Color',[ (0.5+(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) ])
        hold off
        hold off
    end
    xlabel('v_y [mm/s]')
    ...
    for j = 1:(nr_steps+1)
        hold on
        subplot(3,3,4); hold on
        plot(vy_range,Fy_tot(2,:,j),'Color',[ (0.5-(0.5*(j-1))/nr_steps) (0.5+(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) ])
        hold off
        hold off
    end
    xlabel('v_y [mm/s]')
    ...
    for j = 1:(nr_steps+1)
        hold on
        subplot(3,3,7); hold on
        plot(vy_range,Fy_tot(3,:,j),'Color',[ (0.5-(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) (0.5+(0.5*(j-1))/nr_steps) ])
        hold off
        hold off
    end
    xlabel('v_y [mm/s]')
    ...
    for j = 1:(nr_steps+1)
        hold on
        subplot(3,3,2); hold on
        plot(vy_range,Fy_A(1,:,j),'Color',[ (0.5+(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) ])
        hold off
        hold off
    end
    xlabel('v_y [mm/s]')
    ...
    for j = 1:(nr_steps+1)
        hold on
        subplot(3,3,5); hold on
        plot(vy_range,Fy_A(2,:,j),'Color',[ (0.5-(0.5*(j-1))/nr_steps) (0.5+(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) ])
        hold off
        hold off
    end
    xlabel('v_y [mm/s]')
    ...
    for j = 1:(nr_steps+1)
        hold on
        subplot(3,3,8); hold on
        plot(vy_range,Fy_A(3,:,j),'Color',[ (0.5-(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) (0.5+(0.5*(j-1))/nr_steps) ])
        hold off
        hold off
    end
    xlabel('v_y [mm/s]')
    ...
    for j = 1:(nr_steps+1)
        hold on
        subplot(3,3,3); hold on
        plot(vy_range,Fy_I(1,:,j),'Color',[ (0.5+(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) ])
        hold off
        hold off
    end
    xlabel('v_y [mm/s]')
    ...
    for j = 1:(nr_steps+1)
        hold on
        subplot(3,3,6); hold on
        plot(vy_range,Fy_I(2,:,j),'Color',[ (0.5-(0.5*(j-1))/nr_steps) (0.5+(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) ])
        hold off
        hold off
    end
    xlabel('v_y [mm/s]')
    ...
    for j = 1:(nr_steps+1)
        hold on
        subplot(3,3,9); hold on
        plot(vy_range,Fy_I(3,:,j),'Color',[ (0.5-(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) (0.5+(0.5*(j-1))/nr_steps) ])
        hold off
        hold off
    end
    xlabel('v_y [mm/s]')
    ylabel('F_z [N]')
    hold off
    [~,h1] = suplabel('Damping forces v_y', 't');
    set(h1,'FontSize',10)
    print ([char(settings.plot_loc) '/Damping/Damp_vy.eps'] ,'-depsc2'); 
    ...
    for j = 1:(nr_steps+1)
        hold on
        subplot(3,3,1); hold on
        plot(vz_up_range,Fz_up_tot(1,:,j),'Color',[ (0.5+(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) ])
        hold off
        hold off
    end
    xlabel('v_z [mm/s]')
    ...
    for j = 1:(nr_steps+1)
        hold on
        subplot(3,3,4); hold on
        plot(vz_up_range,Fz_up_tot(2,:,j),'Color',[ (0.5-(0.5*(j-1))/nr_steps) (0.5+(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) ])
        hold off
        hold off
    end
    xlabel('v_z [mm/s]')
    ...
    for j = 1:(nr_steps+1)
        hold on
        subplot(3,3,7); hold on
        plot(vz_up_range,Fz_up_tot(3,:,j),'Color',[ (0.5-(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) (0.5+(0.5*(j-1))/nr_steps) ])
        hold off
        hold off
    end
    xlabel('v_z [mm/s]')
    ...
    for j = 1:(nr_steps+1)
        hold on
        subplot(3,3,2); hold on
        plot(vz_up_range,Fz_up_A(1,:,j),'Color',[ (0.5+(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) ])
        hold off
        hold off
    end
    xlabel('v_z [mm/s]')
    ...
    for j = 1:(nr_steps+1)
        hold on
        subplot(3,3,5); hold on
        plot(vz_up_range,Fz_up_A(2,:,j),'Color',[ (0.5-(0.5*(j-1))/nr_steps) (0.5+(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) ])
        hold off
        hold off
    end
    xlabel('v_z [mm/s]')
    ...
    for j = 1:(nr_steps+1)
        hold on
        subplot(3,3,8); hold on
        plot(vz_up_range,Fz_up_A(3,:,j),'Color',[ (0.5-(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) (0.5+(0.5*(j-1))/nr_steps) ])
        hold off
        hold off
    end
    xlabel('v_z [mm/s]')
    ...
    for j = 1:(nr_steps+1)
        hold on
        subplot(3,3,3); hold on
        plot(vz_up_range,Fz_up_I(1,:,j),'Color',[ (0.5+(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) ])
        hold off
        hold off
    end
    xlabel('v_z [mm/s]')
    ...
    for j = 1:(nr_steps+1)
        hold on
        subplot(3,3,6); hold on
        plot(vz_up_range,Fz_up_I(2,:,j),'Color',[ (0.5-(0.5*(j-1))/nr_steps) (0.5+(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) ])
        hold off
        hold off
    end
    xlabel('v_z [mm/s]')
    ...
    for j = 1:(nr_steps+1)
        hold on
        subplot(3,3,9); hold on
        plot(vz_up_range,Fz_up_I(3,:,j),'Color',[ (0.5-(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) (0.5+(0.5*(j-1))/nr_steps) ])
        hold off
        hold off
    end
    xlabel('v_z [mm/s]')
    ylabel('F_z [N]')
    hold off
    [~,h1] = suplabel('Damping forces v_z', 't');
    set(h1,'FontSize',10)
    print ([char(settings.plot_loc) '/Damping/Damp_vz.eps'] ,'-depsc2'); 
    ...
    for j = 1:(nr_steps+1)
        hold on
        subplot(3,3,1); hold on
        plot(wx_range,Mx_tot(1,:,j),'Color',[ (0.5+(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) ])
        hold off
        hold off
    end
    xlabel('w_x [rad/s]')
    ...
    for j = 1:(nr_steps+1)
        hold on
        subplot(3,3,4); hold on
        plot(wx_range,Mx_tot(2,:,j),'Color',[ (0.5-(0.5*(j-1))/nr_steps) (0.5+(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) ])
        hold off
        hold off
    end
    xlabel('w_x [rad/s]')
    ...
    for j = 1:(nr_steps+1)
        hold on
        subplot(3,3,7); hold on
        plot(wx_range,Mx_tot(3,:,j),'Color',[ (0.5-(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) (0.5+(0.5*(j-1))/nr_steps) ])
        hold off
        hold off
    end
    xlabel('w_x [rad/s]')
    ...
    for j = 1:(nr_steps+1)
        hold on
        subplot(3,3,2); hold on
        plot(wx_range,Mx_A(1,:,j),'Color',[ (0.5+(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) ])
        hold off
        hold off
    end
    xlabel('w_x [rad/s]')
    ...
    for j = 1:(nr_steps+1)
        hold on
        subplot(3,3,5); hold on
        plot(wx_range,Mx_A(2,:,j),'Color',[ (0.5-(0.5*(j-1))/nr_steps) (0.5+(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) ])
        hold off
        hold off
    end
    xlabel('w_x [rad/s]')
    ...
    for j = 1:(nr_steps+1)
        hold on
        subplot(3,3,8); hold on
        plot(wx_range,Mx_A(3,:,j),'Color',[ (0.5-(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) (0.5+(0.5*(j-1))/nr_steps) ])
        hold off
        hold off
    end
    xlabel('w_x [rad/s]')
    ...
    for j = 1:(nr_steps+1)
        hold on
        subplot(3,3,3); hold on
        plot(wx_range,Mx_I(1,:,j),'Color',[ (0.5+(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) ])
        hold off
        hold off
    end
    xlabel('w_x [rad/s]')
    ...
    for j = 1:(nr_steps+1)
        hold on
        subplot(3,3,6); hold on
        plot(wx_range,Mx_I(2,:,j),'Color',[ (0.5-(0.5*(j-1))/nr_steps) (0.5+(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) ])
        hold off
        hold off
    end
    xlabel('w_x [rad/s]')
    ...
    for j = 1:(nr_steps+1)
        hold on
        subplot(3,3,9); hold on
        plot(wx_range,Mx_I(3,:,j),'Color',[ (0.5-(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) (0.5+(0.5*(j-1))/nr_steps) ])
        hold off
        hold off
    end
    xlabel('w_x [rad/s]')
    ylabel('M_z [N*mm]')
    hold off    
    [~,h1] = suplabel('Damping moments w_x', 't');
    set(h1,'FontSize',10)
    print ([char(settings.plot_loc) '/Damping/Damp_wx.eps'] ,'-depsc2'); 
    ...
    for j = 1:(nr_steps+1)
        hold on
        subplot(3,3,1); hold on
        plot(wy_down_range,My_down_tot(1,:,j),'Color',[ (0.5+(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) ])
        plot(wy_up_range,My_up_tot(1,:,j),'Color',[ (0.5+(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) ])
        hold off
        hold off
    end
    xlabel('w_y [rad/s]')
    ...
    for j = 1:(nr_steps+1)
        hold on
        subplot(3,3,4); hold on
        plot(wy_down_range,My_down_tot(2,:,j),'Color',[ (0.5-(0.5*(j-1))/nr_steps) (0.5+(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) ])
        plot(wy_up_range,My_up_tot(2,:,j),'Color',[ (0.5-(0.5*(j-1))/nr_steps) (0.5+(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) ])
        hold off
        hold off
    end
    xlabel('w_y [rad/s]')
    ...
    for j = 1:(nr_steps+1)
        hold on
        subplot(3,3,7); hold on
        plot(wy_down_range,My_down_tot(3,:,j),'Color',[ (0.5-(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) (0.5+(0.5*(j-1))/nr_steps) ])
        plot(wy_up_range,My_up_tot(3,:,j),'Color',[ (0.5-(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) (0.5+(0.5*(j-1))/nr_steps) ])
        hold off
        hold off
    end
    xlabel('w_y [rad/s]')
    ...
    for j = 1:(nr_steps+1)
        hold on
        subplot(3,3,2); hold on
        plot(wy_down_range,My_down_A(1,:,j),'Color',[ (0.5+(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) ])
        plot(wy_up_range,My_up_A(1,:,j),'Color',[ (0.5+(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) ])
        hold off
        hold off
    end
    xlabel('w_y [rad/s]')
    ...
    for j = 1:(nr_steps+1)
        hold on
        subplot(3,3,5); hold on
        plot(wy_down_range,My_down_A(2,:,j),'Color',[ (0.5-(0.5*(j-1))/nr_steps) (0.5+(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) ])
        plot(wy_up_range,My_up_A(2,:,j),'Color',[ (0.5-(0.5*(j-1))/nr_steps) (0.5+(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) ])
        hold off
        hold off
    end
    xlabel('w_y [rad/s]')
    ...
    for j = 1:(nr_steps+1)
        hold on
        subplot(3,3,8); hold on
        plot(wy_down_range,My_down_A(3,:,j),'Color',[ (0.5-(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) (0.5+(0.5*(j-1))/nr_steps) ])
        plot(wy_up_range,My_up_A(3,:,j),'Color',[ (0.5-(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) (0.5+(0.5*(j-1))/nr_steps) ])
        hold off
        hold off
    end
    xlabel('w_y [rad/s]')
    ...
    for j = 1:(nr_steps+1)
        hold on
        subplot(3,3,3); hold on
        plot(wy_down_range,My_down_I(1,:,j),'Color',[ (0.5+(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) ])
        plot(wy_up_range,My_up_I(1,:,j),'Color',[ (0.5+(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) ])
        hold off
        hold off
    end
    xlabel('w_y [rad/s]')
    ...
    for j = 1:(nr_steps+1)
        hold on
        subplot(3,3,6); hold on
        plot(wy_down_range,My_down_I(2,:,j),'Color',[ (0.5-(0.5*(j-1))/nr_steps) (0.5+(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) ])
        hold off
        hold off
    end
    xlabel('w_y [rad/s]')
    ...
    for j = 1:(nr_steps+1)
        hold on
        subplot(3,3,9); hold on
        plot(wy_down_range,My_down_I(3,:,j),'Color',[ (0.5-(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) (0.5+(0.5*(j-1))/nr_steps) ])
        plot(wy_up_range,My_up_I(3,:,j),'Color',[ (0.5-(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) (0.5+(0.5*(j-1))/nr_steps) ])
        hold off
        hold off
    end
    xlabel('w_y [rad/s]')
    ylabel('M_z [N*mm]')
    hold off    
    [~,h1] = suplabel('Damping moments w_y', 't');
    set(h1,'FontSize',10)
    print ([char(settings.plot_loc) '/Damping/Damp_wy.eps'] ,'-depsc2'); 
    ...
    for j = 1:(nr_steps+1)
        hold on
        subplot(3,3,1); hold on
        plot(wz_range,Mz_tot(1,:,j),'Color',[ (0.5+(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) ])
        hold off
        hold off
    end
    xlabel('w_z [rad/s]')
    ...
    for j = 1:(nr_steps+1)
        hold on
        subplot(3,3,4); hold on
        plot(wz_range,Mz_tot(2,:,j),'Color',[ (0.5-(0.5*(j-1))/nr_steps) (0.5+(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) ])
        hold off
        hold off
    end
    xlabel('w_z [rad/s]')
    ...
    for j = 1:(nr_steps+1)
        hold on
        subplot(3,3,7); hold on
        plot(wz_range,Mz_tot(3,:,j),'Color',[ (0.5-(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) (0.5+(0.5*(j-1))/nr_steps) ])
        hold off
        hold off
    end
    xlabel('w_z [rad/s]')
    ...
    for j = 1:(nr_steps+1)
        hold on
        subplot(3,3,2); hold on
        plot(wz_range,Mz_A(1,:,j),'Color',[ (0.5+(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) ])
        hold off
        hold off
    end
    xlabel('w_z [rad/s]')
    ...
    for j = 1:(nr_steps+1)
        hold on
        subplot(3,3,5); hold on
        plot(wz_range,Mz_A(2,:,j),'Color',[ (0.5-(0.5*(j-1))/nr_steps) (0.5+(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) ])
        hold off
        hold off
    end
    xlabel('w_z [rad/s]')
    ...
    for j = 1:(nr_steps+1)
        hold on
        subplot(3,3,8); hold on
        plot(wz_range,Mz_A(3,:,j),'Color',[ (0.5-(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) (0.5+(0.5*(j-1))/nr_steps) ])
        hold off
        hold off
    end
    xlabel('w_z [rad/s]')
    ...
    for j = 1:(nr_steps+1)
        hold on
        subplot(3,3,3); hold on
        plot(wz_range,Mz_I(1,:,j),'Color',[ (0.5+(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) ])
        hold off
        hold off
    end
    xlabel('w_z [rad/s]')
    ...
    for j = 1:(nr_steps+1)
        hold on
        subplot(3,3,6); hold on
        plot(wz_range,Mz_I(2,:,j),'Color',[ (0.5-(0.5*(j-1))/nr_steps) (0.5+(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) ])
        hold off
        hold off
    end
    xlabel('w_z [rad/s]')
    ...
    for j = 1:(nr_steps+1)
        hold on
        subplot(3,3,9); hold on
        plot(wz_range,Mz_I(3,:,j),'Color',[ (0.5-(0.5*(j-1))/nr_steps) (0.5-(0.5*(j-1))/nr_steps) (0.5+(0.5*(j-1))/nr_steps) ])
        hold off
        hold off
    end
    xlabel('w_z [rad/s]')
    ylabel('M_z [N*mm]')
    hold off     
    [~,h1] = suplabel('Damping moments w_z', 't');
    set(h1,'FontSize',10)
    print ([char(settings.plot_loc) '/Damping/Damp_wz.eps'] ,'-depsc2'); 
