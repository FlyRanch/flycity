function error_plot( x_strk_sim_steady, x_strk_mov_steady, x_strk_sim_man, x_strk_mov_man, man_id, i, plot_type, n_bins)
...
%     end
    ...
    err_angle_man(j)     = norm(radtodeg(acos(norm(dot(x_strk_sim_man(:,j),x_strk_mov_man(:,j)))/(norm(x_strk_sim_man(:,j))*norm(x_strk_mov_man(:,j))))));
...
%     n_bins = 9;
    ...
    hold on
    plot(x_strk_mov_steady(i,:),err_vect_steady,'o','MarkerSize',1.5,'MarkerEdgeColor', 'r')
    plot(x_strk_mov_man(i,:),err_vect_man,'o','MarkerSize',1.5,'MarkerEdgeColor', [0.5 0.5 0.5])
    plot(bin_loc,bin_mean_vect,'b')
    errorbar(bin_loc,bin_mean_vect,bin_std_vect,'b')
    hold off
    
    elseif plot_type == 1
        
    hold on
    plot(x_strk_mov_steady(i,:),err_angle_steady,'o','MarkerSize',1.5,'MarkerEdgeColor', 'r')
    plot(x_strk_mov_man(i,:),err_angle_man,'o','MarkerSize',1.5,'MarkerEdgeColor', [0.5 0.5 0.5])
    plot(bin_loc,bin_mean_angle,'b')
    errorbar(bin_loc,bin_mean_angle,bin_std_angle,'b')
    hold off
