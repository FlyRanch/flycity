function [a_fit, a_avg] = Fitting_Florian( settings, pathDB)
    ...
    % Create raw data:
        ...
        for j = 1:nr_wb_loc
            ...
            wb_loc_end = find(isnan(pathDB.wingbeat_time(j,:,seq_nr))==0, 1, 'last' );
            
            u = pathDB.wingbeat_time(j,1:wb_loc_end,seq_nr);
            
            raw.theta_L = theta_L_loc(u);

            raw.theta_R = theta_R_loc(u);

            raw.eta_L = eta_L_loc(u);

            raw.eta_R = eta_R_loc(u);

            raw.phi_L = phi_L_loc(u);

            raw.phi_R = phi_R_loc(u);
            
            raw.wb_loc = [ 1 wb_loc_end]; 
            ...
            raw.theta_L = [raw.theta_L; theta_L_loc(u)];

            raw.theta_R = [raw.theta_R; theta_R_loc(u)];

            raw.eta_L = [raw.eta_L; eta_L_loc(u)];

            raw.eta_R = [raw.eta_R; eta_R_loc(u)];

            raw.phi_L = [raw.phi_L; phi_L_loc(u)];

            raw.phi_R = [raw.phi_R; phi_R_loc(u)];
            
            raw.wb_loc = [ raw.wb_loc;  raw.wb_loc(end,2)+1 raw.wb_loc(end,2)+wb_loc_end];
...
%             phi_R = [phi_R; phi_R_t];
    ...
    for k = 1:(length(a_fit.wb1_wb2(:,1)))
        ...
        a_avg.ratio_1_2 = mean(a_fit.ratio_1_2);
...
%         ratio_1_2_avg = mean(ratio_1_2(3:trigger_wb));
    ...
    for k = 1:nr_raw_wb
        ...
        t1 = -1:((m1/m2)/200):(-1+(m1/m2));
        t2 = (-1+(m1/m2)):((2-(m1/m2))/200):1;
        plot(t1,radtodeg(PN_phi(:,:,1)'*a_fit.phi_L1(k,:)'),'Color',[0.5 0.5 0.5])
        plot(t2,radtodeg(PN_phi(:,:,1)'*a_fit.phi_L2(k,:)'),'Color',[0.5 0.5 0.5])
    end
    m1 =a_avg.wb1_wb2(1);
    m2 = a_avg.wb1_wb2(2);
    t1 = -1:((m1/m2)/200):(-1+(m1/m2));
    ...
    hold on
    for k = 1:nr_raw_wb
        m = raw.wb_loc(k,2)-raw.wb_loc(k,1)+1;
        t = -1:2/(m-1):1;
        plot(t,radtodeg(raw.theta_L(raw.wb_loc(k,1):raw.wb_loc(k,2))),'Color',[0.5 0.5 0.5])
    end
    m1 =a_avg.wb1_wb2(1);
    m2 = a_avg.wb1_wb2(2);
    [ t, X ] = Wingbeat_Legendre_matrix( n_pol_theta, a_avg.ratio_1_2, 200, -1, 1, 0 );
    plot(t,radtodeg(X*[a_avg.theta_L1; a_avg.theta_L2]),'r')
    hold off
    ...
    for k = 1:nr_raw_wb
        m = raw.wb_loc(k,2)-raw.wb_loc(k,1)+1;
        t = -1:2/(m-1):1;
        plot(t,radtodeg(raw.eta_L(raw.wb_loc(k,1):raw.wb_loc(k,2))),'Color',[0.5 0.5 0.5])        
    end
    m1 =a_avg.wb1_wb2(1);
    m2 = a_avg.wb1_wb2(2);
    [ t, X ] = Wingbeat_Legendre_matrix( n_pol_eta, a_avg.ratio_1_2, 200, -1, 1, 0 );
    plot(t,radtodeg(X*[a_avg.eta_L1; a_avg.eta_L2]),'r')   
    hold off
    ...
    for k = 1:nr_raw_wb
        m = raw.wb_loc(k,2)-raw.wb_loc(k,1)+1;
        t = -1:2/(m-1):1;
        plot(t,radtodeg(raw.phi_L(raw.wb_loc(k,1):raw.wb_loc(k,2))),'Color',[0.5 0.5 0.5])        
    end
    m1 =a_avg.wb1_wb2(1);
    m2 = a_avg.wb1_wb2(2);
    [ t, X ] = Wingbeat_Legendre_matrix( n_pol_phi, a_avg.ratio_1_2, 200, -1, 1, 0 );
    plot(t,radtodeg(X*[a_avg.phi_L1; a_avg.phi_L2]),'r')
    hold off
    ...
    for k = 1:nr_wb
        m1 =a_fit.wb1_wb2(k,1);
        m2 = a_fit.wb1_wb2(k,2);
        t1 = -1:(2*(m1/(m1+m2))/200):(-1+2*(m1/(m1+m2)));
        t2 = (-1+2*(m1/(m1+m2))):((2*(m2/(m1+m2)))/200):1;
        plot(t1,radtodeg(PN_theta(:,:,1)'*a_fit.theta_L1(k,:)'),'g')
        plot(t2,radtodeg(PN_theta(:,:,1)'*a_fit.theta_L2(k,:)'),'g')
    end
    for k = 1:nr_raw_wb
        m = raw.wb_loc(k,2)-raw.wb_loc(k,1)+1;
        t = -1:2/(m-1):1;
        plot(t,radtodeg(raw.theta_L(raw.wb_loc(k,1):raw.wb_loc(k,2))),'r')        
    end 
    hold off
