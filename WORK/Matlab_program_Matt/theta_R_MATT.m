...
%                             figure(2)
%                             hold on
%                             x = linspace(0,1,length(pathDB.theta_R(start(j,i):stop(j,i))));
%                             y = pathDB.theta_R(start(j,i):stop(j,i),i);
...
%                             hold on
%                             x = linspace(0,1,length(pathDB.theta_R(start(j,i):stop(j,i))));
%                             y = rad2deg(pathDB.theta_R(start(j,i):stop(j,i),i));
% 
% %                             plot(x,y,'color', color(i,1:3))
%                             plot(x,y,'color', 'blue')
%                             ylabel('stroke dev - right wing [deg]')
%                        end 
%                     end   
%                 end                         
%             end  
...
for i = seqs_plot % individuals
    for j = 1:length(stop(:,i))-1 % wingbeats
        ...
        if A > 0 && B > 0 && C > 0   
            x = linspace(0,0.5,length(pathDB.theta_R(A:B)));
            x2 = linspace(0.5,1,length(pathDB.theta_R(B:C)));
            y = rad2deg(pathDB.theta_R(A:B,i));
            y2 = rad2deg(pathDB.theta_R(B:C,i));
        ...
        if C > 0 && D > 0
            ...
            E = linspace(0,1.5,length(pathDB.theta_R(C:D)));
            F = rad2deg(pathDB.theta_R(C:D,i));
