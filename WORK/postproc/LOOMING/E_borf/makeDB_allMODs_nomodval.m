...
%     load(list(i).name)
    ...
    Fx_mean_all(i,1) = nanmean(Fx_now(n_start:n_stop));
    Fy_mean_all(i,1) = nanmean(Fy_now(n_start:n_stop));
    Fz_mean_all(i,1) = nanmean(Fz_now(n_start:n_stop));
    
    Mx_mean_all(i,1) = nanmean(Mx_now(n_start:n_stop));
    My_mean_all(i,1) = nanmean(My_now(n_start:n_stop));
    Mz_mean_all(i,1) = nanmean(Mz_now(n_start:n_stop));
    
    Astroke_all(i,1) = max([stroke_R_now;stroke_L_now]) - min([stroke_R_now;stroke_L_now]);
    Apitch_all(i,1) = max([pitch_R_now;pitch_L_now]) - min([pitch_R_now;pitch_L_now]);
    Adev_all(i,1) = max([dev_R_now;dev_L_now]) - min([dev_R_now;dev_L_now]);
    
    %% store all data
    t_all(1:length(t),i) = t_now;
    
    Fx_all(1:length(t),i) = Fx_now;
    Fy_all(1:length(t),i) = Fy_now;
    Fz_all(1:length(t),i) = Fz_now;
    
    Mx_all(1:length(t),i) = Mx_now;
    My_all(1:length(t),i) = My_now;
    Mz_all(1:length(t),i) = Mz_now;
    
    stroke_L_all(1:length(t),i) = stroke_L_now;
    pitch_L_all(1:length(t),i) = pitch_L_now;
    dev_L_all(1:length(t),i) = dev_L_now;
    
    stroke_R_all(1:length(t),i) = stroke_R_now;
    pitch_R_all(1:length(t),i) = pitch_R_now;
    dev_R_all(1:length(t),i) = dev_R_now;
