...
mkdir(['RollAccel_mov_figs'])
cd(['RollAccel_mov_figs'])
...
for j = 1:length(steps)
    ...
    hold off
        ...
        h=quiver(x_wing,y_wing,Fx_tip,Fy_tip,'linewidth',2,'color',F_map(1,:));
        adjust_quiver_arrowhead_size(h, 3);
        hold on
        plot([x_LE x_TE],[y_LE y_TE],'-','linewidth',3,'color',cord_map(1,:))
        plot([x_LE],[y_LE],'o','markersize',8,'markerfacecolor',cord_map(1,:),'color',cord_map(1,:))
        ...
        h=quiver(x_wing,y_wing,Fx_tip,Fy_tip,'linewidth',2,'color',F_map(2,:));
        adjust_quiver_arrowhead_size(h, 3);
        hold on
        plot([x_LE x_TE],[y_LE y_TE],'-','linewidth',3,'color',cord_map(2,:))
        plot([x_LE],[y_LE],'o','markersize',8,'markerfacecolor',cord_map(2,:),'color',cord_map(2,:))
    ...
    saveas(gca, ['profNforce_RollAccel_',num2str(j),'.tif'])
end
cd ..
