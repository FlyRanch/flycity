function [ kine ] = angular_velocities( a_fit )
    ...
    b1_theta_L = -mean(theta_ddot_L_t(1:(nr_points-1)));
    b1_eta_L = -mean(eta_ddot_L_t(1:(nr_points-1)));
    b1_phi_L = -mean(phi_ddot_L_t(1:(nr_points-1)));
    
    b1_theta_R = -mean(theta_ddot_R_t(1:(nr_points-1)));
    b1_eta_R = -mean(eta_ddot_R_t(1:(nr_points-1)));
    b1_phi_R = -mean(phi_ddot_R_t(1:(nr_points-1)));
    
    b2_theta_L = -mean(theta_dot_L_t(1:(nr_points-1)));
    b2_eta_L = -mean(eta_dot_L_t(1:(nr_points-1)));
    b2_phi_L = -mean(phi_dot_L_t(1:(nr_points-1)));
    
    b2_theta_R = -mean(theta_dot_R_t(1:(nr_points-1)));
    b2_eta_R = -mean(eta_dot_R_t(1:(nr_points-1)));
    b2_phi_R = -mean(phi_dot_R_t(1:(nr_points-1)));
...
%     theta_L         = theta_L_t+b2_theta_L*(0.5*t)';
%     eta_L           = eta_L_t+b2_eta_L*(0.5*t)';
%     phi_L           = phi_L_t+b2_phi_L*(0.5*t)';
%     
%     theta_R         = theta_R_t+b2_theta_R*(0.5*t)';
%     eta_R           = eta_R_t+b2_eta_R*(0.5*t)';
%     phi_R           = phi_R_t+b2_phi_R*(0.5*t)';
%     
...
%     theta_L         = theta_L_t-(1/(f^2))*mean(theta_ddot_L_t(1:(nr_points-1)));
%     eta_L           = eta_L_t-(1/(f^2))*mean(eta_ddot_L_t(1:(nr_points-1)));
%     phi_L           = phi_L_t-(1/(f^2))*mean(phi_ddot_L_t(1:(nr_points-1)));
%     
%     theta_R         = theta_R_t-(1/(f^2))*mean(theta_ddot_R_t(1:(nr_points-1)));
%     eta_R           = eta_R_t-(1/(f^2))*mean(eta_ddot_R_t(1:(nr_points-1)));
%     phi_R           = phi_R_t-(1/(f&2))*mean(phi_ddot_R_t(1:(nr_points-1)));
...
%     theta_dot_L     = theta_dot_L_t-mean(theta_dot_L_t(1:(nr_points-1)));
%     eta_dot_L       = eta_dot_L_t-mean(eta_dot_L_t(1:(nr_points-1)));
%     phi_dot_L       = phi_dot_L_t-mean(phi_dot_L_t(1:(nr_points-1)));
%     
%     theta_dot_R     = theta_dot_R_t-mean(theta_dot_R_t(1:(nr_points-1)));
%     eta_dot_R       = eta_dot_R_t-mean(eta_dot_R_t(1:(nr_points-1)));
%     phi_dot_R       = phi_dot_R_t-mean(phi_dot_R_t(1:(nr_points-1)));
%     
%     theta_ddot_L    = theta_ddot_L_t-mean(theta_ddot_L_t(1:(nr_points-1)));
%     eta_ddot_L      = eta_ddot_L_t-mean(eta_ddot_L_t(1:(nr_points-1)));
%     phi_ddot_L      = phi_ddot_L_t-mean(phi_ddot_L_t(1:(nr_points-1)));
% 
%     theta_ddot_R    = theta_ddot_R_t-mean(theta_ddot_R_t(1:(nr_points-1)));
%     eta_ddot_R      = eta_ddot_R_t-mean(eta_ddot_R_t(1:(nr_points-1)));
%     phi_ddot_R      = phi_ddot_R_t-mean(phi_ddot_R_t(1:(nr_points-1)));
...
%     theta_dot_L     = d_theta_L_dt;
%     eta_dot_L       = d_eta_L_dt;
%     phi_dot_L       = d_phi_L_dt;
%     
%     theta_dot_R     = d_theta_R_dt;
%     eta_dot_R       = d_eta_R_dt;
%     phi_dot_R       = d_phi_R_dt;
%     
%     theta_ddot_L    = d2_theta_L_dt2;
%     eta_ddot_L      = d2_eta_L_dt2;
%     phi_ddot_L      = d2_phi_L_dt2;
% 
%     theta_ddot_R    = d2_theta_R_dt2;
%     eta_ddot_R      = d2_eta_R_dt2;
%     phi_ddot_R      = d2_phi_R_dt2; 
    
    

%     figure()
%     hold on
%     subplot(3,1,1); plot(t,theta_dot_L,'r',t,d_theta_L_dt,'b')
%     subplot(3,1,2); plot(t,eta_dot_L,'r',t,d_eta_L_dt,'b')
%     subplot(3,1,3); plot(t,phi_dot_L,'r',t,d_phi_L_dt,'b')
%     hold off
%     
%     figure()
%     hold on
%     subplot(3,1,1); plot(t,theta_dot_R,'r',t,d_theta_R_dt,'b')
%     subplot(3,1,2); plot(t,eta_dot_R,'r',t,d_eta_R_dt,'b')
%     subplot(3,1,3); plot(t,phi_dot_R,'r',t,d_phi_R_dt,'b')
%     hold off
%     
%     figure()
%     hold on
%     subplot(3,1,1); plot(t,theta_ddot_L,'r',t,d2_theta_L_dt2,'b')
%     subplot(3,1,2); plot(t,eta_ddot_L,'r',t,d2_eta_L_dt2,'b')
%     subplot(3,1,3); plot(t,phi_ddot_L,'r',t,d2_phi_L_dt2,'b')
%     hold off
%     
%     figure()
%     hold on
%     subplot(3,1,1); plot(t,theta_ddot_R,'r',t,d2_theta_R_dt2,'b')
%     subplot(3,1,2); plot(t,eta_ddot_R,'r',t,d2_eta_R_dt2,'b')
%     subplot(3,1,3); plot(t,phi_ddot_R,'r',t,d2_phi_R_dt2,'b')
%     hold off
...
%                                 0  0 -1];
    ...
    for i = 1:nr_points
        ...
        wL(:,i)             = wL(:,i)-w_dot_L_mean*(1/f);
        wR(:,i)             = wR(:,i)-w_dot_R_mean*(1/f);
    ...
    for i = 1:nr_points
        ...
        wL_b(:,i)      = wL_b(:,i)-w_dot_L_b_mean*(1/f);
        wR_b(:,i)      = wR_b(:,i)-w_dot_R_b_mean*(1/f);
        wL_strk(:,i)   = wL_strk(:,i)-w_dot_L_strk_mean*(1/f);
        wR_strk(:,i)   = wR_strk(:,i)-w_dot_R_strk_mean*(1/f);
