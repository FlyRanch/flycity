...
movie_step = 15;
fps = 7500/15;
movfilename_top = ['3Dflightpaths_topview_fast_NOrevexp_',num2str(fps),'fps.avi'];
movfilename_4views = ['3Dflightpaths_4views_fast_NOrevexp_',num2str(fps),'fps.avi'];
...
for i = 1:seqs
    if settings.expansion.speed(i) == 1 && settings.expansion.HorPos(i) == 0
        j = j +1;
...
%                     teta_exp = teta_exp + 90;
                    ...
                    if exp_frame>1 && exp_steps(exp_frame,1) < exp_steps(exp_frame-1,1)
                        plot(x_ring,y_ring,'-g','linewidth',5)
                    end
