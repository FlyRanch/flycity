...
stim_angle_vel_Ahormax = nan;
stim_angle_accel_Ahormax = nan;
stim_angle_yaw_Ahormax = nan;

accel_angle_hor_vel_Ahormax = nan;
accel_angle_hor_body_Ahormax = nan;
...
stim_angle_vel_Amax = nan;
stim_angle_accel_Amax = nan;
stim_angle_yaw_Amax = nan;

accel_angle_hor_vel_Amax = nan;
accel_angle_hor_body_Amax = nan;
...
F_hor_Ahormax = nan;
Fsp_pitch_Ahormax = nan;
Fsp_roll_Ahormax = nan;
...
accel_angle_hor_vel_pre_resp = nan;
accel_angle_hor_vel_pre_turn = nan;
accel_angle_hor_vel_pre_accel = nan;

accel_angle_hor_vel_post_resp = nan;
accel_angle_hor_vel_post_turn = nan;
accel_angle_hor_vel_post_accel = nan;

accel_angle_hor_body_pre_resp = nan;
accel_angle_hor_body_pre_turn = nan;
accel_angle_hor_body_pre_accel = nan;

accel_angle_hor_body_post_resp = nan;
accel_angle_hor_body_post_turn = nan;
accel_angle_hor_body_post_accel = nan;

accel_angle_hor_vel_resp_mean = nan;
accel_angle_hor_body_resp_mean = nan;
...
accel_angle_hor_vel = pathDB.accel_angle_hor_vel(start_frame:end,i,1);
accel_angle_hor_body = pathDB.accel_angle_hor_body(start_frame:end,i,1);
...
F = pathDB.F(start_frame:end,i);
F_hor = pathDB.F_hor(start_frame:end,i);
Fsp_pitch = pathDB.Fsp_pitch(start_frame:end,i);
Fsp_roll = pathDB.Fsp_roll(start_frame:end,i);
...
if IDX_shift(1) == 5 && length(IDX_shift) > 1
    ...
    stim_angle_vel_pre_resp = (stim_angle_vel(n_resp));
    stim_angle_accel_pre_resp = (stim_angle_accel(n_resp));
    stim_angle_yaw_pre_resp = (stim_angle_yaw(n_resp));
    
    accel_angle_hor_vel_pre_resp = (accel_angle_hor_vel(n_resp));
    accel_angle_hor_body_pre_resp = (accel_angle_hor_body(n_resp));
    
    slip_pre_resp = (slip(n_resp));
    pitch_pre_resp = (pitch(n_resp));
    roll_pre_resp = (roll(n_resp));
    
    V_pre_resp = V(n_resp);
    A_hor_pre_resp = A_hor(n_resp);
    ...
    if min(IDX_shift)<4 || max(IDX_shift)>6
        ...
        accel_angle_hor_vel_pre_turn = accel_angle_hor_vel(n_turn_start);
        accel_angle_hor_body_pre_turn = accel_angle_hor_body(n_turn_start);
        
        slip_pre_turn = slip(n_turn_start);
        pitch_pre_turn = pitch(n_turn_start);
        roll_pre_turn = roll(n_turn_start);
        
        A_hor_pre_turn = A_hor(n_turn_start);
        ...
        if IDX_turn_start < 4
            ...
            while turn_stop == 0
                ...
                elseif IDX(n) > 3
                    ...
                    accel_angle_hor_vel_post_turn = accel_angle_hor_vel(n);
                    accel_angle_hor_body_post_turn = accel_angle_hor_body(n);
                    
                    slip_post_turn = slip(n);
                    pitch_post_turn = pitch(n);
                    roll_post_turn = roll(n);
                    
                    A_hor_post_turn = A_hor(n);
    ...
    if isempty(find(IDX_shift==3, 1))==0 || isempty(find(IDX_shift==6, 1))==0 || isempty(find(IDX_shift==9, 1))==0 
        ...
        if isnan(t_turn_stop)==1 || t_accel_start < t_turn_stop
            ...
            A_hor_pre_accel = A_hor(n_accel_start);
    ...
    % before response
        stim_angle_vel_trig2resp = circ_mean_deg_nonan(stim_angle_vel(n_first:n_resp));
        stim_angle_accel_trig2resp = circ_mean_deg_nonan(stim_angle_accel(n_first:n_resp));
        stim_angle_yaw_trig2resp = circ_mean_deg_nonan(stim_angle_yaw(n_first:n_resp));

        slip_trig2resp = circ_mean_deg_nonan(slip(n_first:n_resp));
        pitch_trig2resp = circ_mean_deg_nonan(pitch(n_first:n_resp));
        roll_trig2resp = circ_mean_deg_nonan(roll(n_first:n_resp));
    ...
    if length(nr_steady) > 1
        ...
        accel_angle_hor_vel_post_resp = accel_angle_hor_vel(n_resp_end);
        accel_angle_hor_body_post_resp = accel_angle_hor_body(n_resp_end);
        ...
        A_hor_post_resp = A_hor(n_resp_end);
        ...
        if length(IDX_shift) > nr_resp_end
            ...
            stim_angle_vel_postresp2end = circ_mean_deg_nonan(stim_angle_vel(n_resp_end:n_steady_end));
            stim_angle_accel_postresp2end = circ_mean_deg_nonan(stim_angle_accel(n_resp_end:n_steady_end));
            stim_angle_yaw_postresp2end = circ_mean_deg_nonan(stim_angle_yaw(n_resp_end:n_steady_end));
            
            slip_postresp2end = circ_mean_deg_nonan(slip(n_resp_end:n_steady_end));
            pitch_postresp2end = circ_mean_deg_nonan(pitch(n_resp_end:n_steady_end));
            roll_postresp2end = circ_mean_deg_nonan(roll(n_resp_end:n_steady_end));
            
            V_postresp2end = mean(V(n_resp_end:n_steady_end));
        else
            stim_angle_vel_postresp2end = circ_mean_deg_nonan(stim_angle_vel(n_resp_end:end));
            stim_angle_accel_postresp2end = circ_mean_deg_nonan(stim_angle_accel(n_resp_end:end));
            stim_angle_yaw_postresp2end = circ_mean_deg_nonan(stim_angle_yaw(n_resp_end:end));
            
            slip_postresp2end = circ_mean_deg_nonan(slip(n_resp_end:end));
            pitch_postresp2end = circ_mean_deg_nonan(pitch(n_resp_end:end));
            roll_postresp2end = circ_mean_deg_nonan(roll(n_resp_end:end));
        ...
        A_hor_mean = nanmean(A_hor(n_resp:n_resp_end));

        A_max = max(A(n_resp:n_resp_end));
        A_mean = nanmean(A(n_resp:n_resp_end));
        ...
        accel_angle_hor_vel_resp_mean = circ_mean_deg_nonan(accel_angle_hor_vel(n_resp:n_resp_end));
        accel_angle_hor_body_resp_mean = circ_mean_deg_nonan(accel_angle_hor_body(n_resp:n_resp_end));

    else
        % mean values from start of response until end of seq
        A_hor_max = max(A_hor(n_resp:end));
        A_hor_mean = nanmean(A_hor(n_resp:end));
        
        A_max = max(A(n_resp:end));
        A_mean = nanmean(A(n_resp:end));
        ...
        accel_angle_hor_vel_resp_mean = circ_mean_deg_nonan(accel_angle_hor_vel(n_resp:end));
        accel_angle_hor_body_resp_mean = circ_mean_deg_nonan(accel_angle_hor_body(n_resp:end));
    end
    
    %% values at A_hor_max & A_max
    if isnan(A_hor_max) == 0
        t_Ahormax = t(A_hor==A_hor_max);
        V_Ahormax = V(A_hor==A_hor_max);
        An_hor_Ahormax = An_hor(A_hor==A_hor_max);
        At_hor_Ahormax = At_hor(A_hor==A_hor_max);
        
        stim_angle_vel_Ahormax = stim_angle_vel(A_hor==A_hor_max);
        stim_angle_accel_Ahormax = stim_angle_accel(A_hor==A_hor_max);
        stim_angle_yaw_Ahormax = stim_angle_yaw(A_hor==A_hor_max);
        
        accel_angle_hor_vel_Ahormax = accel_angle_hor_vel(A_hor==A_hor_max);
        accel_angle_hor_body_Ahormax = accel_angle_hor_body(A_hor==A_hor_max);

        slip_Ahormax = slip(A_hor==A_hor_max);
        pitch_Ahormax = pitch(A_hor==A_hor_max);
        roll_Ahormax = roll(A_hor==A_hor_max);
        
        F_Ahormax = F(A_hor==A_hor_max);
        F_hor_Ahormax = F_hor(A_hor==A_hor_max);
        Fsp_pitch_Ahormax = Fsp_pitch(A_hor==A_hor_max);
        Fsp_roll_Ahormax = Fsp_roll(A_hor==A_hor_max);
    end
    if isnan(A_max) == 0
        t_Amax = t(A==A_max);
        V_Amax = V(A==A_max);
        An_hor_Amax = An_hor(A==A_max);
        At_hor_Amax = At_hor(A==A_max);
        
        stim_angle_vel_Amax = stim_angle_vel(A==A_max);
        stim_angle_accel_Amax = stim_angle_accel(A==A_max);
        stim_angle_yaw_Amax = stim_angle_yaw(A==A_max);
        
        accel_angle_hor_vel_Amax = accel_angle_hor_vel(A==A_max);
        accel_angle_hor_body_Amax = accel_angle_hor_body(A==A_max);

        slip_Amax = slip(A==A_max);
        pitch_Amax = pitch(A==A_max);
        roll_Amax = roll(A==A_max);
        
        F_Amax = F(A==A_max);
        Fsp_pitch_Amax = Fsp_pitch(A==A_max);
        Fsp_roll_Amax = Fsp_roll(A==A_max);
    end
end
