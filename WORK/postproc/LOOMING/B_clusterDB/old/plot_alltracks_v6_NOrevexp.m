...
seqs = length(settings.expansion.HorPos);
for i = 1:seqs
    j = seqs - i +1;
    if settings.expansion.HorPos(j) == 180
        settings.expansion.HorPos(j) = [];
        x(:,j) = [];
        y(:,j) = [];
        z(:,j) = [];
    end
...
if topview == true
    movie_step = 15;
    fps = 7500/15;
    movfilename = ['3Dflightpaths_topview_NOrevexp_',num2str(fps),'fps.avi'];
...
if fourviews == true
    movie_step = 1;
    movfilename = '3Dflightpaths_4views_NOrevexp.avi';
    mov = avifile(movfilename, 'compression', 'none');
