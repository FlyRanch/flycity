...
%         counter = 100*(size(n_down_start_L,2)-seq) +size(n_down_start_L,1) -wb
        ...
        subset_roll_now = subset_roll(wb,seq);
        subset_pitch_now = subset_pitch(wb,seq);
        subset_F_now = subset_F(wb,seq);
        
        % ONLY NO ROLL
        if abs(subset_roll_now) < subset_roll_mid
        if abs(subset_pitch_now) < subset_pitch_mid || abs(subset_pitch_now) > subset_pitch_max ||...
           abs(subset_F_now) < subset_F_mid || abs(subset_F_now) > subset_F_max
            ...
            if abs(subset_pitch_now) < subset_pitch_mid
                plot_pitch = 1;
                color_code_pitch_L_now = 'g';
                color_code_pitch_R_now = 'g';
                mid_pitch=mid_pitch+1;
            elseif subset_pitch_now > subset_pitch_max
                plot_pitch = 1;
                color_code_pitch_L_now = 'b';
                color_code_pitch_R_now = 'r';
                hi_pitch=hi_pitch+1;
%             elseif subset_pitch_now < -subset_pitch_max
%                 plot_pitch = 1;
...
%                 low_pitch=low_pitch+1;
            ...
            if abs(subset_F_now) < subset_F_mid
                plot_F = 1;
                color_code_F_L_now = 'g';
                color_code_F_R_now = 'g';
                mid_F=mid_F+1;
            elseif subset_F_now > subset_F_max
                plot_F = 1;
                color_code_F_L_now = 'b';
                color_code_F_R_now = 'r';
                hi_F=hi_F+1;
%             elseif subset_F_now < -subset_F_max
%                 plot_F = 1;
%                 color_code_F_now = 'b';
...
%             xi_R = [1:length(stroke_wb_R_now)/100:length(stroke_wb_R_now)];
            ...
            if plot_pitch ==1
                subplot(2,5,1)
                plot(xi,stroke_wb_L_interp,'-','color',color_code_pitch_L_now,'linewidth',linewidth_timelines)
                plot(xi,stroke_wb_R_interp,'-','color',color_code_pitch_R_now,'linewidth',linewidth_timelines)
                subplot(2,5,2)
                plot(xi,pitch_wb_L_interp,'-','color',color_code_pitch_L_now,'linewidth',linewidth_timelines)
                plot(xi,pitch_wb_R_interp,'-','color',color_code_pitch_R_now,'linewidth',linewidth_timelines)
                subplot(2,5,3)
                plot(xi,dev_wb_L_interp,'-','color',color_code_pitch_L_now,'linewidth',linewidth_timelines)
                plot(xi,dev_wb_R_interp,'-','color',color_code_pitch_R_now,'linewidth',linewidth_timelines)
                 subplot(2,5,4)
                plot(xi,aoa_wb_L_interp,'-','color',color_code_pitch_L_now,'linewidth',linewidth_timelines)
                plot(xi,aoa_wb_R_interp,'-','color',color_code_pitch_R_now,'linewidth',linewidth_timelines)
                subplot(2,5,5)
                plot(xi,U_wb_L_interp,'-','color',color_code_pitch_L_now,'linewidth',linewidth_timelines)
                plot(xi,U_wb_R_interp,'-','color',color_code_pitch_R_now,'linewidth',linewidth_timelines)
            end
            
            if plot_F ==1
                subplot(2,5,6)
                plot(xi,stroke_wb_L_interp,'-','color',color_code_F_L_now,'linewidth',linewidth_timelines)
                plot(xi,stroke_wb_R_interp,'-','color',color_code_F_R_now,'linewidth',linewidth_timelines)
                subplot(2,5,7)
                plot(xi,pitch_wb_L_interp,'-','color',color_code_F_L_now,'linewidth',linewidth_timelines)
                plot(xi,pitch_wb_R_interp,'-','color',color_code_F_R_now,'linewidth',linewidth_timelines)
                subplot(2,5,8)
                plot(xi,dev_wb_L_interp,'-','color',color_code_F_L_now,'linewidth',linewidth_timelines)
                plot(xi,dev_wb_R_interp,'-','color',color_code_F_R_now,'linewidth',linewidth_timelines)
                subplot(2,5,9)
                plot(xi,aoa_wb_L_interp,'-','color',color_code_F_L_now,'linewidth',linewidth_timelines)
                plot(xi,aoa_wb_R_interp,'-','color',color_code_F_R_now,'linewidth',linewidth_timelines)
                subplot(2,5,10)
                plot(xi,U_wb_L_interp,'-','color',color_code_F_L_now,'linewidth',linewidth_timelines)
                plot(xi,U_wb_R_interp,'-','color',color_code_F_R_now,'linewidth',linewidth_timelines)
            end
        end
        end
    end
