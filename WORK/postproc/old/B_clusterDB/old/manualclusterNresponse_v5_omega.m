...
roll_dot  = rad2deg(pathDB.omega(:,:,1));
pitch_dot = rad2deg(pathDB.omega(:,:,2));
yaw_dot   = rad2deg(pathDB.omega(:,:,3));
...
omega_turn = nan(size(x));
omega_turn_axis = nan(size(x));
...
for j = 1:size(x,2)
    ...
    for i = 1:size(x,1)
        if isnan(x(i,j))==0
            ...
            % omega data
            omega_turn(i,j) = norm([pitch_dot(i,j),yaw_dot(i,j)]);
            omega_turn_axis(i,j) = rad2deg(atan2(yaw_dot(i,j),pitch_dot(i,j)));
            
            if i > trigger_frame
                % previous d's
                if isnan(droll(i-1,j))==0
                    droll_prev = droll(i-1,j);
                    dpitch_prev = dpitch(i-1,j);
                    dyaw_prev = dyaw(i-1,j);
                else
                    droll_prev = 0;
                    dpitch_prev = 0;
                    dyaw_prev = 0;
                ...
                % integrate
                droll(i,j) = droll_prev + roll_dot(i,j)*dt;
                dpitch(i,j) = dpitch_prev + pitch_dot(i,j)*dt;
                dyaw(i,j) = dyaw_prev + yaw_dot(i,j)*dt;
            end
            ...
            % V_hor vector in body coord system
...
pathDB.yaw_dot = yaw_dot;
pathDB.pitch_dot = pitch_dot;
pathDB.roll_dot = roll_dot;

pathDB.dyaw = dyaw;
pathDB.dpitch = dpitch;
pathDB.droll = droll;

pathDB.omega_turn = omega_turn;
pathDB.omega_turn_axis = omega_turn_axis;
