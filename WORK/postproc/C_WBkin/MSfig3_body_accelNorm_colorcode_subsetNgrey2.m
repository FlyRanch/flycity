...
for i=1:size(stim_angle_vel,2)
    ...
    if isnan(color_var(i)) == 0 && color_var(i)~=0
        subplot(3,3,1)
        plot(t_wb_L(:,i)-t_shift(i),roll_dot_dot_mean_wb_norm(:,i),'-','color',[.75 .75 .75],'linewidth',.25)
        subplot(3,3,4)
        plot(t_wb_L(:,i)-t_shift(i),pitch_dot_dot_mean_wb_norm(:,i),'-','color',[.75 .75 .75],'linewidth',.25)
        subplot(3,3,7)
        plot(t_wb_L(:,i)-t_shift(i),yaw_dot_dot_mean_wb_norm(:,i),'-','color',[.75 .75 .75],'linewidth',.25)
    end
end
...
for j=1:length(subset_seqs)
    ...
    if isnan(color_var(i)) == 0 && color_var(i)~=0
%         subplot(3,3,1)
%         plot(t_wb_L(:,i)-t_shift(i),roll_dot_dot_mean_wb_norm(:,i),'-o','color',cmap_plot(color_var(i),:),'linewidth',linewidth_timelines)
%         subplot(3,3,4)
%         plot(t_wb_L(:,i)-t_shift(i),pitch_dot_dot_mean_wb_norm(:,i),'-o','color',cmap_plot(color_var(i),:),'linewidth',linewidth_timelines)
%         subplot(3,3,7)
%         plot(t_wb_L(:,i)-t_shift(i),yaw_dot_dot_mean_wb_norm(:,i),'-o','color',cmap_plot(color_var(i),:),'linewidth',linewidth_timelines)
