...
for wb = 1:size(wb_nr,1)
    counter = size(wb_nr,1) -wb
        ...
        %% NONsteady wb
        elseif abs(roll_dot_dot_mean_wb(wb)) > rollaccel_limit_mod ||...
                abs(pitch_dot_dot_mean_wb(wb)) > pitchaccel_limit_mod ||...
                abs(yaw_dot_dot_mean_wb(wb)) > yawaccel_limit_mod ||...
                abs(F_mean_wb(wb)-1) > Fenhance_limit_mod
