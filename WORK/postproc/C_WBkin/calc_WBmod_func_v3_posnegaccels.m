...
% t1_mod=(t1+1)/2;
% t2_mod=(t2+1)/2;
% t_mod = [t1_mod(1:end-1) t2_mod]';
% 
% PN = Legendre_polynomial(length(a_avg.stroke1)-1,2,t_pn');
% var = PN(:,:,1)'*a_avg.stroke1;
% var = [var(1:end-1);PN(:,:,1)'*a_avg.stroke2];
% stroke_wb_steady = interp1(t_mod,var,t);
% 
% PN = Legendre_polynomial(length(a_avg.pitch1)-1,2,t_pn');
% var = PN(:,:,1)'*a_avg.pitch1;
% var = [var(1:end-1);PN(:,:,1)'*a_avg.pitch2];
% pitch_wb_steady = interp1(t_mod,var,t);
% 
% PN = Legendre_polynomial(length(a_avg.dev1)-1,2,t_pn');
% var = PN(:,:,1)'*a_avg.dev1;
% var = [var(1:end-1);PN(:,:,1)'*a_avg.dev2];
% dev_wb_steady = interp1(t_mod,var,t);
% 
...
%         counter = 100*(size(n_down_start_L,2)-seq) +size(n_down_start_L,1) -wb
        ...
        if abs(roll_dot_dot_mean_wb_now) > abs(roll_dot_dot_limit)
            if roll_dot_dot_mean_wb_now > 0
                ...
                Dstroke_rollaccel_up(end+1:end+length(t),1) = Dstroke_wb_L / abs(roll_dot_dot_mean_wb_now) * roll_dot_dot_max;
                Dstroke_rollaccel_down(end+1:end+length(t),1) = Dstroke_wb_R / abs(roll_dot_dot_mean_wb_now) * roll_dot_dot_max;
                Ddev_rollaccel_up(end+1:end+length(t),1) = Ddev_wb_L / abs(roll_dot_dot_mean_wb_now) * roll_dot_dot_max;
                Ddev_rollaccel_down(end+1:end+length(t),1) = Ddev_wb_R / abs(roll_dot_dot_mean_wb_now) * roll_dot_dot_max;
                Dpitch_rollaccel_up(end+1:end+length(t),1) = Dpitch_wb_L / abs(roll_dot_dot_mean_wb_now) * roll_dot_dot_max;
                Dpitch_rollaccel_down(end+1:end+length(t),1) = Dpitch_wb_R / abs(roll_dot_dot_mean_wb_now) * roll_dot_dot_max;

                Ddstroke_rollaccel(end+1:end+length(t),1) = (Dstroke_wb_L-Dstroke_wb_R) / abs(roll_dot_dot_mean_wb_now) * roll_dot_dot_max;
                Ddpitch_rollaccel(end+1:end+length(t),1) = (Dpitch_wb_L-Dpitch_wb_R) / abs(roll_dot_dot_mean_wb_now) * roll_dot_dot_max;
                Dddev_rollaccel(end+1:end+length(t),1) = (Ddev_wb_L-Ddev_wb_R) / abs(roll_dot_dot_mean_wb_now) * roll_dot_dot_max;
            else
                t_roll(end+1:end+length(t),1) = t;
                t_roll_LR(end+1:end+length(t),1) = t;
                t_roll_LR(end+1:end+length(t),1) = t;

                Dstroke_rollaccel_up(end+1:end+length(t),1) = Dstroke_wb_R / abs(roll_dot_dot_mean_wb_now) * roll_dot_dot_max;
                Dstroke_rollaccel_down(end+1:end+length(t),1) = Dstroke_wb_L / abs(roll_dot_dot_mean_wb_now) * roll_dot_dot_max;
                Ddev_rollaccel_up(end+1:end+length(t),1) = Ddev_wb_R / abs(roll_dot_dot_mean_wb_now) * roll_dot_dot_max;
                Ddev_rollaccel_down(end+1:end+length(t),1) = Ddev_wb_L / abs(roll_dot_dot_mean_wb_now) * roll_dot_dot_max;
                Dpitch_rollaccel_up(end+1:end+length(t),1) = Dpitch_wb_R / abs(roll_dot_dot_mean_wb_now) * roll_dot_dot_max;
                Dpitch_rollaccel_down(end+1:end+length(t),1) = Dpitch_wb_L / abs(roll_dot_dot_mean_wb_now) * roll_dot_dot_max;

                Ddstroke_rollaccel(end+1:end+length(t),1) = (Dstroke_wb_R-Dstroke_wb_L) / abs(roll_dot_dot_mean_wb_now) * roll_dot_dot_max;
                Ddpitch_rollaccel(end+1:end+length(t),1) = (Dpitch_wb_R-Dpitch_wb_L) / abs(roll_dot_dot_mean_wb_now) * roll_dot_dot_max;
                Dddev_rollaccel(end+1:end+length(t),1) = (Ddev_wb_R-Ddev_wb_L) / abs(roll_dot_dot_mean_wb_now) * roll_dot_dot_max;
            end
        end
        
        if abs(yaw_dot_dot_mean_wb_now) > abs(yaw_dot_dot_limit)
            if yaw_dot_dot_mean_wb_now > 0
                t_yaw(end+1:end+length(t),1) = t;
                t_yaw_LR(end+1:end+length(t),1) = t;
                t_yaw_LR(end+1:end+length(t),1) = t;

                Dstroke_yawaccel_right(end+1:end+length(t),1) = Dstroke_wb_L / yaw_dot_dot_mean_wb_now * yaw_dot_dot_max;
                Dstroke_yawaccel_left(end+1:end+length(t),1) = Dstroke_wb_R / yaw_dot_dot_mean_wb_now * yaw_dot_dot_max;
                Ddev_yawaccel_right(end+1:end+length(t),1) = Ddev_wb_L / yaw_dot_dot_mean_wb_now * yaw_dot_dot_max;
                Ddev_yawaccel_left(end+1:end+length(t),1) = Ddev_wb_R / yaw_dot_dot_mean_wb_now * yaw_dot_dot_max;
                Dpitch_yawaccel_right(end+1:end+length(t),1) = Dpitch_wb_L / yaw_dot_dot_mean_wb_now * yaw_dot_dot_max;
                Dpitch_yawaccel_left(end+1:end+length(t),1) = Dpitch_wb_R / yaw_dot_dot_mean_wb_now * yaw_dot_dot_max;
                ...
                Dstroke_yawaccel_right(end+1:end+length(t),1) = Dstroke_wb_R / yaw_dot_dot_mean_wb_now * yaw_dot_dot_max;
                Dstroke_yawaccel_left(end+1:end+length(t),1) = Dstroke_wb_L / yaw_dot_dot_mean_wb_now * yaw_dot_dot_max;
                Ddev_yawaccel_right(end+1:end+length(t),1) = Ddev_wb_R / yaw_dot_dot_mean_wb_now * yaw_dot_dot_max;
                Ddev_yawaccel_left(end+1:end+length(t),1) = Ddev_wb_L / yaw_dot_dot_mean_wb_now * yaw_dot_dot_max;
                Dpitch_yawaccel_right(end+1:end+length(t),1) = Dpitch_wb_R / yaw_dot_dot_mean_wb_now * yaw_dot_dot_max;
                Dpitch_yawaccel_left(end+1:end+length(t),1) = Dpitch_wb_L / yaw_dot_dot_mean_wb_now * yaw_dot_dot_max;

                Ddstroke_yawaccel(end+1:end+length(t),1) = (Dstroke_wb_R-Dstroke_wb_L) / yaw_dot_dot_mean_wb_now * yaw_dot_dot_max;
                Ddpitch_yawaccel(end+1:end+length(t),1) = (Dpitch_wb_R-Dpitch_wb_L) / yaw_dot_dot_mean_wb_now * yaw_dot_dot_max;
                Dddev_yawaccel(end+1:end+length(t),1) = (Ddev_wb_R-Ddev_wb_L) / yaw_dot_dot_mean_wb_now * yaw_dot_dot_max;
            end                
        end
...
% plot_WBmod_pitchaccelNforce_meanCI
