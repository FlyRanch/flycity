...
PAR.videopath = PathName;
PAR.filetag = FileName(1:end-7);

cd(PathName)
cd ..
PAR.solutionpath = [cd,'/solutions/'];
...
for cam=1:PAR.numcam
    fr_nr = sprintf('%03d.bmp', frame);
    input_filename = [PAR.videopath(1:end-2),num2str(cam),'/',PAR.filetag,fr_nr];
...
%     pts3D(occidx,:) = [];
%     
%     %line directions: First 3 components of ray's Pl�ker
%     %coordinates
