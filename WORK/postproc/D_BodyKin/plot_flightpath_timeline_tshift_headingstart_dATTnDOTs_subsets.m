...
for j=1:length(seqs)
    ...
    if isnan(color_var(i)) == 0 && color_var(i)~=0
        subplot(3,3,1)
        plot(t-t_shift(i),droll_plot(:,i),'-','color',cmap_plot(color_var(i),:),'linewidth',linewidth_timelines)
        subplot(3,3,4)
        plot(t-t_shift(i),dpitch_plot(:,i),'-','color',cmap_plot(color_var(i),:),'linewidth',linewidth_timelines)
        subplot(3,3,7)
        plot(t-t_shift(i),dyaw_plot(:,i),'-','color',cmap_plot(color_var(i),:),'linewidth',linewidth_timelines)
...
subplot(3,3,3)
axis([t_start t_stop -3e5 3e5])
%     set(gca,'XTick',t_start:-t_start:t_stop) 
    set(gca,'XTick',t_start:-t_start:t_stop,'XTickLabel',[]) 
    set(gca,'YTick',[-3e5:3e5:3e5],'fontsize',8)
%     xlabel('time','fontsize',10)
    ylabel('roll accel','fontsize',10)
    grid on
subplot(3,3,6)
axis([t_start t_stop -2e5 2e5])
%     set(gca,'XTick',t_start:-t_start:t_stop) 
    set(gca,'XTick',t_start:-t_start:t_stop,'XTickLabel',[]) 
    set(gca,'YTick',[-2e5;0;2e5],'fontsize',8)
%     xlabel('time','fontsize',10)
