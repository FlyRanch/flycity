...
for i = 1:length(list)
    ...
    if exist(file_now) == 2
        ...
        n=n+1;
        run_type_pitch{n} = exp_type;
        mod_value_pitch(n,1) = mod_val;
    
        t_pitch(1:seq_length_max,end+1) = nan;
        
        stroke_L_pitch(1:seq_length_max,end+1) = nan;
        pitch_L_pitch(1:seq_length_max,end+1) = nan;
        dev_L_pitch(1:seq_length_max,end+1) = nan;
        
        stroke_R_pitch(1:seq_length_max,end+1) = nan;
        pitch_R_pitch(1:seq_length_max,end+1) = nan;
        dev_R_pitch(1:seq_length_max,end+1) = nan;
        
        Fx_pitch(1:seq_length_max,1:Nwb,end+1) = nan;
        Fy_pitch(1:seq_length_max,1:Nwb,end+1) = nan;
        Fz_pitch(1:seq_length_max,1:Nwb,end+1) = nan;
        Mx_pitch(1:seq_length_max,1:Nwb,end+1) = nan;
        My_pitch(1:seq_length_max,1:Nwb,end+1) = nan;
        Mz_pitch(1:seq_length_max,1:Nwb,end+1) = nan;
...
%             length(stroke_L_now)-j
            Fx_cali_L(j,1) = interp3(cali_L.rot_mesh,cali_L.str_mesh,cali_L.dev_mesh,cali_L.Fx_array,...
                stroke_L_now(j),pitch_L_now(j),dev_L_now(j),'spline');
            Fy_cali_L(j,1) = interp3(cali_L.rot_mesh,cali_L.str_mesh,cali_L.dev_mesh,cali_L.Fy_array,...
                stroke_L_now(j),pitch_L_now(j),dev_L_now(j),'spline');
            Fz_cali_L(j,1) = interp3(cali_L.rot_mesh,cali_L.str_mesh,cali_L.dev_mesh,cali_L.Fz_array,...
                stroke_L_now(j),pitch_L_now(j),dev_L_now(j),'spline');
            
            Fx_cali_R(j,1) = interp3(cali_R.rot_mesh,cali_R.str_mesh,cali_R.dev_mesh,cali_R.Fx_array,...
                stroke_L_now(j),pitch_L_now(j),dev_L_now(j),'spline');
            Fy_cali_R(j,1) = interp3(cali_R.rot_mesh,cali_R.str_mesh,cali_R.dev_mesh,cali_R.Fy_array,...
                stroke_L_now(j),pitch_L_now(j),dev_L_now(j),'spline');
            Fz_cali_R(j,1) = interp3(cali_R.rot_mesh,cali_R.str_mesh,cali_R.dev_mesh,cali_R.Fz_array,...
                stroke_L_now(j),pitch_L_now(j),dev_L_now(j),'spline');
            
            Mx_cali_L(j,1) = interp3(cali_L.rot_mesh,cali_L.str_mesh,cali_L.dev_mesh,cali_L.Mx_array,...
                stroke_L_now(j),pitch_L_now(j),dev_L_now(j),'spline');
            My_cali_L(j,1) = interp3(cali_L.rot_mesh,cali_L.str_mesh,cali_L.dev_mesh,cali_L.My_array,...
                stroke_L_now(j),pitch_L_now(j),dev_L_now(j),'spline');
            Mz_cali_L(j,1) = interp3(cali_L.rot_mesh,cali_L.str_mesh,cali_L.dev_mesh,cali_L.Mz_array,...
                stroke_L_now(j),pitch_L_now(j),dev_L_now(j),'spline');
            
            Mx_cali_R(j,1) = interp3(cali_R.rot_mesh,cali_R.str_mesh,cali_R.dev_mesh,cali_R.Mx_array,...
                stroke_L_now(j),pitch_L_now(j),dev_L_now(j),'spline');
            My_cali_R(j,1) = interp3(cali_R.rot_mesh,cali_R.str_mesh,cali_R.dev_mesh,cali_R.My_array,...
                stroke_L_now(j),pitch_L_now(j),dev_L_now(j),'spline');
            Mz_cali_R(j,1) = interp3(cali_R.rot_mesh,cali_R.str_mesh,cali_R.dev_mesh,cali_R.Mz_array,...
                stroke_L_now(j),pitch_L_now(j),dev_L_now(j),'spline');
...
%             Mz_now(j,:) = Mz_robo(j,:) - Mz_cali_L(j) - Mz_cali_R(j);
        ...
        %% calc wb mean data
        Fx_mean_pitch(n,1) = nanmean(Fx_fly(:));
        Fy_mean_pitch(n,1) = nanmean(Fy_fly(:));
        Fz_mean_pitch(n,1) = nanmean(Fz_fly(:));

        Mx_mean_pitch(n,1) = nanmean(Mx_fly(:));
        My_mean_pitch(n,1) = nanmean(My_fly(:));
        Mz_mean_pitch(n,1) = nanmean(Mz_fly(:));

        %% store pitch data
        t_pitch(1:length(t_now),n) = t_now;

        stroke_L_pitch(1:length(t_now),n) = stroke_L_now;
        pitch_L_pitch(1:length(t_now),n) = pitch_L_now;
        dev_L_pitch(1:length(t_now),n) = dev_L_now;

        stroke_R_pitch(1:length(t_now),n) = stroke_R_now;
        pitch_R_pitch(1:length(t_now),n) = pitch_R_now;
        dev_R_pitch(1:length(t_now),n) = dev_R_now;

        Fx_pitch(1:length(t_now),:,n) = Fx_now;
        Fy_pitch(1:length(t_now),:,n) = Fy_now;
        Fz_pitch(1:length(t_now),:,n) = Fz_now;

        Mx_pitch(1:length(t_now),:,n) = Mx_now;
        My_pitch(1:length(t_now),:,n) = My_now;
        Mz_pitch(1:length(t_now),:,n) = Mz_now;
    end
end
