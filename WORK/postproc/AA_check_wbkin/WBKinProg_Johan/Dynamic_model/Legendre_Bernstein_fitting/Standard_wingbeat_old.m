function [a_fit_L,a_fit_R,a_avg_L,a_avg_R,a_avg_LR,f_avg,down_up,trigger_wb] = Standard_wingbeat_old( settings, pathDB, seq_nr, order_pol_theta, order_pol_eta, order_pol_phi )
...
%     end
    ...
    wb_loc1 = zeros(nr_wb,2);
    
    wb_loc2 = zeros(nr_wb,2);
    ...
    for k = 1:nr_wb
        ...
        if k == nr_wb
            ...
            wb_loc1(k,:) = [ pathDB.wingbeat_time(nr_wb,1,seq_nr) frame_half ];
            
            wb_loc2(k,:) = [frame_half pathDB.wingbeat_time(nr_wb,wb_end,seq_nr)];
            ...
            wb_loc1(k,:) = [ pathDB.wingbeat_time(k,1,seq_nr) frame_half ];
            
            wb_loc2(k,:) = [frame_half pathDB.wingbeat_time(k+1,1,seq_nr)];
    ...
    a_fit_theta_L1 = Piecewise_polynomial_fit(theta_L,n_pol_theta,wb_loc1);

    a_fit_eta_L1 = Piecewise_polynomial_fit(eta_L,n_pol_eta,wb_loc1);

    a_fit_phi_L1 = Piecewise_polynomial_fit(phi_L,n_pol_phi,wb_loc1);


    a_fit_theta_L2 = Piecewise_polynomial_fit(theta_L,n_pol_theta,wb_loc2);

    a_fit_eta_L2 = Piecewise_polynomial_fit(eta_L,n_pol_eta,wb_loc2);

    a_fit_phi_L2 = Piecewise_polynomial_fit(phi_L,n_pol_phi,wb_loc2);
    
    
    
    a_fit_theta_R1 = Piecewise_polynomial_fit(theta_R,n_pol_theta,wb_loc1);

    a_fit_eta_R1 = Piecewise_polynomial_fit(eta_R,n_pol_eta,wb_loc1);

    a_fit_phi_R1 = Piecewise_polynomial_fit(phi_R,n_pol_phi,wb_loc1);
    
    
    a_fit_theta_R2 = Piecewise_polynomial_fit(theta_R,n_pol_theta,wb_loc1);

    a_fit_eta_R2 = Piecewise_polynomial_fit(eta_R,n_pol_eta,wb_loc1);

    a_fit_phi_R2 = Piecewise_polynomial_fit(phi_R,n_pol_phi,wb_loc1);
    ...
    var_theta_L = zeros(2*(trigger_wb-2),2*(trigger_wb-2),n_pol_theta+1);
    
    var_theta_R = zeros(2*(trigger_wb-2),2*(trigger_wb-2),n_pol_theta+1);
    
    var_eta_L = zeros(2*(trigger_wb-2),2*(trigger_wb-2),n_pol_eta+1);
    
    var_eta_R = zeros(2*(trigger_wb-2),2*(trigger_wb-2),n_pol_eta+1);
    
    var_phi_L = zeros(2*(trigger_wb-2),2*(trigger_wb-2),n_pol_phi+1);
    
    var_phi_R = zeros(2*(trigger_wb-2),2*(trigger_wb-2),n_pol_phi+1);
    
    var_theta_LR = zeros(2*(trigger_wb-2),2*(trigger_wb-2),n_pol_theta+1);
      
    var_eta_LR = zeros(2*(trigger_wb-2),2*(trigger_wb-2),n_pol_eta+1);
    
    var_phi_LR = zeros(2*(trigger_wb-2),2*(trigger_wb-2),n_pol_phi+1);

    
    for m = 1:(2*(trigger_wb-2))
            
        for n = 1:(2*(trigger_wb-2))
    ...
    weight_theta_L = zeros(n_pol_theta+1,2*(trigger_wb-2));
    
    weight_theta_R = zeros(n_pol_theta+1,2*(trigger_wb-2));
    ...
    weight_eta_L = zeros(n_pol_eta+1,2*(trigger_wb-2));
    
    weight_eta_R = zeros(n_pol_eta+1,2*(trigger_wb-2));
    ...
    weight_phi_L = zeros(n_pol_phi+1,2*(trigger_wb-2));
    
    weight_phi_R = zeros(n_pol_phi+1,2*(trigger_wb-2));
    ...
    wb_loc_mean = zeros((trigger_wb-2)*2,2);

    for k = 1:(trigger_wb-2)

        if k == (trigger_wb-2)

            wb_end = find(isnan(pathDB.wingbeat_time((trigger_wb-2),:,seq_nr))==0, 1, 'last' );
            
            frame_half = round((pathDB.wingbeat_time((trigger_wb-2),wb_end,seq_nr)+pathDB.wingbeat_time((trigger_wb-2),1,seq_nr))/2);

            wb_loc_mean(2*k-1,:) = [ pathDB.wingbeat_time((trigger_wb-2),1,seq_nr) frame_half ];
            
            wb_loc_mean(2*k,:) = [frame_half pathDB.wingbeat_time((trigger_wb-2),wb_end,seq_nr)];
            ...
            wb_loc_mean(2*k-1,:) = [ pathDB.wingbeat_time(k,1,seq_nr) frame_half ];
            
            wb_loc_mean(2*k,:) = [frame_half pathDB.wingbeat_time(k+1,1,seq_nr)];

        end

    end
    
    wb_loc_mean1 = zeros((trigger_wb-2),2);
    
    wb_loc_mean2 = zeros((trigger_wb-2),2);
    ...
    for k = 1:(trigger_wb-2)
        ...
        if k == (trigger_wb-2)
            ...
            wb_loc_mean1(k,:) = [ pathDB.wingbeat_time((trigger_wb-2),1,seq_nr) frame_half ];
            
            wb_loc_mean2(k,:) = [frame_half pathDB.wingbeat_time((trigger_wb-2),wb_end,seq_nr)];
            ...
            wb_loc_mean1(k,:) = [ pathDB.wingbeat_time(k,1,seq_nr) frame_half ];
            
            wb_loc_mean2(k,:) = [frame_half pathDB.wingbeat_time(k+1,1,seq_nr)];
    ...
    a_avg_theta_L1 = average_fit(theta_L,n_pol_theta,wb_loc_mean1,W_theta_L);
    
    a_avg_theta_L2 = average_fit(theta_L,n_pol_theta,wb_loc_mean2,W_theta_L);
    
    a_avg_theta_R1 = average_fit(theta_R,n_pol_theta,wb_loc_mean1,W_theta_R);
    
    a_avg_theta_R2 = average_fit(theta_R,n_pol_theta,wb_loc_mean2,W_theta_R);
    
    
    
    a_avg_eta_L1 = average_fit(eta_L,n_pol_eta,wb_loc_mean1,W_eta_L);
    
    a_avg_eta_L2 = average_fit(eta_L,n_pol_eta,wb_loc_mean2,W_eta_L);
    
    a_avg_eta_R1 = average_fit(eta_R,n_pol_eta,wb_loc_mean1,W_eta_R);
    
    a_avg_eta_R2 = average_fit(eta_R,n_pol_eta,wb_loc_mean2,W_eta_R);
    
   
    a_avg_phi_L1 = average_fit(phi_L,n_pol_phi,wb_loc_mean1,W_phi_L);
    
    a_avg_phi_L2 = average_fit(phi_L,n_pol_phi,wb_loc_mean2,W_phi_L);
    
    a_avg_phi_R1 = average_fit(phi_R,n_pol_phi,wb_loc_mean1,W_phi_R);
    
    a_avg_phi_R2 = average_fit(phi_R,n_pol_phi,wb_loc_mean2,W_phi_R);
    ...
    weight_theta_LR1 = zeros(n_pol_theta+1,2*(trigger_wb-2));
    
    for i = 1:(n_pol_theta+1)
        
        weight_theta_LR1(i,:) = sum([var_theta_L(:,:,i) var_theta_LR(:,:,i); var_theta_LR(:,:,i) var_theta_R(:,:,i)]);
    ...
    weight_eta_LR = zeros(n_pol_eta+1,4*(trigger_wb-2));
    ...
    weight_phi_LR = zeros(n_pol_phi+1,4*(trigger_wb-2));
    ...
    wb_loc_LR = zeros(4*(trigger_wb-2),2);
