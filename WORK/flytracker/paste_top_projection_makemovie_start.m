...
function paste_top_projection_makemovie_start
% clear
clc;
close all
warning off
...
frame_prev = str2num(frames(1).name(4:end-4));
for i = 2:length(frames)
    frame_now = str2num(frames(i).name(4:end-4));
    if frame_now ~= frame_prev +1;
        frame_end = frame_prev;
    end
    frame_prev = frame_now;
end

if exist('frame_end')==0
...
fps = 7500/skip;
    ...
    if cams == 1;
        movfilename = ['side1_projection_start_',num2str(fps),'fps.avi'];
    elseif cams == 2;
        movfilename = ['side2_projection_start_',num2str(fps),'fps.avi'];
    elseif cams == 3;
        movfilename = ['top_projection_start_',num2str(fps),'fps.avi'];
    end
...
%                 'fontsize',20);
              text(PAR.imgres(2)/2 - 50,100,['\color{white}Frame ' num2str(frame)], ...
                  'fontsize',20);
        end
