...
if cams_found == 0
    ...
    data.images.flexload(1).path = data.images.path;
        ...
        %% Photron structure FTMmod 20120611
    for i = 2:data.setup.cam_num
        
        data.images.flexload(i).path = [data.images.path(1:end-11),num2str(i),data.images.path(end-9:end)];
        cd(data.images.flexload(i).path)
        ims = dir(['*',data.images.file(end-3:end)]);
        data.images.flexload(i).file = ims(1).name
        cd(data.setup.mfile_path)
