function varargout = quivert(varargin)
...
% Modyfied by Christoffer Johansson 2007
...
if (~notails),
	if (isColMatrix),
% 		h(2) = patch(ptx(tailind,nonZeroTail),pty(tailind,nonZeroTail),col(tailind,:),'FaceColor','none','FaceVertexCData',[(col(tailind(1),nonZeroTail))';(col(tailind(2),nonZeroTail))'],'EdgeColor','flat');
		h(2) = patch(ptx(tailind,:),pty(tailind,:),col(tailind,:),'FaceColor','none','EdgeColor','flat');
	else
		h(2) = patch(ptx(tailind,nonZeroTail),pty(tailind,nonZeroTail),col,'FaceColor','none','EdgeColor',col);
	end;
end;
