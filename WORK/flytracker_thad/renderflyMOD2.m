function [Y,idx,IMout,Nrml] = renderflyMOD2(X,PAR,M,X1)

% [Y,IDX,IMout] = renderflyMOD2(X,PAR)
...
% boundary points.
%
% This function gets the boundary points from the body without the wings
% present because we can see the body in the real images during
% self-occlusions because it is much darker than the wings and legs.
%
...
%     end
    ...
    if j == 1
        %Save the body only rendered image
        %THLmod 20140219 wierd assignment stmt. Why is IMbody assigned to
        %IMout?
        %old code:IMbody = IMout;
        %new code:
        IMbody = tmp;
        % Fill in the holes
...
for k = 1:length(B)
    ...
    elseif (all(diff(B{k}(:,1)) == 0) || ...
            all(diff(B{k}(:,2)) == 0))
        %This boundary is the image border becasue the boundry has straight
        %edges
        bndy2rid(end+1) = k;
    elseif (size(B{k},1) == PAR.imgres(1) || size(B{k},1) >= PAR.imgres(2))
        %This boundary is also probably an image border 
        %THLmod 20140218 ... because it is a long boundry - this alone is 
        %not a great condition - as a hack we should also add the condition 
        %that it is not the only boundry in B: if(lenght(B)>1)
        if (length(B)>1)
            bndy2rid(end+1) = k;
        end
    end
...
%% Now, determine the correspondence for just wings
...
%% Now repeat the same code from previous 2 cells, but do it for the body
%% only image

%% Extract body image boundaries
B = bwboundaries(IMbody);
...
%% Now, determine the correspondence for just body

% Make sure it's a column vector
X2 = X2(:);

%Get the projected body points
ALL2DPts = X2{1};
