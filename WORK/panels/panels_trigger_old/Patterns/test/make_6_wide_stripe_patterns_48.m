...
pattern.x_num = 192; %96 	% There are 192 pixel around the display (24x8) 
pattern.y_num = 4; 		% two frames of Y, at 2 different spatial frequencies
pattern.num_panels = 120; %48; 	% This is the number of unique Panel IDs required.
pattern.gs_val = 1; 	% This pattern will use 8 intensity levels
pattern.row_compression = 1;

% Pats = zeros(4, 96, pattern.x_num, pattern.y_num);
Pats = zeros(4, 192, pattern.x_num, pattern.y_num);

Pats(:, :, 1, 1) = [ones(4,186) zeros(4,6)]; % one stripe
% two stripes, 90 degs apart
Pats(:, :, 1, 2) = [ones(4,6) zeros(4,6) ones(4,162) zeros(4,6) ones(4,12)]; 
% 3 stripes
Pats(:, :, 1, 3) = repmat([ones(4, 26) zeros(4,6)], 1, 6);
% 8 stripes
Pats(:, :, 1, 4) = repmat([ones(4, 6) zeros(4,6)], 1, 16);
