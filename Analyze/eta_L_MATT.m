...
%                             figure(7)
%                             hold on
%                             x = linspace(0,1,length(pathDB.eta_L(start(j,i):stop(j,i))));
%                             y = pathDB.eta_L(start(j,i):stop(j,i),i);
...
%                             hold on
%                             x = linspace(0,1,length(pathDB.eta_L(start(j,i):stop(j,i))));
%                             y = rad2deg(pathDB.eta_L(start(j,i):stop(j,i),i));
% 
% %                             plot(x,y,'color', color(i,1:3))
%                             plot(x,y,'color','blue')
%                             ylabel('wing pitch - left wing [deg]')
%                        end 
%                     end   
%                 end                         
%             end  
...
for i = seqs_plot % individuals
    for j = 1:length(stop(:,i))-1 % wingbeats
        ...
        if A > 0 && B > 0 && C > 0     
            x = linspace(0,0.5,length(pathDB.eta_L(A:B)));
            x2 = linspace(0.5,1,length(pathDB.eta_L(B:C)));
            y = rad2deg(pathDB.eta_L(A:B,i));
            y2 = rad2deg(pathDB.eta_L(B:C,i));
        ...
        if C > 0 && D > 0
            ...
            E = linspace(0,1.5,length(pathDB.eta_L(C:D)));
            F = rad2deg(pathDB.eta_L(C:D,i));
