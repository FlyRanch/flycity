...
% panel data
poe_hor = 8*9 + 4; % horizontal position of point of expansion (0 point in cali)
...
for i=1:pattern.x_num
    ...
    for j=2:pattern.y_num
        ...
        values(j,1:3) = [R 2*R/pixperm rad2deg(2*R/pixperm/d)];
...
directory_name = cd;
str = [directory_name '\Pattern_binary_pointexpansion_MIDpos_shift'];  % name must begin with ?Pattern_?
