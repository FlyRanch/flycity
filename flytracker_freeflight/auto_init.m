...
%             end
            ...
            % use ManualFit calib if exists
            if isfield(ManualFit,'DLT')
                DLT = ManualFit.DLT;
                cam = ManualFit.cam;
            else
...
%             load(['ICFromKine/' PAR.FileFromKine]);
            ...
            % use ManualFit params if they exist
...
%             % 'srcdkf_const' saved
%             
%             delete([PAR.solutionpath PAR.solutiondirname '/fly' sprintf(['%0',num2str(PAR.digits),'d'],frame-PAR.framesample) '.mat'])
