...
% Global transformation for all body parts (and scaling!) - edited May 26,
% 2013 to bypass libblas.so.3gf c++ library. May increase run time but
% should preserve function across platforms.
for k = 1:length(x)
    if k == 1
...
%function [xOut, yOut, zOut] = xformq_surf_NEW (x, y, z, Tvec, qbody, S)
%     q = qbody;
%     
%     Qp = [q(4) -q(3)  q(2) q(1);
%           q(3)  q(4) -q(1) q(2);
...
%     y1 = reshape(y, 1, m*n);
%     z1 = reshape(z, 1, m*n);
%     pts = [x1; y1; z1; ones(1,m*n)];
% 
%     ptsOut = S * T * pts;
% 
%     xOut = reshape(ptsOut(1,:), m, n);
%     yOut = reshape(ptsOut(2,:), m, n);
%     zOut = reshape(ptsOut(3,:), m, n);
